...
class MiniMapRenderer:
    ...
    @classmethod
    def render(cls, console: tcod.console.Console, start_x: int, start_y: int, game_map, player, pet, entities):
        # 枠線描画
        console.draw_frame(
            x=start_x, y=start_y, width=cls.WIDTH + 2, height=cls.HEIGHT + 2,
            title="🗺️MAP", fg=(80, 120, 180), bg=(10, 14, 22)
        )
...
class DynamicLighting:
    ...
    @staticmethod
    def calculate_distance_intensity(x: int, y: int, px: int, py: int, max_radius: float = 8.0) -> float:
...
class GaugeBar:
    ...
    @staticmethod
    def render(current: int, maximum: int, length: int = 10, fill_char: str = "■", empty_char: str = "□") -> str:
        if maximum <= 0:
...
class TutorialManager:
    ...
    def check_triggers(self, trigger_condition: str, completed_set: set) -> Optional[TutorialGuide]:
...
class NotificationManager:
    ...
    def notify(self, title: str, message: str, category: str = "general", color: Tuple[int, int, int] = (255, 215, 0), duration: int = 30) -> None:
        self.active_notifications.append(FloatingNotification(
    ...
    def get_latest(self) -> Optional[FloatingNotification]:
...
class SkillTreeUI:
    ...
    @staticmethod
    def format_tree_summary(tree_id: str, tree_name: str, tiers_count: int, learned_count: int) -> str:
        return f"【{tree_name}】 習得済: {learned_count}/{tiers_count} ティア"

    @staticmethod
    def format_tier_line(tier_name: str, cost: int, learned: bool, can_learn: bool, frame_count: int = 0) -> str:
    ...
    @classmethod
    def render_circuit_overlay(cls, console: tcod.console.Console, x: int, y: int, width: int, height: int, frame_count: int):
...
def play_pet_fusion_fx(engine: Any, pet1_name: str, pet2_name: str, result_name: str) -> None:
    ...
    for _ in range(25):
        engine.particles.append(Particle(
            ...
            color=random.choice([(255, 200, 100), (200, 100, 255), (100, 255, 220), (255, 255, 255)]),
            lifetime=15,
            char=random.choice(["*", "✨", "+", "⭐"])
        ))
    engine.floating_texts.append(FloatingText(
        x=px, y=py - 1,
    ...
    def apply_atmosphere(
        ...
        for vy in range(view_h):
            for vx in range(view_w):
                ...
                noise_val = math.sin(mx * 0.15 + t) * math.cos(my * 0.2 - t * 0.8) + math.sin((mx + my) * 0.1 + t * 0.5)
                # 0.0 ~ 1.0 に正規化
                ...
                for layer in range(1, 4):
                    ...
                    layer_noise = math.sin(mx * (0.1 * layer) + layer_t) * math.cos(my * (0.1 * layer) - layer_t)
                ...
                if weather == "fog" and intensity > 0.65:
                    ...
                    # アルファ合成
                    nbg_r = min(255, int(cur_bg[0] * (1.0 - alpha) + (insane_tint[0] + 150) * alpha))
                    nbg_g = min(255, int(cur_bg[1] * (1.0 - alpha) + (insane_tint[1] + 160) * alpha))
                    nbg_b = min(255, int(cur_bg[2] * (1.0 - alpha) + (insane_tint[2] + 180) * alpha))
            ...
            if f % 2 == 0:
                for y in range(h):
                    for x in range(w):
                        ...
                        console.bg[x, y] = (255 - cur_bg[0], 255 - cur_bg[1], 255 - cur_bg[2])
        ...
        if is_poisoned:
            ...
            for y in range(0, h, 2):
                ...
                for x in range(0, w, 3):
                    ...
                    console.fg[x, scan_y] = (int(cur_fg[0] * 0.6), min(255, cur_fg[1] + 60), int(cur_fg[2] * 0.6))
...
class ItemInspectorUI:
    ...
    @classmethod
    def render_inspection(cls, console: tcod.console.Console, item: Any, appraisal_level: int = 1, x: int = 10, y: int = 12) -> None:
        """鑑定レベルと素材に応じた超詳細クローズアップの描画"""
        bw, bh = 60, 14
        console.draw_rect(x=x, y=y, width=bw, height=bh, ch=0, bg=(14, 18, 28))
        console.draw_frame(x=x, y=y, width=bw, height=bh, title=f" 🔬 精密インスペクター: {item.display_name} ", fg=(255, 215, 0))

        # アスキーアート拡大図の描画
        art_lines = cls.ITEM_ART.get(getattr(item, "category", ""), cls.ITEM_ART["default"])
        for idx, line in enumerate(art_lines):
            console.print(x=x + 3, y=y + 3 + idx, string=line, fg=getattr(item, "color", (200, 200, 255)))
        ...
        if appraisal_level >= 1:
            console.print(x=x + 22, y=y + 4, string=f"重量: {getattr(item, 'weight', 1.0)}s   価値: {getattr(item, 'value', 0)}G", fg=(200, 240, 255))
        if appraisal_level >= 3:
            console.print(x=x + 22, y=y + 6, string="刻印: 『エルス領主の鍛冶職人による微細な銘』", fg=(160, 220, 180))
            console.print(x=x + 22, y=y + 7, string="微細摩耗: 刃こぼれなし (耐久度 100%)", fg=(180, 200, 220))
        if appraisal_level >= 5:
            console.print(x=x + 22, y=y + 9, string="神気共鳴: ★★★★★ (神の祝福が宿っている)", fg=(255, 215, 80))
        else:
            console.print(x=x + 22, y=y + 9, string="隠された刻印: (鑑定スキル Lv.3 以上で可視化)", fg=(100, 100, 120))
...
class EmotionalUI:
    ...
    def draw_emotional_frame(
        ...
        # 2. 歓喜・祝福: 虹色グラデーション
            ...
            q = int(255 * (1 - f))
            t = int(255 * f)
            if hi == 0: fg_col = (255, t, 0)
            elif hi == 1: fg_col = (q, 255, 0)
            elif hi == 2: fg_col = (0, 255, t)
            elif hi == 3: fg_col = (0, q, 255)
            elif hi == 4: fg_col = (t, 0, 255)
            else: fg_col = (255, 0, q)
    ...
    def render_cinematic_logs(
        ...
        for i, lmsg in enumerate(recent):
            ...
            if is_latest and lmsg.level in ("WARNING", "DANGER", "CRITICAL"):
                ...
                console.print(x=start_x + shake_x, y=y_pos, string=text, fg=(pulse_r, pulse_g, pulse_b))
            elif is_latest and lmsg.level == "SUCCESS":
                # 祝福の黄金フラッシュ
                gold_pulse = int(50 * math.sin(frame_count * 0.4))
                console.print(
                    x=start_x, y=y_pos, string=text,

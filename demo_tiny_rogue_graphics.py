...
from systems import MonsterPreset, CombatSystem
from fx_manager import FXManager
from core_framework import EventBus, Point
from render_context import RenderContext
from map_engine import GameMap, TILE_REGISTRY
from core.tile_atlas import TileAtlas
from feature_flags import set_flag, is_enabled
from ui_fx_systems import GaugeBar, DynamicLighting
from core.tiny_rogue_tiles import get_dungeon_tile_id, get_item_tile_id
...
class DemoEngine:
    ...
    def __init__(self):
        ...
        self.player = Entity(
            ...
            color=(255, 255, 255),
            name="Hero",
            attributes=Attributes(strength=12, endurance=12, dexterity=12, 
                                  perception=12, learning=12, will=12, magic=12, charisma=12)
        )
        ...
        self.pet = Entity(
            ...
            color=(255, 200, 100),
            name="Ciel",
            attributes=Attributes(strength=10, endurance=10, dexterity=12,
                                  perception=10, learning=10, will=10, magic=8, charisma=14)
        )
        ...
        self.monsters = []
        monster_types = ['slime', 'goblin', 'orc', 'kobold', 'snail', 'red_slime', 
                         'hound_fire', 'rogue_thief', 'novice_wizard', 'minotaur', 'lich', 'dragon_red']
    ...
    def update(self):
        ...
        if self.frame_count % 30 == 0:
            ...
            WeatherAtmosphereLayer.spawn_weather_particles(
                self.fx_manager, self.current_weather, 
                cam_x, cam_y, VIEW_WIDTH, VIEW_HEIGHT, self.frame_count
            )
    ...
    with tcod.context.new(
        ...
        while True:
            ...
            WeatherAtmosphereLayer.apply_atmosphere(
                root_console, cam_x, cam_y, VIEW_WIDTH, VIEW_HEIGHT,
                engine.current_weather, engine.frame_count,
            ...
            root_console.print(2, ui_y + 1, f"Tiny Rogue GFX: {'ON' if is_enabled('ENABLE_TINY_ROGUE_GFX') else 'OFF'} (Press G to toggle)", fg=(100, 255, 100))
            root_console.print(2, ui_y + 2, f"Weather: {engine.current_weather} (Press W to cycle) | Frame: {engine.frame_count}", fg=(170, 170, 170))
            root_console.print(2, ui_y + 3, "Keys: 1=Magic 2=Fire 3=Ice 4=Lightning 5=Poison 6=Heal 7=Teleport 8=Explosion 9=Sparkle 0=Smoke", fg=(200, 200, 150))
            ...
            for event in tcod.event.get():
                if isinstance(event, tcod.event.KeyDown):
                    if event.sym == tcod.event.KeySym.ESCAPE:
                        ...
                        idx = weathers.index(engine.current_weather) if engine.current_weather in weathers else 0
                        ...
                        engine.player.y -= 1
                        engine.fx_manager.spawn_footstep_particles(engine.player.x, engine.player.y, "stone", (0, -1))
                        ...
                        engine.player.x -= 1
                        engine.fx_manager.spawn_footstep_particles(engine.player.x, engine.player.y, "stone", (-1, 0))

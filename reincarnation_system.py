...
class ReincarnationRegistry:
    ...
    def get(self, reincarnation_id: str) -> Optional[ReincarnationData]:
        """特定の転生データを取得"""
...
class ReincarnationManager:
    ...
    def __init__(self, registry: Optional[ReincarnationRegistry] = None):
    ...
    def can_reincarnate(self, player: Any) -> bool:
        ...
        return (player.level >= reincarnation_data.min_level and 
                player.level <= reincarnation_data.max_level)
    
    def reincarnate(self, player: Any, engine: Optional[Any] = None) -> bool:
        """転生を実行"""
        ...
        try:
            ...
            mgr = engine.meta_progression_manager if (engine and hasattr(engine, "meta_progression_manager")) else MetaProgressionManager()
        ...
        if engine and hasattr(engine, "log"):
            engine.log(f"★転生を実行した！ (転生回数: {player.reincarnation_count}, 累計レベル: {getattr(player, 'total_level_earned', 0)})", (255, 215, 0))
            if player.cycle_modifiers:
                mod_names = "、".join([m.get("name", "") for m in player.cycle_modifiers if isinstance(m, dict)])

...
class WorldStateRegistry:
    ...
    _instance: Optional[WorldStateRegistry] = None
...
class WorldStateManager:
    ...
    def __init__(self, registry: Optional[WorldStateRegistry] = None):
    ...
    def set_phase(self, phase: WorldPhase, engine: Optional[Any] = None) -> None:
    ...
    def set_variable(self, player: "Entity", var_name: str, value: Any, engine: Optional[Any] = None) -> None:
        """ワールド変数を更新 (Step 53)"""
        if not player:
            return
        player.story_variables[var_name] = value
    def generate_world_news(self, engine: Optional[Any] = None) -> List[str]:
        """世界イベントニュースの自動生成 (Step 8.1)"""
        import random
        actors = ["冒険者『ロキ』", "パルミア王室親衛隊", "吟遊詩人のルカ", "ヴェルニースの炭鉱夫", "魔道士組合の斥候"]
        actions = ["地下深層のドラゴンを討伐した！", "古代遺跡で伝説の遺物を発見した。", "隣国の盗賊団を壊滅させた。", "街の復興支援を完了した。"]
        festivals = ["パルミアで収穫祭が開催されている。", "ポート・カプールで大バザールが開幕した。", "ノイエルで聖夜祭の準備が始まった。"]
    ...
    def get_action_echo(self, player: "Entity", faction_id: str = "adventurer_guild") -> Optional[str]:

...
class DungeonGenerationFeedback:
    ...
    boss_room_coords: Optional[Dict[str, Any]] = None
    boss_room_id: str = ""
    entrance_coords: Optional[Dict[str, Any]] = None
    exit_coords: Optional[Dict[str, Any]] = None
    total_rooms: int = 0
    total_traps: int = 0
    total_enemies: int = 0
    ...
    def process_generation_result(
        ...
        for floor in floors:
            ...
            if entrance and entrance_coords is None:
                entrance_coords = {"floor": floor_num, "x": entrance[0], "y": entrance[1]}
    ...
    def update_quest_objectives(
        ...
        for obj in quest_objectives:
            ...
            if target_element == "generated_floors":
                obj.current_count = min(obj.required_count, self._feedback_cache[obj.objective_id].generated_floors if obj.objective_id in self._feedback_cache else 0)
                # または feedback から直接
                pass  # 下で統一処理
        ...
        for obj in quest_objectives:
            ...
            elif target == "boss_room_id" and feedback.boss_room_id:
                obj.current_count = 1
            elif target == "entrance_coords" and feedback.entrance_coords:
                obj.current_count = 1
            elif target == "exit_coords" and feedback.exit_coords:
                obj.current_count = 1
            
            # 完了判定
            if obj.current_count >= obj.required_count and not obj.is_completed:
                obj.is_completed = True
                logs.append(f"【目的達成】{obj.description} ({obj.current_count}/{obj.required_count})")
...
class DungeonQuestPipeline:
    ...
    def generate_synced_quest_dungeon(
        ...
        from quest_dungeon_spec import DungeonSpec, build_dungeon_spec_from_yaml
    ...
    def _load_spec(self, spec_id: str):
        ...
        if spec_id == "default_dungeon":
            from quest_dungeon_spec import DungeonSpec, FloorSpec, RoomSpec, TrapSpec, EnemySpec, RoomType, TrapType, EnemyRole
            return DungeonSpec(

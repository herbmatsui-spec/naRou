...
class DialogueManager:
    ...
    @classmethod
    def get_dialogue(cls, speaker: Any, player: Any, engine: Optional[Any] = None) -> Tuple[str, str]:
        ...
        if engine and hasattr(engine, "world_state_manager"):
            ...
            if "村の長" in speaker_name:
                if phase == WorldPhase.BEGINNING:
                    return speaker_name, "「若き旅人よ、この古文書を読み解き、世界の異変を止めてくれぬか。」"
                elif phase == WorldPhase.AWAKENING:
                    return speaker_name, "「古の守護者を倒したか！ さすがじゃ。次は各地に散らばる断片を集めるのじゃ。」"
                elif phase == WorldPhase.EXPLORATION:
                    return speaker_name, "「世界の真実が見えてきたな。だが、闇の勢力も動き出しておるぞ。」"
        ...
        if hasattr(engine, "relationship_manager") and hasattr(player, "character_relationships"):
            ...
            if speaker_name in rel_dict:
                ...
                if trust >= 50:
                    return speaker_name, f"「{player.name}さん、いつも頼りにしていますよ！」"
                elif trust <= -30:

"""
Quest Condition Evaluator - Step 3
AST評価エンジン：プレイヤー/ワールド状態 → bool
"""

from __future__ import annotations
from typing import Any, Dict, List, Optional, Set, Union, TYPE_CHECKING
from dataclasses import dataclass, field
from enum import Enum
import time

from quest_condition_ast import (
    ConditionNode, LeafCondition, NotCondition, AndCondition, OrCondition, XorCondition
)

if TYPE_CHECKING:
    from entity import Entity
    from game import Engine
    from world_state_system import WorldStateManager, WorldPhase
@dataclass
class DictContext:
    """入れ子辞書で状態を保持する標準コンテキスト実装 (後方互換性)。"""

    _state: Dict[str, Any]

    def __post_init__(self):
        if not isinstance(self._state, dict):
            raise TypeError("DictContext state must be a dictionary")

    def resolve(self, key: str) -> Any:
        """ドット区切りパス (例: ``player.level``) を状態値に解決する。"""
        node: Any = self._state
        for part in key.split("."):
            if isinstance(node, dict) and part in node:
                node = node[part]
            else:
                return None
        return node



@dataclass
class EvaluationContext:
    """条件評価のためのコンテキスト（プレイヤー/ワールド状態へのアクセス）"""

    player: Optional["Entity"] = None
    engine: Optional["Engine"] = None
    world_state: Optional["WorldStateManager"] = None
    extra_vars: Dict[str, Any] = field(default_factory=dict)

    # キャッシュ
    _kill_counts: Dict[str, int] = field(default_factory=dict)
    _collect_counts: Dict[str, int] = field(default_factory=dict)
    _visit_flags: Set[str] = field(default_factory=set)
    _variable_values: Dict[str, Any] = field(default_factory=dict)
    _skill_levels: Dict[str, int] = field(default_factory=dict)
    _pet_data: Dict[str, Any] = field(default_factory=dict)

    def __post_init__(self):
        self._load_from_player()

    def _load_from_player(self):
        if not self.player:
            return

        # キルカウント
        if hasattr(self.player, "kill_counts"):
            self._kill_counts = dict(self.player.kill_counts)

        # 採取カウント
        if hasattr(self.player, "collect_counts"):
            self._collect_counts = dict(self.player.collect_counts)

        # 訪問フラグ
        if hasattr(self.player, "visited_locations"):
            self._visit_flags = set(self.player.visited_locations)

        # ストーリ変数
        if hasattr(self.player, "story_variables"):
            self._variable_values = dict(self.player.story_variables)

        # スキルレベル
        if hasattr(self.player, "skills"):
            for skill_id, skill_data in self.player.skills.items():
                if isinstance(skill_data, dict):
                    self._skill_levels[skill_id] = skill_data.get("level", 0)
                elif hasattr(skill_data, "level"):
                    self._skill_levels[skill_id] = skill_data.level

        # ペットデータ
        if hasattr(self.player, "pets"):
            for pet in self.player.pets:
                if hasattr(pet, "species"):
                    self._pet_data[pet.species] = {
                        "contract_tier": getattr(pet, "contract_tier", 0),
                        "fusion_history": getattr(pet, "fusion_history", []),
                        "evolution_stage": getattr(pet, "evolution_stage", 0),
                    }

    def resolve(self, key: str) -> Any:
        """
        ドット区切りパスを解決する
        例: "player.level", "player.kill_counts.goblin", "world_state.variables.gold"
        """
        # 特殊プレフィックスを処理
        if key.startswith("player."):
            return self._resolve_player_key(key[7:])  # Remove "player."
        elif key.startswith("world_state."):
            return self._resolve_world_state_key(key[12:])  # Remove "world_state."
        elif key.startswith("extra."):
            return self._resolve_extra_key(key[6:])  # Remove "extra."
        elif key.startswith("flags."):
            # フラグはストーリー変数の中で管理される
            flag_key = key[6:]  # Remove "flags."
            return self._resolve_player_key(f"story_variables.{flag_key}")
        else:
            # デフォルトはプレイヤーキーとして扱う
            return self._resolve_player_key(key)

    def _resolve_player_key(self, key: str) -> Any:
        """プレイヤー関連のキーを解決"""
        if key == "level":
            return getattr(self.player, "level", 1)
        elif key == "gold":
            return getattr(self.player, "gold", 0)
        elif key == "job":
            return getattr(self.player, "job", "")
        elif key.startswith("kill_counts."):
            target = key.split(".", 1)[1]
            return self._kill_counts.get(target, 0)
        elif key.startswith("collect_counts."):
            target = key.split(".", 1)[1]
            return self._collect_counts.get(target, 0)
        elif key.startswith("visited_locations."):
            # これは訪問済みかどうかのチェックになる
            target = key.split(".", 1)[1]
            visited_locations = getattr(self.player, "visited_locations", set())
            return target in visited_locations
        elif key.startswith("story_variables."):
            target = key.split(".", 1)[1]
            return self._variable_values.get(target, None)
        elif key.startswith("skills."):
            parts = key.split(".", 2)
            if len(parts) >= 3:
                skill_id, attr = parts[1], parts[2]
                skills = getattr(self.player, "skills", {})
                skill_data = skills.get(skill_id, {})
                if isinstance(skill_data, dict):
                    return skill_data.get(attr, None)
                elif hasattr(skill_data, attr):
                    return getattr(skill_data, attr, None)
            return None
        elif key.startswith("character_relationships."):
            parts = key.split(".", 2)
            if len(parts) >= 3:
                target, attr = parts[1], parts[2]
                rels = getattr(self.player, "character_relationships", {})
                rel = rels.get(target, {})
                return rel.get(attr, None)
            return None
        elif key.startswith("faction_reputation."):
            target = key.split(".", 1)[1]
            rep = getattr(self.player, "faction_reputation", {})
            return rep.get(target, 0)
        elif key == "pets":
            return getattr(self.player, "pets", [])
        else:
            # 未知のプレイヤーキーはextra_varsから探す
            return self.extra_vars.get(key, None)

    def _resolve_world_state_key(self, key: str) -> Any:
        """ワールドステート関連のキーを解決"""
        if not self.world_state:
            return None
            
        if key == "phase":
            return self.world_state.get_phase().name
        elif key.startswith("variables."):
            target = key.split(".", 1)[1]
            if self.player:
                return self.world_state.get_variable(self.player, target)
            return None
        else:
            # 未知のワールドステートキーはextra_varsから探す
            return self.extra_vars.get(key, None)

    def _resolve_extra_key(self, key: str) -> Any:
        """extra_varsからキーを解決"""
        return self.extra_vars.get(key, None)

    # ペット関連のヘルパーメソッド
    def _has_pet_with_species(self, species: str) -> bool:
        """指定の種族のペットを持っているか"""
        return any(
            hasattr(pet, "species") and getattr(pet, "species", None) == species
            for pet in getattr(self.player, "pets", [])
        )

    def _get_pet_data(self, species: str) -> Optional[Dict[str, Any]]:
        """指定種族のペットデータを取得（最初に見つかったもの）"""
        for pet in getattr(self.player, "pets", []):
            if hasattr(pet, "species") and getattr(pet, "species", None) == species:
                return {
                    "contract_tier": getattr(pet, "contract_tier", 0),
                    "fusion_history": getattr(pet, "fusion_history", []),
                    "evolution_stage": getattr(pet, "evolution_stage", 0),
                }
        return None


def _apply_operator(op: str, left: Any, right: Any) -> bool:
    """Leaf の比較/述語演算子を実行する。（quest_condition_astからコピー）"""
    if op == "==":
        return left == right
    if op == "!=":
        return left != right
    if op in (">=", "<=", ">", "<"):
        try:
            if op == ">=":
                return left >= right
            if op == "<=":
                return left <= right
            if op == ">":
                return left > right
            return left < right
        except TypeError:
            return False
    if op in ("has", "contains"):
        if right is None:
            return bool(left)
        if isinstance(left, (list, tuple, set, dict, str)):
            return right in left
        return left == right
    if op == "in":
        if right is None:
            return bool(left)
        if isinstance(right, (list, tuple, set, dict, str)):
            return left in right
        return False
    if op == "exists":
        return left is not None
    if op == "truthy":
        return bool(left)
    raise ValueError(f"未対応の演算子: {op!r}")

def evaluate(node: ConditionNode, context: Any) -> bool:
    """
    後方互換性のための評価関数。
    条件ノードを評価し、真偽を返す（ノードの evaluate に委譲）。
    """
    return node.evaluate(context)


def evaluate_condition(
    node: ConditionNode,
    player: Optional["Entity"] = None,
    engine: Optional["Engine"] = None,
    world_state: Optional["WorldStateManager"] = None,
    extra_vars: Optional[Dict[str, Any]] = None
) -> bool:
    """
    条件ノードを評価するエントリーポイント
    
    Usage:
        result = evaluate_condition(ast, player=player, engine=engine)
    """
    context = EvaluationContext(player, engine, world_state, extra_vars or {})
    return node.evaluate(context)


# テスト用
if __name__ == "__main__":
    from quest_condition_parser import parse_condition

    # モックプレイヤー
    class MockPlayer:
        level = 10
        gold = 5000
        job = "mage"
        kill_counts = {"goblin": 7, "slime": 3}
        collect_counts = {"herb": 5}
        visited_locations = {"shrine", "village"}
        story_variables = {"boss_defeated": False, "gold": 5000}
        skills = {"fire_mastery": {"level": 3}}
        character_relationships = {"npc_001": {"trust": 60}}
        faction_reputation = {"adventurer_guild": 150}
        pets = []

    class MockPet:
        species = "fenrir"
        contract_tier = 2
        fusion_history = ["ice"]
        evolution_stage = 1

    class MockEngine:
        class TimeSystem:
            def get_current_time(self):
                class T: hour = 10; minute = 30
                return T()
        class WeatherSystem:
            def get_current_weather(self): return "clear"
        class CalendarSystem:
            def get_current_season(self): return "spring"
            def get_moon_phase(self): return 3
        time_system = TimeSystem()
        weather_system = WeatherSystem()
        calendar_system = CalendarSystem()

    class MockWorldState:
        def get_phase(self):
            class Phase: name = "EXPLORATION"
            return Phase()
        def get_variable(self, player, var_name, default=None):
            return player.story_variables.get(var_name, default)

    player = MockPlayer()
    player.pets = [MockPet()]
    engine = MockEngine()
    world_state = MockWorldState()

    # テストケース（ドット記法キーを使用）
    test_cases = [
        ("player.kill_counts.goblin>=", 5, True),
        ("player.kill_counts.goblin>=", 10, False),
        ("player.kill_counts.goblin>=", 5, True) and ("player.collect_counts.herb>=", 3, True),  # AND
        ("player.level>=", 10, True) and ("player.job", "==", "mage", True),  # AND
        ("player.story_variables.boss_defeated", "==", False, True),  # False
    ]

    print("=== Testing Quest Condition System ===\n")
    
    passed = 0
    total = 0
    
    for i in range(0, len(test_cases), 3):
        if i + 2 < len(test_cases):
            key = test_cases[i]
            op = test_cases[i+1]
            value = test_cases[i+2]
            expected = test_cases[i+3] if i+3 < len(test_cases) else False
            
            # キーと演算子と値を組み合わせて条件文字列を作る
            condition_str = f"{key}{op}{value}"
            total += 1
            
            try:
                # パース
                ast = parse_condition(condition_str)
                # 評価
                result = evaluate_condition(ast, player, engine, world_state)
                # 結果チェック
                if result == expected:
                    print(f"✓ {condition_str:<35} => {result}")
                    passed += 1
                else:
                    print(f"✗ {condition_str:<35} => {result} (expected {expected})")
            except Exception as e:
                print(f"✗ {condition_str:<35} => ERROR: {e}")
    
    print(f"\n結果: {passed}/{total} 成功")
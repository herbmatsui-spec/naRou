...
def draw_entity(img, draw, ent, pulse=0):
    ...
    if sprite:
        if pulse > 0:
            c = ent.color
            draw.rectangle([px, py, px+SCEL-1, py+SCEL-1], fill=(min(255, c[0]+pulse), c[1], c[2]))
...
def draw_minimap(draw, dmap, player_x, player_y, x_off=660, y_off=52):
    draw.rectangle([x_off-2, y_off-2, x_off+34, y_off+26], fill=(15,18,28), outline=(50,60,80))
    for ry, row in enumerate(dmap):
        for rx, ch in enumerate(row):
            px = x_off + rx * 2
            py = y_off + ry * 2
    ...
    draw.rectangle([x_off+player_x*2, y_off+player_y*2, x_off+player_x*2+2, y_off+player_y*2+2], fill=C_PLAYER)
...
def gen_scene_1_dungeon_explore():
    ...
    for fi, (px, py) in enumerate(path):
        ...
        if fi >= 5: player.emote = 'alert'

        draw_hud(draw, "主人公", 85, 100, 40, 50, 10, "廃棄層 B7F", toxicity=15, skills="3個", gold=1200)
        draw_dungeon(img, draw, DUNGEON_SLUM)
        draw_minimap(draw, DUNGEON_SLUM, player.x, player.y)
        for e in enemies:
            draw_entity(img, draw, e)
            if e.hp > 0: draw_hp_bar_above(draw, e, e.hp/e.max_hp, C_ENEMY)
        draw_entity(img, draw, pet, pulse=int(20*math.sin(fi*0.5)))
        draw_entity(img, draw, player, pulse=int(30*math.sin(fi*0.8)))

        if fi >= 5:
            log_lines_show = log_lines + [("ミダス重装兵 を発見！ 《解析(X)》で弱点を確認しよう。", (255, 200, 100))]
        else:
...
def gen_scene_2_scan_analyze():
    ...
    for fi in range(20):
        img = Image.new("RGB", (W, H), C_BG)
        draw = ImageDraw.Draw(img)
        draw_hud(draw, "主人公", 85, 100, 38, 50, 10, "廃棄層 B7F", toxicity=15, skills="3個", gold=1200)
...
def gen_scene_3_devour_combat():
    ...
    for fi in range(28):
        ...
        draw_hud(draw, "主人公", 80, 100, 35, 50, 10, "廃棄層 B7F", toxicity=tox, skills="3→4個" if fi >= 16 else "3個", gold=1200)
        draw_dungeon(img, draw, DUNGEON_SLUM)
        draw_minimap(draw, DUNGEON_SLUM, player.x, player.y)
...
def gen_scene_4_synthesis_base():
    ...
    for fi in range(28):
        ...
        draw_hud(draw, "主人公", 95, 100, 45, 50, 12, "スラム地下拠点 Lv.2", toxicity=10, skills="6個", gold=gold)
...
def gen_scene_5_boss_warp():
    ...
    for fi in range(32):
        ...
        draw_hud(draw, "主人公 [神格覚醒]", 400, 450, 200, 225, 50, "バベルタワー 最上層", toxicity=65, skills="22個", gold=45000)
        ...
        if fi < 24:
            ...
            for r in range(3):
                ...
                draw.ellipse([cx-radius, cy-radius, cx+radius, cy+radius], outline=(100+r*50, 200, 255), width=3)
...
def main():
    ...
    for out in [out_main, out_asset]:
        all_frames[0].save(out, save_all=True, append_images=all_frames[1:], duration=int(1000 / FPS), loop=0, optimize=True)

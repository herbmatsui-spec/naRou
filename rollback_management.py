        ...
        rollback["steps"] = [
            {
                "name": "stop_traffic",
                "action": "stop_traffic",
                "params": {"version": rollout_id},
            },
            {
                "name": "restore_database",
                "action": "restore_database",
                "params": {"target": target_version},
            },
            {
                "name": "restore_config",
...
def main():
    ...
    parser.add_argument(
        "--create",
        nargs=3,
        metavar=("ROLLOUT_ID", "TARGET_VERSION", "REASON"),
    ...
    parser.add_argument(
        "--auto",
        nargs=2,
        metavar=("ROLLOUT_ID", "HEALTH_URL"),
        help="Auto-rollback monitoring",
    )
    ...
    if args.create:
        ...
        for rollback in mgr.list_rollbacks():
            print(
                f"{rollback['id']} - {rollback['rollout_id']} -> v{rollback['target_version']} - {rollback['status']}"
            )
    else:
        parser.print_help()

...
class ServantActionResult:
    ...
    skill_used_id: str | None
    skill_used_name: str | None
    target_id: str
    damage: int = 0
    heal: int = 0
    log_message: str = ""
...
class SkillEaterServantSystem:
    ...
    def capture_husk(
        self, husk_enemy: CharacterState, custom_name: str | None = None
    ) -> ServantCharacter | None:
    ...
    def transplant_skill(
        self, player: CharacterState, servant: ServantCharacter, skill_id: str
    ) -> tuple[bool, str]:
        ...
        return (
            True,
            f"《{skill_name}》を {servant.custom_name} に注入！ 自律オートタレットとして起動！（稼働限界: {servant.duration_turns}ターン）",
        )

    def crumble_servant(self, servant_id: str) -> ServantCharacter | None:
    ...
    def execute_servant_turn(
        ...
        if "com_combat_002" in skills:
            ...
            target = (
                min(wounded, key=lambda a: a.hp)
                if wounded
                else (allies[0] if allies else servant.state)
            )

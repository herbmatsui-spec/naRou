...
class MissedOpportunity:
    ...
    penalty: Dict[str, Any] = field(default_factory=dict)  # 喪失によるペナルティ（例: 評判低下, アイテム喪失）
...
class MissedOpportunitySystem:
    ...
    def record_missed_opportunity(self, opportunity_id: str, quest_id: str, reason: str,
                                  timestamp: Optional[float] = None,
                                  penalty: Optional[Dict[str, Any]] = None) -> None:
        """機会喪失を記録"""
    ...
    def get_missed_opportunities(self, quest_id: Optional[str] = None) -> List[MissedOpportunity]:
    ...
    def clear_missed_opportunities(self, quest_id: Optional[str] = None) -> None:
    ...
    def get_npc_dialogue_modifier(self, entity: Entity) -> Optional[str]:
...
def record_missed_opportunity(opportunity_id: str, quest_id: str, reason: str,
                              timestamp: Optional[float] = None,
                              penalty: Optional[Dict[str, Any]] = None) -> None:
    """機会喪失を記録するヘルパー関数"""
    MISSED_OPPORTUNITY_SYSTEM.record_missed_opportunity(opportunity_id, quest_id, reason, timestamp, penalty)


def get_missed_opportunities(quest_id: Optional[str] = None) -> List[MissedOpportunity]:
    """機会喪失を取得するヘルパー関数"""
    return MISSED_OPPORTUNITY_SYSTEM.get_missed_opportunities(quest_id)


def clear_missed_opportunities(quest_id: Optional[str] = None) -> None:
    """機会喪失の記録をクリアするヘルパー関数"""
    MISSED_OPPORTUNITY_SYSTEM.clear_missed_opportunities(quest_id)


def get_npc_dialogue_modifier(entity: Entity) -> Optional[str]:

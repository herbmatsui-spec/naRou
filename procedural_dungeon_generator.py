...
class ProceduralDungeonGenerator:
    ...
    def _generate_floor(
        ...
        # 入口・出口の接続情報
        entrance_pos = (random.randint(0, dungeon_spec.width // 4), random.randint(0, dungeon_spec.height // 4))
        exit_pos = (random.randint(3 * dungeon_spec.width // 4, dungeon_spec.width), 
    ...
    def _build_room_from_spec(
        ...
        if width <= 0: width = random.randint(6, 12)
        if height <= 0: height = random.randint(6, 12)

...
class WorldLayer:
    ...
    def load_theme(self) -> None:
        ...
        try:
            ...
            print(f"テーマロードエラー ({self.zone}:{self.biome}:{self.depth}:{self.dimension}): {e}")
            self.theme_data = self._get_default_theme()
    ...
    def get_monster_pool(self) -> Dict[str, List[str]]:
        """現在の層に基づくモンスタープールを取得"""
        return self.theme_data.get("enemy_pools", {
            "common": ["slime"],
            "uncommon": ["goblin"],
            "rare": [],
            "unique_boss": None
    ...
    def get_resources(self) -> Dict[str, List[str]]:
        """資源・アイテムテーブルを取得"""
        return self.theme_data.get("resources", {
            "common": [],
            "uncommon": [],
            "rare": []
        })
    ...
    def get_unique_boss(self) -> Optional[str]:
        """固有ボスを取得"""
    ...
    def get_entrance_position(self) -> Tuple[int, int]:
        ...
        if not self.game_map or not self.game_map.rooms:
            # デフォルト: マップ中央
            return (self.game_map.width // 2, self.game_map.height // 2) if self.game_map else (10, 10)

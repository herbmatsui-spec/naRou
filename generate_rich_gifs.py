...
def draw_sprite(canvas: Image.Image, tile_key: str, x: int, y: int, scale: int = 2, anchor: str = "nw"):
    """指定座標にタイル/スプライトを描画"""
...
def draw_emote(canvas: Image.Image, emote_name: str, x: int, y: int, size: int = 32, anchor: str = "center"):
    """指定座標にEmoteを描画"""
...
def draw_dungeon_background(canvas: Image.Image, floor_tile: str = "TR_FLOOR_01", wall_tile: str = "TR_WALL_01"):
...
def draw_health_bar(draw: ImageDraw.ImageDraw, x: int, y: int, w: int, h: int, cur_hp: float, max_hp: float, fill_color=(231, 76, 60), label=""):
    """HPバーおよびステータスバーの描画"""
...
def draw_hud(canvas: Image.Image, hp: int, max_hp: int, mp: int, max_mp: int, title: str = "A-World: Skill Eater"):
    """画面上部のステータスバーとヘッダーUI"""
...
def draw_scan_effect(canvas: Image.Image, target_x: int, target_y: int, progress: float):
    ...
    draw.line([(bx, scan_line_y), (bx + box_w, scan_line_y)], fill=(50, 255, 255, 255), width=2)
    
    # 解析中テキスト & Emote
    f = get_font(12)
    draw.text((bx, by - 18), f">> SCANNING... {int(progress * 100)}%", font=f, fill=(0, 255, 220, 255))
    draw_emote(canvas, "emote_dots3", target_x, target_y - 65, size=24, anchor="center")

def draw_devour_particles(canvas: Image.Image, src_x: int, src_y: int, dst_x: int, dst_y: int, progress: float):
    ...
    # 核本体（光球）
    r = 12
    draw.ellipse([cur_x - r, cur_y - r, cur_x + r, cur_y + r], fill=(220, 100, 255, 255), outline=(255, 255, 255, 255), width=2)
    # 中心コア
...
def draw_devour_burst(canvas: Image.Image, hero_x: int, hero_y: int, skill_name: str = "Inferno Breath"):
    ...
    by = hero_y - 110
    draw.rectangle([bx, by, bx + banner_w, by + banner_h], fill=(40, 10, 60, 230), outline=(220, 100, 255, 255), width=2)
    f = get_font(13)
    draw.text((bx + 12, by + 8), f"ACQUIRED: {skill_name}", font=f, fill=(255, 220, 100, 255))

def draw_synergy_explosion(canvas: Image.Image, target_x: int, target_y: int, progress: float, synergy_name: str = "THERMAL EXPLOSION"):
    """属性シナジー発動時の爆風・炎リング演出"""
    draw = ImageDraw.Draw(canvas)
    ...
    if r1 > 0:
        draw.ellipse([target_x - r1, target_y - r1, target_x + r1, target_y + r1], outline=(255, 120, 0, alpha1), width=4)
        
    r2 = int(max_radius * 0.6 * ease_out_quad(progress))
    if r2 > 0:
        draw.ellipse([target_x - r2, target_y - r2, target_x + r2, target_y + r2], fill=(255, 60, 0, int(alpha1 * 0.5)))
    ...
    # シナジー名ポップアップ
    f = get_font(13)
    draw.text((target_x - 70, target_y - 85), f"🔥 {synergy_name}! 🔥", font=f, fill=(255, 200, 50, 255))

def draw_damage_number(canvas: Image.Image, x: int, y: int, damage: int, progress: float, is_crit: bool = False):
    """跳ねるダメージ数値ポップアップ"""
    draw = ImageDraw.Draw(canvas)
...
def generate_combat_devour_gif(output_path: Path):
    ...
    for i in range(30):
        ...
        # 敵HPバー
        draw_health_bar(ImageDraw.Draw(c), enemy_x - 40, ey - 50, 80, 8, 120, 120, label="Abyssal Drake")
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=40, max_mp=50, title="Phase 1: Thermal Synergy Triggered!")
        ...
        draw_health_bar(ImageDraw.Draw(c), enemy_x - 40, hero_y - 50, 80, 8, enemy_hp, 120, label="Abyssal Drake")
    ...
    for i in range(45):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=30, max_mp=50, title="Phase 1: Devouring Core Skill...")
        ...
        draw_health_bar(ImageDraw.Draw(c), enemy_x - 40, hero_y - 50, 80, 8, 0, 120, label="Drake Husk")
    ...
    for i in range(35):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=120, max_hp=120, mp=60, max_mp=60, title="Phase 1: Devour Complete! (Stats Up)")
...
def draw_alchemy_station(canvas: Image.Image, center_x: int, center_y: int, skill_a_name: str, skill_b_name: str, slot_offset_x: int = 120):
    """錬金・合成炉と2つの素材スキル枠を描画"""
    draw = ImageDraw.Draw(canvas)
    ...
    draw.rectangle([slot_a_x - 60, center_y - 25, slot_a_x + 60, center_y + 25], fill=(30, 30, 45, 220), outline=(0, 200, 255, 255), width=2)
    f = get_font(12)
    draw.text((slot_a_x - 50, center_y - 8), skill_a_name, font=f, fill=(200, 240, 255, 255))
    
    # 素材スキルB（右側）
    slot_b_x = center_x + slot_offset_x
    draw.rectangle([slot_b_x - 60, center_y - 25, slot_b_x + 60, center_y + 25], fill=(45, 30, 30, 220), outline=(255, 100, 50, 255), width=2)
    draw.text((slot_b_x - 50, center_y - 8), skill_b_name, font=f, fill=(255, 220, 200, 255))

def draw_synthesis_merging(canvas: Image.Image, center_x: int, center_y: int, skill_a_name: str, skill_b_name: str, progress: float):
    ...
    draw.rectangle([slot_a_x - 50, center_y - 20, slot_a_x + 50, center_y + 20], fill=(30, 30, 45, alpha), outline=(0, 200, 255, alpha), width=2)
    f = get_font(11)
    draw.text((slot_a_x - 42, center_y - 7), skill_a_name, font=f, fill=(200, 240, 255, alpha))
    
    # スキルBスロット
    slot_b_x = center_x + cur_offset
    draw.rectangle([slot_b_x - 50, center_y - 20, slot_b_x + 50, center_y + 20], fill=(45, 30, 30, alpha), outline=(255, 100, 50, alpha), width=2)
...
def draw_alchemy_circle(canvas: Image.Image, center_x: int, center_y: int, progress: float):
    ...
    # 魔法陣の外枠円
    r = 65
    draw.ellipse([center_x - r, center_y - r, center_x + r, center_y + r], outline=(100, 200, 255, 200), width=2)
    draw.ellipse([center_x - r + 8, center_y - r + 8, center_x + r - 8, center_y + r - 8], outline=(150, 100, 255, 180), width=1)
...
def draw_synthesis_success(canvas: Image.Image, center_x: int, center_y: int, new_skill_name: str = "Inferno Storm Slash"):
    """融合成功時の稲妻・新スキル誕生バナー演出"""
    draw = ImageDraw.Draw(canvas)
    
    # 炉の上で輝くキメラスキルアイコン枠
    slot_w, slot_h = 220, 48
    draw.rectangle([center_x - slot_w // 2, center_y - 60, center_x + slot_w // 2, center_y - 60 + slot_h], 
                   fill=(60, 20, 80, 240), outline=(255, 215, 0, 255), width=3)
    
    f_title = get_font(13)
    draw.text((center_x - 90, center_y - 52), "★ CHIMERA SYNTHESIS ★", font=f_title, fill=(255, 230, 100, 255))
    f_name = get_font(12)
    draw.text((center_x - 80, center_y - 32), new_skill_name, font=f_name, fill=(100, 255, 230, 255))
...
def draw_black_market_deal(canvas: Image.Image, center_x: int, center_y: int, progress: float, earned_gold: int = 2400):
    """密売成立時のコイン散乱とゴールド獲得演出"""
    draw = ImageDraw.Draw(canvas)
    
    # 飛び散るコイン (emote_cash)
    cash_offsets = [(-50, -40), (50, -50), (-80, 10), (80, 0), (0, -70), (-30, 40), (40, 50)]
    ...
    # 取引メッセージ
    deal_box_w = 260
    draw.rectangle([center_x - deal_box_w // 2, center_y - 100, center_x + deal_box_w // 2, center_y - 60],
                   fill=(20, 30, 20, 240), outline=(50, 255, 100, 255), width=2)
    f = get_font(13)
    draw.text((center_x - 110, center_y - 92), f"CONTRABAND SOLD: +{earned_gold} G", font=f, fill=(255, 235, 80, 255))
...
def draw_inquisition_raid(canvas: Image.Image, progress: float):
    ...
    # 警告バナー
    draw.rectangle([0, HEIGHT // 2 - 35, WIDTH, HEIGHT // 2 + 35], fill=(40, 5, 5, 230), outline=(255, 40, 40, 255), width=2)
    f_warn = get_font(16)
    draw.text((WIDTH // 2 - 140, HEIGHT // 2 - 24), "⚠ INQUISITION RAID DETECTED! ⚠", font=f_warn, fill=(255, 80, 80, 255))
    f_sub = get_font(12)
    draw.text((WIDTH // 2 - 110, HEIGHT // 2 + 6), "Audit Level Critical: Contraband Confiscation Imminent", font=f_sub, fill=(255, 200, 200, 255))
...
def generate_synthesis_economy_gif(output_path: Path):
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=50, max_mp=50, title="Phase 2: Transmuting Elements...")
        p = i / 30.0
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_black_market_scene(c)
        draw_hud(c, hp=100, max_hp=100, mp=50, max_mp=50, title="Phase 2: Black Market Contraband")
        p = i / 30.0
...
def draw_laws_override_ui(canvas: Image.Image, rule_text: str = "GLOBAL DAMAGE MULTIPLIER: 1.0x -> 2.5x", cost_text: str = "COST: MAX HP -25%"):
    ...
    draw.rectangle([wx, wy, wx + win_w, wy + win_h], fill=(10, 20, 30, 245), outline=(0, 255, 230, 255), width=2)
    # タイトルバー
    draw.rectangle([wx, wy, wx + win_w, wy + 26], fill=(0, 100, 120, 255))
    f_bar = get_font(12)
    draw.text((wx + 10, wy + 6), "[!] SYSTEM LAW OVERRIDE APPLIED", font=f_bar, fill=(255, 255, 255, 255))
...
def draw_corruption_penalty(canvas: Image.Image, progress: float):
    ...
    draw_hud(canvas, hp=cur_max, max_hp=cur_max, mp=50, max_mp=50, title="Phase 3: Sanity Erosion & Law Binding")
    
    # 侵食警告
    draw.rectangle([WIDTH // 2 - 120, 50, WIDTH // 2 + 120, 85], fill=(50, 10, 20, 220), outline=(255, 60, 60, 255), width=2)
    f = get_font(12)
    draw.text((WIDTH // 2 - 100, 60), f"SANITY EROSION: +25% [CORRUPTED]", font=f, fill=(255, 120, 120, 255))
...
def draw_reincarnation_collapse(canvas: Image.Image, progress: float):
    ...
    if cur_r > 0:
        draw.ellipse([cx - cur_r, cy - cur_r, cx + cur_r, cy + cur_r], fill=(5, 5, 10, 255), outline=(120, 50, 200, 255), width=3)
        
    # 吸い込まれる歪みエフェクトアイコン
    if progress < 0.7:
        draw_emote(canvas, "emote_swirl", cx, cy, size=int(48 * (1.0 + progress)), anchor="center")
...
def draw_karma_settlement(canvas: Image.Image, progress: float):
    ...
    f_title = get_font(15)
    draw.text((WIDTH // 2 - 120, 50), "=== CYCLE SETTLEMENT ===", font=f_title, fill=(255, 215, 0, 255))
    ...
    for i in range(min(len(k_logs), max_k)):
        draw.text((WIDTH // 2 - 160, 100 + i * 28), f"> {k_logs[i]}", font=f_log, fill=(200, 240, 255, 255))
...
def draw_reborn_intro(canvas: Image.Image, progress: float):
    ...
    draw_hud(canvas, hp=120, max_hp=120, mp=60, max_mp=60, title="Cycle #2: Reborn with Inherited Skills")
    
    # 主人公スプライト
    draw_sprite(canvas, "TR_HERO_IDLE_01", WIDTH // 2, HEIGHT // 2 + 20, scale=4, anchor="center")
    ...
    # 新生バナー
    draw = ImageDraw.Draw(canvas)
    draw.rectangle([WIDTH // 2 - 150, 60, WIDTH // 2 + 150, 95], fill=(20, 40, 30, 230), outline=(0, 255, 180, 255), width=2)
    f = get_font(13)
    draw.text((WIDTH // 2 - 130, 70), "NEW INCARNATION INITIATED", font=f, fill=(100, 255, 200, 255))
...
def draw_skill_implantation(canvas: Image.Image, hero_x: int, hero_y: int, husk_x: int, husk_y: int, progress: float):
    ...
    for _ in range(3):
        ...
        draw.line([(hero_x, hero_y), (mid_x, mid_y), (cur_x, cur_y)], fill=(100, 200, 255, 180), width=1)
...
def draw_servant_awakening(canvas: Image.Image, husk_x: int, husk_y: int):
    ...
    # 覚醒テキスト
    f = get_font(12)
    draw.text((husk_x - 70, husk_y - 75), "[SERVANT TURRET AWAKENED]", font=f, fill=(255, 100, 100, 255))

def draw_servant_attack(canvas: Image.Image, servant_x: int, servant_y: int, target_x: int, target_y: int, progress: float):
    """従属者の自律射撃・エネルギー弾道演出"""
    draw = ImageDraw.Draw(canvas)
    ...
    # エネルギー弾
    r = 8
    draw.ellipse([bullet_x - r, bullet_y - r, bullet_x + r, bullet_y + r], fill=(255, 100, 50, 255), outline=(255, 255, 200, 255), width=2)
    
    # 砲火マズルフラッシュ
    if progress < 0.3:
        draw_emote(canvas, "emote_anger", servant_x - 30, servant_y, size=24, anchor="center")

def draw_servant_lifespan(canvas: Image.Image, servant_x: int, servant_y: int, remaining_turns: int):
    ...
    draw.rectangle([bx, by, bx + badge_w, by + badge_h], fill=(20, 20, 30, 220), outline=(*color, 255), width=2)
    f = get_font(11)
    draw.text((bx + 8, by + 4), f"LIFESPAN: {remaining_turns}T", font=f, fill=(255, 255, 255, 255))
...
def draw_servant_destruction(canvas: Image.Image, servant_x: int, servant_y: int, progress: float):
    ...
    # ログ
    f = get_font(12)
    draw.text((servant_x - 70, servant_y - 65), "[SERVANT DESTROYED]", font=f, fill=(180, 180, 200, 255))
...
def generate_husk_servant_gif(output_path: Path):
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=50, max_mp=50, title="Phase 4: Dead Monster Husk Discovered")
    ...
    for i in range(35):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=35, max_mp=50, title="Phase 4: Implanting Skill Circuit...")
        draw_sprite(c, "TR_HERO_IDLE_01", hx, hy, scale=4, anchor="center")
        draw_husk_sprite(c, "TR_MONSTER_01", sx, sy, scale=4)
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=35, max_mp=50, title="Phase 4: Turret Servant Online")
        draw_sprite(c, "TR_HERO_IDLE_01", hx, hy, scale=4, anchor="center")
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=35, max_mp=50, title="Phase 4: Autonomous Turret Fire")
        draw_sprite(c, "TR_HERO_IDLE_01", hx, hy, scale=4, anchor="center")
    ...
    for i in range(25):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=35, max_mp=50, title="Phase 4: Critical Lifespan Exceeded!")
        draw_sprite(c, "TR_HERO_IDLE_01", hx, hy, scale=4, anchor="center")
    ...
    for i in range(30):
        c = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255))
        draw_dungeon_background(c)
        draw_hud(c, hp=100, max_hp=100, mp=35, max_mp=50, title="Phase 4: Servant Disassembled")
        draw_sprite(c, "TR_HERO_IDLE_01", hx, hy, scale=4, anchor="center")

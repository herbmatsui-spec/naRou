...
class ResourceNode:
    ...
    NODE_DROPS: Dict[str, List[Tuple[str, float]]] = field(default_factory=lambda: {
        "herb":     [("ハーブ", 0.7), ("高級ハーブ", 0.25), ("幻のハーブ", 0.05)],
        "ore_vein": [("鉄鉱石", 0.6), ("銀鉱石", 0.3), ("金鉱石", 0.1)],
        "mushroom": [("毒キノコ", 0.5), ("旨いキノコ", 0.35), ("カオスマッシュルーム", 0.15)],
    })

    def harvest(self, player: "Entity") -> Tuple[Optional["Item"], str]:
        """採取実行"""
        from item_system import Item, CAT_FOOD, CAT_ORE
        if self.depleted:
            return None, "もう何もない。"
        self.depleted = True
        ...
        itm = Item(chosen_name, cat, "%", (150, 220, 120), self.x, self.y, base_weight=0.3, base_value=30, nutrition=500)
...
def try_craft(recipe: Recipe, player: "Entity", inventory: "Inventory") -> Tuple[bool, str, Optional["Item"]]:
    """クラフト実行 + 品質決定"""
    from item_system import Item, QUALITY_NORMAL, QUALITY_GOOD, QUALITY_MIRACLE
    # 素材チェック
    ...
    if actual_lv < recipe.required_skill_level:
        return False, f"スキル【{recipe.required_skill}】Lv{recipe.required_skill_level}以上が必要。", None
    ...
    result = Item(
        recipe.result_name, recipe.result_category, "!", color,
        base_weight=recipe.base_weight, base_value=recipe.base_value,
        quality=quality, heal_amount=35, nutrition=2000
    )

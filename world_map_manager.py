...
class WorldMapManager:
    ...
    def __init__(self, max_loaded_layers: int = 8):
        ...
        # プレイヤーの現在位置
        self.current_player_layer: Optional[Tuple[str, str, int, str]] = None
        self.current_player_pos: Optional[Tuple[int, int]] = None
    ...
    def load_layer(self, zone: str, biome: str, depth: int, dimension: str) -> Optional[GameMap]:
    ...
    def _unload_least_recently_used_layer(self) -> None:
        ...
        for entry in self.layer_priority_queue:
            if (entry.layer_key in self.loaded_layers and 
                abs(entry.last_accessed - self.loaded_layers[entry.layer_key].last_accessed) < 1.0):
    ...
    def _update_layer_priority(self, layer_key: Tuple[str, str, int, str]) -> None:
        ...
        if layer_key in self.loaded_layers:
            ...
            heapq.heappush(self.layer_priority_queue, LayerPriority(
                layer_key=layer_key,
    ...
    def set_player_position(self, zone: str, biome: str, depth: int, dimension: str, 
                          x: int, y: int) -> None:
        """プレイヤーの位置を設定"""
        layer_key = (zone, biome, depth, dimension)
    ...
    def get_player_position(self) -> Tuple[Optional[Tuple[str, str, int, str]], Optional[Tuple[int, int]]]:
        """プレイヤーの現在位置を取得"""
        return self.current_player_layer, self.current_player_pos
    
    def get_player_position_in_layer(self, zone: str, biome: str, depth: int, dimension: str) -> Optional[Tuple[int, int]]:
        """特定の層におけるプレイヤーの位置を取得"""
        layer_key = (zone, biome, depth, dimension)

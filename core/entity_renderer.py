...
class EntityAnimState:
    ...
    def update(
        self, dt: float, new_direction: int, new_state: str, is_attacking: bool = False
    ) -> bool:
...
class EntityRenderer:
    ...
    def register_entity(
        self, tile_id: str, x: int, y: int, direction: int = 0, state: str = "idle"
    ) -> int:
        """新規エンティティ登録、ID返却"""
    ...
    def get_subimage(self, eid: int) -> Any | None:
        """tcod用サブイメージ取得"""
        anim = self.entity_anims.get(eid)
        if not anim:
            return None

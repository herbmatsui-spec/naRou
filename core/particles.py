...
class SDFCollision:
    ...
    def query(self, pos: np.ndarray) -> float:
        ...
        if not (0 <= gx < self.grid_size - 1 and 
                0 <= gy < self.grid_size - 1 and 
                0 <= gz < self.grid_size - 1):
            return 1e6
    ...
    def collide_particle(self, pos: np.ndarray, vel: np.ndarray, radius: float, 
                         restitution: float = 0.3) -> tuple:
...
class Emitter:
    ...
    def __init__(self, emitter_type: Type = Type.POINT, rate: float = 100.0,
                 position: np.ndarray = None, direction: np.ndarray = None,
                 params: dict = None):
        ...
        self.direction = direction if direction is not None else np.array([0, 1, 0], dtype=np.float32)
        self.params = params if params is not None else {}
        self.accumulator = 0.0
        self.active = True
    
    def update(self, dt: float, particle_buffer: ParticleBuffer, 
               base_velocity: np.ndarray, base_color: np.ndarray,
               base_size: float, base_life: float, material_id: int = 0) -> int:
        ...
        for idx in indices:
            self._init_particle(idx, particle_buffer, base_velocity, base_color, base_size, base_life, material_id)
        
        return count
    
    def _init_particle(self, idx: int, buffer: ParticleBuffer,
                       base_vel: np.ndarray, base_color: np.ndarray,
                       base_size: float, base_life: float, material_id: int) -> None:
        ...
        buffer.rotation[idx] = np.array([0.0, self.params.get('rot_speed', 0.0), 0.0, 0.0], dtype=np.float32)
...
def simulate_particles_cpu(buffer: ParticleBuffer, dt: float, 
                          curl_noise: Optional[CurlNoise] = None,
                          sdf_collision: Optional[SDFCollision] = None,
                          gravity: np.ndarray = None,
                          global_wind: np.ndarray = None) -> None:
    ...
    for idx in alive:
        ...
        # Size interpolation
        buffer.size[idx, 2] = buffer.size[idx, 0] + (buffer.size[idx, 1] - buffer.size[idx, 0]) * life_ratio
        
        # Color interpolation (fade out)
        alpha = 1.0 - life_ratio
        buffer.color[idx, 3] = base_alpha * alpha if (base_alpha := buffer.color[idx, 3]) > 0 else alpha

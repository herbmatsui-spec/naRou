...
from core.hdr import HDRTarget, HDRCompositor, AutoExposure
from core.renderer_base import RendererBase, TileDrawCall, TextDrawCall, Viewport
...
class Compositor:
    ...
    def set_bloom_params(self, threshold: float = 1.0, intensity: float = 1.0, 
                         radius: int = 8, iterations: int = 5) -> None:
    ...
    def apply(self, console, framebuffer: np.ndarray) -> None:
        ...
        for y in range(h):
            for x in range(w):
                ...
                console.rgb[y, x]["fg"] = (color >> 16 & 0xFF, color >> 8 & 0xFF, color & 0xFF)
...
def apply_pseudo_hdr(console, framebuffer: np.ndarray, 
                     exposure: float = 1.0, gamma: float = 2.2,
                     bloom: bool = True) -> None:

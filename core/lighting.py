...
class LightSource:
    ...
    def update_flicker(self, time: float) -> None:
        ...
        self.flicker = 1.0 + 0.12 * __import__('math').sin(time * 9.0 + self.seed) + \
...
class LightingDrawCall:
    """ライティング描画コール"""
    light_map: Optional[LightMap] = None
...
class TerminalLightingSystem:
    ...
    def __init__(self, width: int, height: int, tile_size: int = 32):
        ...
        self.light_map: Optional[LightMap] = None
...
class TerminalParticleSystem:
    ...
    def _get_from_pool(self, ptype: str) -> Optional[Particle]:
        """プールから取得"""
...
def compute_ssao_from_tiles(console, cam_x: int, cam_y: int, view_w: int, view_h: int) -> 'np.ndarray':
    """

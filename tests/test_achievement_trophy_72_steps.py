...
def test_all_72_steps_achievement_trophy_system():
    ...
    with open("data/achievements.yaml", "r", encoding="utf-8") as f:
    ...
    assert goblin_slayer.get("reward_title") == "goblin_slayer", "Step 3 Failed: reward_title mismatch"
    print("[OK] Step 3 (称号連動実績 goblin_slayer)")

    # Step 4: data/meta_progression.yaml 基本構造作成
    with open("data/meta_progression.yaml", "r", encoding="utf-8") as f:
        meta_raw = yaml.safe_load(f)
    assert meta_raw and "meta_progression" in meta_raw, "Step 4 Failed: meta_progression key missing"
    ...
    assert hasattr(e, "achievement_progress") and isinstance(e.achievement_progress, dict), "Step 8 Failed"
    assert hasattr(e, "achievement_timers") and isinstance(e.achievement_timers, dict), "Step 9 Failed"
    assert hasattr(e, "monster_killed_types") and isinstance(e.monster_killed_types, dict), "Step 10 Failed"
    assert hasattr(e, "unique_items_obtained") and isinstance(e.unique_items_obtained, list), "Step 11 Failed"
    ...
    assert hasattr(e, "reincarnation_count") and isinstance(e.reincarnation_count, int), "Step 14 Failed"
    assert hasattr(e, "total_level_earned") and isinstance(e.total_level_earned, int), "Step 15 Failed"
    assert hasattr(e, "permanent_bonuses") and isinstance(e.permanent_bonuses, dict), "Step 16 Failed"
    assert hasattr(e, "meta_progression") and isinstance(e.meta_progression, dict), "Step 17 Failed"
    assert hasattr(e, "dungeon_floors_visited") and isinstance(e.dungeon_floors_visited, set), "Step 33 Failed"
    assert hasattr(e, "play_time_seconds") and isinstance(e.play_time_seconds, int), "Step 38 Failed"
    assert hasattr(e, "last_festival_check") and isinstance(e.last_festival_check, str), "Step 42 Failed"
    assert hasattr(e, "friend_helps") and isinstance(e.friend_helps, int), "Step 56 Failed"
    assert hasattr(e, "special_items_combo") and isinstance(e.special_items_combo, list), "Step 67 Failed"
    print("[OK] Steps 6-17, 33, 38, 42, 56, 67 (entity.py 実績・メタ進行フィールド)")

    # Steps 18-24: achievement_system.py クラス・メソッド定義
    from achievement_system import AchievementData, AchievementRegistry, AchievementManager, REGISTRY
    ...
    assert "# TODO: Achievement check" in game_code or "achievement_manager.check_all_achievements" in game_code, "Step 26 Failed"
    ...
    assert loaded_eng is not None and "first_blood" in loaded_eng.player.achievements, "Step 31 Load failed"
    print("[OK] Steps 29-31 (SaveSystem 実績データ保存・復元)")

    # Steps 32, 34, 35: ダンジョン探検家実績 & カウンター
    dungeon_exp = REGISTRY.get("dungeon_explorer")
    assert dungeon_exp is not None and dungeon_exp.reward_title == "dungeon_explorer", "Step 32 Failed"
    ...
    eng.player.monster_killed_types = {"goblin": 5, "slime": 3, "orc": 2, "dragon": 1, "kobold": 4}
    ...
    eng.player.unique_items_obtained = ["ragnarok", "muramasa", "excalibur", "aegis", "gaiaring"]
    ...
    assert r_hero is not None and r_hero.requirement.get("reincarnation_count") == 5, "Step 59 Failed"
    ...
    assert "# TODO: Achievement notification" in render_code or "achievement_notifications" in render_code, "Step 70, 71 Failed"

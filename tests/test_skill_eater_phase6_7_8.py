...
class TestSkillEaterPhase678(unittest.TestCase):
    ...
    def test_meta_counter_boss_mechanic(self):
        ...
        succ2, msg = self.quest_sys.check_boss_meta_counter(player, "midas_ceo")
    ...
    def test_global_rule_override_root_access(self):
        ...
        # ROOT未所持での操作
        succ, err, _ = self.rules.override_rule("damage_multiplier", 5.0, player=player)
        self.assertFalse(succ)

        # 頭取を倒してマスタースキル(ROOT権限)取得
        self.rules.root_access_granted = True
        succ2, msg, _ = self.rules.override_rule("damage_multiplier", 10.0, player=player)
        self.assertTrue(succ2)
    ...
    def test_reincarnation_inheritance(self):
        ...
        new_player, msg = self.reincarnation_sys.process_reincarnation(
            player,
            selected_carryover_skill_ids=["rar_combat_012"]
        )

...
def test_archaeology_metagame_full_36_steps():
    ...
    sites = yaml.safe_load(open("data/archaeology_sites.yaml", encoding="utf-8"))["archaeology_sites"]
    ...
    assert set(keys.keys()) == {"goblin_rune_key", "heroic_glyph_key", "abyssal_script_key"}, "Step 8 Failed"
    ...
    assert set(truths.keys()) == {"truth_of_coexistence", "truth_of_inheritance", "truth_of_drowned_age"}, "Step 9 Failed"
    print("[OK] Step 9 (truth_codex.yaml: 3真理ノード)")

    # Step 10: ArchaeologyComponent フィールド
    comp = ArchaeologyComponent()
    for f in ("excavated_sites", "collected_fragments", "decoded_fragments",
              "owned_keys", "reached_truths", "leaned_endings", "interpretation_notes"):
    ...
    assert mgr.decode_fragment(e5, "traitor_kings_will") is False, "Step 19 Failed: 鍵なしで解読される"
    # 鍵入手時に未解読断片が自動解読される（改善③ recheck_decoding）
    mgr.acquire_key(e5, "goblin_rune_key")
    assert "traitor_kings_will" in e5.archaeology.decoded_fragments, "Step 19 Failed: 鍵ありで自動解読失敗"
    ...
    assert mgr.interpret_truth(e4, "truth_of_coexistence", "goblin_peace_bringer", "共存こそ答え") is True
    assert e4.archaeology.leaned_endings["truth_of_coexistence"] == "goblin_peace_bringer", "Step 23 Failed"
    assert e4.story_flags.get("ending_goblin_peace_bringer_unlocked_by_archaeology") is True, "Step 23 Failed"
    # 無効な候補は却下
    assert mgr.interpret_truth(e4, "truth_of_coexistence", "nonexistent_ending") is False, "Step 23 Failed: 無効候補"
    ...
    assert mgr.registry.get_site(mgr.registry.find_site_for_depth(20)).get("name") == "深淵の発掘坑", "Step 27 Failed"
    ...
    assert any(f.get("fragment_id") == "goblin_child_screams" for f in rcomp.collected_fragments if isinstance(f, dict)), "Step 30 Failed"
    ...
    # Step 33: 共有サマリー出力
    assert isinstance(mgr.export_share_summary(e4), str) and "私の到達した真実" in mgr.export_share_summary(e4), "Step 33 Failed"
    ...
    assert e6.story_flags.get("ending_goblin_peace_bringer_unlocked_by_archaeology") is True, "改善① Failed: 接続フラグ"
    assert e6.story_flags.get("spared_cubs_resolved") is True, "改善① Failed: unlock_conditions 未満たし"
    assert int(e6.ending_progress.get("goblin_peace_bringer", 0)) >= 1, "改善① Failed: ending_progress 未更新"
    assert mgr.is_ending_reachable(e6, "goblin_peace_bringer") is True, "改善① Failed: is_ending_reachable"
    ...
    assert set(groups["truth_of_coexistence"]) == {"goblin_peace_bringer", "ruthless_slayer_conqueror"}, "改善② Failed: 候補グループ"

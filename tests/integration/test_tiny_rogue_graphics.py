    ...
    def test_monster_directional_frames(self):
        ...
        for direction in range(4):
            for frame in range(4):
                uv = atlas.get_uv('TR_MONSTER_01', frame=frame, direction=direction, scale='tiny_rogue_16')
                assert uv.w == 16
                assert uv.h == 16
...
class TestTinyRogueTileRegistry:
    ...
    def test_registry_loads_tiny_rogue(self):
        ...
        assert 'tiny_rogue_16' in dir(TILE_REGISTRY) or hasattr(TILE_REGISTRY, 'atlas_tiny_rogue_meta')
...
class TestTinyRogueDungeonGeneration:
    ...
    def test_floor_variants_assigned(self):
        ...
        floor_variants = [v for pos, v in gm.tile_variants.items() 
...
class TestTinyRogueFXSystem:
    ...
    def test_loot_sparkle(self):
        ...
        sparkle_particles = [p for p in fx.particles if getattr(p, 'tile_id', None) == 'TR_EFFECT_09']
    ...
    def test_flash_on_crit(self):
        ...
        flash_particles = [p for p in fx.particles if getattr(p, 'tile_id', None) in ('TR_EFFECT_09', 'TR_EFFECT_01')]
    ...
    def test_all_effect_types(self):
        ...
        effect_types = ['magic_cast', 'fire', 'ice', 'lightning', 'poison', 'heal', 
                       'teleport', 'explosion', 'sparkle', 'smoke', 'slash', 'shockwave']
        
        for effect_type in effect_types:
            fx.particles.clear()
            getattr(fx, f'spawn_{effect_type}_effect')(10, 10)
            particles_with_tile = [p for p in fx.particles if getattr(p, 'tile_id', None)]
            assert len(particles_with_tile) > 0, f'No particles for {effect_type}'
...
class TestTinyRogueVisualRegression:
    ...
    def test_atlas_image_dimensions(self):
        ...
        img = Image.open('assets/tiles/tileset_tiny_rogue_16x16.png')
        assert img.width == 509
        assert img.height == 115
        assert img.mode == 'RGBA'
    
    def test_all_tiles_in_atlas(self):
        """Verify all 105 tile entries exist in atlas metadata."""
        import json
        with open('assets/tiles/tileset_tiny_rogue_16x16.json') as f:
            meta = json.load(f)

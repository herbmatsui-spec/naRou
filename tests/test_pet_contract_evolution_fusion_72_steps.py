...
def test_all_72_steps_pet_contract_evolution_fusion():
    ...
    with open("data/pet_contracts.yaml", encoding="utf-8") as f:
    ...
    with open("data/pet_evolutions.yaml", encoding="utf-8") as f:
    ...
    assert magic is not None and "magic_basic" in magic.get("requirements", {}).get(
        "skills", []
    ), "Step 13 Failed"
    ...
    assert (
        pet_ent.pet_ai.evolution_stage == 1 and "hound" in pet_ent.pet_ai.evolution_path
    ), "Step 43 Failed"
    ...
    with open("data/pet_fusion.yaml", encoding="utf-8") as f:
    ...
    assert dh.get("required_bond") == [400, 350] and dh.get("required_level") == [
        20,
        15,
    ], "Step 51 Failed"
    assert (
        "dragon_scale" in dh.get("required_items", [])
        and dh.get("required_facility") == "alchemy_lab"
    ), "Step 52 Failed"
    assert (
        dh.get("result_pet") == "dragon_hound" and dh.get("inheritance_rate") == 0.70
    ), "Step 53 Failed"
    ...
    from ui_fx_systems import PetUI, play_pet_fusion_fx

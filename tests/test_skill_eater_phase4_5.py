...
class TestSkillEaterPhase4And5(unittest.TestCase):
    ...
    def test_husk_capture_and_skill_transplant(self):
        ...
        enemy = CharacterState(
            id="e_husk", name="元ガードマン",
            hp=1, max_hp=50, mp=0, max_mp=20,
            atk=5, defense=5, intelligence=10, speed=5,
            is_husk=True
        )
        ...
        # スキル移植
        success, msg = self.servant_sys.transplant_skill(player, servant, "com_combat_002")
    ...
    def test_servant_turret_healing_and_crumble(self):
        servant_char = CharacterState(
            id="s1", name="衛生兵",
            hp=50, max_hp=50, mp=50, max_mp=50,
            atk=5, defense=5, intelligence=20, speed=10,
            is_husk=True
        )
        ...
        wounded_ally = CharacterState(
            id="ally", name="レジスタンス戦士",
            hp=10, max_hp=100, mp=10, max_mp=10,
            atk=20, defense=10, intelligence=5, speed=5
        )
    ...
    def test_black_market_sale_and_facility_upgrade(self):
        ...
        succ_up, msg = self.economy_sys.upgrade_facility(player, "rehab_lab")

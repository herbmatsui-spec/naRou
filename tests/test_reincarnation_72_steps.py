...
def test_all_72_steps_reincarnation_system():
    ...
    with open("data/reincarnation.yaml", "r", encoding="utf-8") as f:
    ...
    with open("data/reincarnation_inheritance.yaml", "r", encoding="utf-8") as f:
    ...
    assert inh_data.get("selective_keep", {}).get("points_per_reincarnation") == 100, "Step 4 Failed: points mismatch"
    print("[OK] Step 4 (基本継承ルール always_keep & selective_keep)")

    # Step 5: data/karma.yaml 基本構造作成
    with open("data/karma.yaml", "r", encoding="utf-8") as f:
    ...
    assert k_align.get("law_chaos", {}).get("range") == [-100, 100], "Step 6 Failed: law_chaos range"
    ...
    with open("data/reincarnation_dungeons.yaml", "r", encoding="utf-8") as f:
    ...
    assert d_first is not None and d_first.get("name") == "最初の試練", "Step 8 Failed: name mismatch"
    ...
    with open("data/reincarnation_scaling.yaml", "r", encoding="utf-8") as f:
    ...
    assert "# TODO: Reincarnation fields" in ent_code, "Step 11 Failed: placeholder comment missing"
    
    e = Entity()
    assert hasattr(e, "reincarnation_count") and isinstance(e.reincarnation_count, int), "Step 12 Failed"
    ...
    assert hasattr(e, "unlocked_reincarnation_dungeons") and isinstance(e.unlocked_reincarnation_dungeons, list), "Step 16 Failed"
    assert hasattr(e, "collected_fragments") and isinstance(e.collected_fragments, list), "Step 17 Failed"
    assert hasattr(e, "favor") and isinstance(e.favor, dict), "Step 18 Failed"
    assert hasattr(e, "inheritance_selection") and isinstance(e.inheritance_selection, dict), "Step 19 Failed"
    assert hasattr(e, "challenge_progress") and isinstance(e.challenge_progress, dict), "Step 20 Failed"
    print("[OK] Steps 11-20 (entity.py 転生・カーマ・レガシーフィールド)")

    # Steps 21-27: reincarnation_system.py
    from reincarnation_system import ReincarnationData, ReincarnationRegistry, ReincarnationManager, REGISTRY as REINC_REG
    ...
    from inheritance_system import InheritanceData, InheritanceRegistry, InheritanceManager, REGISTRY as INH_REG
    ...
    from karma_system import KarmaData, KarmaRegistry, KarmaManager, REGISTRY as KARMA_REG
    ...
    from reincarnation_dungeon_system import ReincarnationDungeonData, ReincarnationDungeonRegistry, ReincarnationDungeonManager, REGISTRY as RD_REG
    ...
    from legacy_skill_system import LegacySkillData, LegacySkillRegistry, LegacySkillManager, REGISTRY as LS_REG
    ...
    assert "soul_memory" in unlocked_l or "soul_memory" in eng.player.legacy_skills, "Step 65 Failed"
    ...
    from reincarnation_challenge_system import ReincarnationChallengeData, ReincarnationChallengeRegistry, ReincarnationChallengeManager, REGISTRY as RC_REG
    ...
    assert loaded is not None and loaded.player.reincarnation_count == 3 and loaded.player.karma_law_chaos == 50, "Step 72 Load failed"

...
from core.pixel_perfect import logical_to_physical
...
def render_scene_tcod(renderer: TCODRenderer, atlas: MSDFAtlas, seed: int) -> np.ndarray:
    ...
    # Draw some tiles
    for i in range(5):
        renderer.draw_tile(TileDrawCall(
            texture_id=0,
            x=i * 4,
            y=2,
            width=3,
    ...
    # Draw text
    renderer.draw_text(TextDrawCall(
        text="Hello naRou",
        x=2,
        y=10,
        font_size=16,
...
def compare_images(img1: np.ndarray, img2: np.ndarray, threshold: float = 1/255) -> tuple[bool, float]:

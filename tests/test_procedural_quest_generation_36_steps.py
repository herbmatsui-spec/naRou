...
def test_all_36_steps_procedural_quest_generation():
    ...
    print(f"[OK] Step 2 (アーキタイプ定義 {len(arch)}種: slay/gather/escort/explore/boss_hunt/rescue/delivery)")
    ...
    print(f"[OK] Step 5 (舞台設定 {len(settings)}種: town/forest/cave/ruins/volcano/snowfield/swamp/abyss)")
    ...
    from procedural_quest_generator import (
        QuestArchetype, DifficultyTier, RewardTable, StageSetting,
        NPCQuestTheme, QuestObjectiveSpec, GeneratedQuest,
    )
    # Step 9
    ...
    print("[OK] Steps 9-14 (QuestArchetype/DifficultyTier/RewardTable/StageSetting/NPCQuestTheme/QuestObjectiveSpec/GeneratedQuest)")

    # ---------------------------------------------------------
    # フェーズC: レジストリ (Steps 15-18)
    # ---------------------------------------------------------
    from procedural_quest_generator import QuestGenerationRegistry, REGISTRY
    # Step 15
    ...
    cq = gen._compose("board", REGISTRY.get_archetype("slay"), REGISTRY.get_difficulty("normal"),
                      REGISTRY.get_reward("gold"), REGISTRY.get_setting("forest"), 777)
    ...
    assert "{setting}" not in cq.title and "{enemy}" not in cq.title, "Step 21 Failed: template unfilled"
    # Step 22: 目的自動生成（難易度でスケール）
    easy = gen._compose("board", REGISTRY.get_archetype("slay"), REGISTRY.get_difficulty("tutorial"),
                        REGISTRY.get_reward("copper"), REGISTRY.get_setting("forest"), 1)
    hard = gen._compose("board", REGISTRY.get_archetype("slay"), REGISTRY.get_difficulty("abyss"),
                        REGISTRY.get_reward("legendary"), REGISTRY.get_setting("forest"), 2)
    assert easy.objectives and easy.objectives[0].required_count >= 1, "Step 22 Failed: objective"
    assert hard.objectives[0].required_count > easy.objectives[0].required_count, "Step 22 Failed: scaling"
    # Step 23: 報酬合成
        ...
        for did in REGISTRY.all_difficulties():
            for rid in REGISTRY.all_rewards():
                for sid in REGISTRY.all_settings():
                    q = gen._compose("board", REGISTRY.get_archetype(aid),
                                     REGISTRY.get_difficulty(did), REGISTRY.get_reward(rid),
                                     REGISTRY.get_setting(sid), 1)

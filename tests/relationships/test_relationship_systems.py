...
from src.relationships import (
    ...
    DynamicRelationshipSystem, BranchingScenarioGenerator, FactionRelationshipSystem,
    RomanceMechanics, RomanceStage, MentorshipMechanics, BetrayalConflictSystem, BetrayalType, ConflictState,
    MemorySystem, MemoryType, MemoryImportance,
    PersonalitySystem, PersonalityTrait, CharacterArchetype,
    DialogueGenerationSystem, DialogueContext, RelationshipVisualizer, ComprehensiveRelationshipSaveSystem, create_engine
)
...
class TestRelationshipModels(unittest.TestCase):
    ...
    def test_relationship_edge(self):
        ...
        edge.add_modifier(__import__('src.relationships.models', fromlist=['RelationshipModifier']).RelationshipModifier(
            InteractionType.TALK, 10
        ))
...
class TestRelationshipGraph(unittest.TestCase):
    ...
    def test_multi_layer_graph(self):
        """多層グラフのテスト"""
        self.graph.add_edge(RelationshipEdge("alice", "bob", RelationshipType.FAVORABILITY, level=30))
    ...
    def test_graph_statistics(self):
        """グラフ統計のテスト"""
        self.graph.add_edge(RelationshipEdge("alice", "bob", RelationshipType.FAVORABILITY, level=30))
        self.graph.add_edge(RelationshipEdge("bob", "charlie", RelationshipType.FAVORABILITY, level=-40))
...
""")
    ...
    def test_skill_teaching(self):
        ...
        # レベルを上げる
        self.manager.modify_relationship("master", "apprentice", InteractionType.KNOWLEDGE_SHARE, 50)
...
""")
    ...
    def test_betrayal_commit(self):
        ...
        result = self.betrayal.commit_betrayal(
            "npc1", "npc2", BetrayalType.BACKSTAB,
...
""")
    ...
    def test_memory_creation(self):
        ...
        memory = self.memory.create_memory(
            "npc1", MemoryType.POSITIVE_EVENT, "Good event", 
            MemoryImportance.SIGNIFICANT, other_id="npc2"
        )
...
""")
        ...
        self.engine.memory.create_memory(
            "player", MemoryType.POSITIVE_EVENT, "Test memory",
            MemoryImportance.MAJOR, other_id="npc1"
        )
    ...
    def test_comprehensive_load(self):
        ...
        # 関係が復元されているかチェック
        level = new_engine.manager.get_relationship_level("player", "npc1", RelationshipType.FAVORABILITY)

...
class BattleScenario:
    ...
    def __init__(self, name: str, player_level: int, monster_type: str, trials: int = 100, expected_win_rate: float = 0.5):
        self.name = name
...
class BalanceSimulator:
    ...
    def simulate_battle(self, player_level: int = 1, monster_type: str = 'slime', trials: int = 100, expected_win_rate: Optional[float] = None) -> Dict[str, Any]:
        wins = 0
        turns_list = []
        total_dmg_dealt = []
        total_dmg_taken = []
        ...
        for _ in range(trials):
            # Create player
            player = Entity(
                x=0, y=0, name='Hero', is_player=True,
        ...
        result = {
            ...
            'is_balanced': (stalled_battles == 0 and one_shot_warnings < trials * 0.3 and not win_rate_alert)
        }
        return result

    def generate_html_report(self, report_data: Dict[str, Any], output_html: str = 'balance_report.html') -> str:
        ...
        for key, sc in report_data.get("scenarios", {}).items():
            ...
            html_lines.append(f"<tr><td>{key}</td><td>{sc.get('player_level')}</td><td>{sc.get('monster_type')}</td><td>{sc.get('win_rate')}</td><td>{sc.get('avg_turns')}</td><td class='{status_cls}'>{status_txt}</td></tr>")
    ...
    def run_full_validation(self, output_path: str = 'balance_report.json') -> Dict[str, Any]:
        ...
        for lvl, mtype, exp_rate in scenarios:
            ...
            res = self.simulate_battle(player_level=lvl, monster_type=mtype, trials=100, expected_win_rate=exp_rate)
            results[key] = res
        ...
        self.report = {
            'summary': 'PASS' if all_passed and checker_res["status"] == "PASS" else 'WARNINGS_DETECTED',
            'scenarios': results,

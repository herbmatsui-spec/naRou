...
def test_all_72_steps_storyteller_system():
    print(
        "=== ダンジョン・ワールド自動生成ストーリーテラー 全72ステップ 総合検証開始 ==="
    )
    ...
    assert scen_raw and "scenario_templates" in scen_raw, (
        "Step 1 Failed: scenario_templates missing"
    )
    ...
    with open("data/story_choices.yaml", encoding="utf-8") as f:
        cho_raw = yaml.safe_load(f)
    assert cho_raw and "choice_consequences" in cho_raw, (
        "Step 3 Failed: choice_consequences missing"
    )
    ...
    assert len(fss.get("immediate_effects", [])) >= 2, (
        "Step 4 Failed: immediate_effects missing"
    )
    print("[OK] Step 4 (農家の生存者救出結果 farm_survivor_saved)")

    # Step 5: data/world_state.yaml 基本構造
    with open("data/world_state.yaml", encoding="utf-8") as f:
        ws_raw = yaml.safe_load(f)
    assert ws_raw and "world_state_template" in ws_raw, (
        "Step 5 Failed: world_state_template missing"
    )
    ...
    assert "goblin_threat_level" in wst.get("persistent_variables", {}), (
        "Step 6 Failed: persistent_variables missing"
    )
    print("[OK] Step 6 (基本ワールド状態テンプレート)")

    # Step 7: data/dungeon_themes.yaml 基本構造
    with open("data/dungeon_themes.yaml", encoding="utf-8") as f:
        dt_raw = yaml.safe_load(f)
    assert dt_raw and "dungeon_themes" in dt_raw, (
        "Step 7 Failed: dungeon_themes missing"
    )
    ...
    with open("data/character_relations.yaml", encoding="utf-8") as f:
        cr_raw = yaml.safe_load(f)
    assert cr_raw and "relationship_templates" in cr_raw, (
        "Step 9 Failed: relationship_templates missing"
    )
    ...
    assert len(sv.get("interaction_effects", [])) >= 2, (
        "Step 10 Failed: interaction_effects missing"
    )
    print("[OK] Step 10 (助けた村人関係 saved_villager)")

    # Step 11: data/world_events.yaml 基本構造
    with open("data/world_events.yaml", encoding="utf-8") as f:
    ...
    with open("data/memory_fragments.yaml", encoding="utf-8") as f:
        mf_raw = yaml.safe_load(f)
    assert mf_raw and "memory_fragments" in mf_raw, (
        "Step 13 Failed: memory_fragments missing"
    )
    ...
    assert len(gcs.get("resolution_paths", [])) >= 2, (
        "Step 14 Failed: resolution_paths missing"
    )
    print("[OK] Step 14 (ゴブリン子どもの悲鳴フラグメント goblin_child_screams)")

    # Step 15: data/story_endings.yaml 基本構造
    with open("data/story_endings.yaml", encoding="utf-8") as f:
    ...
    assert "spared_cubs_resolved" in gpb.get("unlock_conditions", []), (
        "Step 16 Failed: unlock_conditions mismatch"
    )
    print("[OK] Step 16 (ゴブリンの和平使者エンディング goblin_peace_bringer)")

    # Step 17: data/story_ui.yaml 基本構造
    with open("data/story_ui.yaml", encoding="utf-8") as f:
    ...
    assert sn.get("display_priority") == "high", (
        "Step 18 Failed: display_priority mismatch"
    )
    ...
    assert "# TODO: Story/world state fields" in ent_code, (
        "Step 19 Failed: placeholder comment missing"
    )

    e = Entity()
    assert hasattr(e, "story_flags") and isinstance(e.story_flags, dict), (
        "Step 20 Failed"
    )
    assert hasattr(e, "story_variables") and isinstance(e.story_variables, dict), (
        "Step 21 Failed"
    )
    assert hasattr(e, "story_choices_made") and isinstance(
        e.story_choices_made, list
    ), "Step 22 Failed"
    assert hasattr(e, "world_state_version") and e.world_state_version == "1.0", (
        "Step 23 Failed"
    )
    assert hasattr(e, "player_legacy") and isinstance(e.player_legacy, dict), (
        "Step 24 Failed"
    )
    assert hasattr(e, "character_relationships") and isinstance(
        e.character_relationships, dict
    ), "Step 25 Failed"
    assert hasattr(e, "memory_fragments") and isinstance(e.memory_fragments, list), (
        "Step 26 Failed"
    )
    assert hasattr(e, "active_world_events") and isinstance(
        e.active_world_events, list
    ), "Step 27 Failed"
    assert hasattr(e, "completed_storylines") and isinstance(
        e.completed_storylines, list
    ), "Step 28 Failed"
    assert hasattr(e, "available_storylines") and isinstance(
        e.available_storylines, list
    ), "Step 29 Failed"
    assert hasattr(e, "story_notifications") and isinstance(
        e.story_notifications, list
    ), "Step 30 Failed"
    assert hasattr(e, "current_choice_prompt"), "Step 31 Failed"
    assert hasattr(e, "ending_progress") and isinstance(e.ending_progress, dict), (
        "Step 32 Failed"
    )
    print("[OK] Steps 19-32 (entity.py ストーリー・ワールド状態フィールド)")

    # Steps 33-40: storyteller_system.py
    from storyteller_system import REGISTRY as STORY_REG
    ...
    assert ok_act and e.story_flags.get("goblin_invasion_active"), (
        "Step 40 Failed: activate_scenario"
    )
    print("[OK] Steps 33-40 (storyteller_system.py Data/Registry/Manager)")

    # Steps 41-46: choice_system.py
    from choice_system import REGISTRY as CHO_REG
    from choice_system import ChoiceConsequenceData, ChoiceManager, ChoiceRegistry
    ...
    assert ok_cho and e.story_variables.get("rescued_villagers_count") == 5, (
        "Step 46 Failed: apply_consequence"
    )
    print("[OK] Steps 41-46 (choice_system.py Data/Registry/Manager)")

    # Steps 47-53: world_state_system.py
    from world_state_system import REGISTRY as WS_REG
    from world_state_system import (
        WorldStateManager,
        WorldStateRegistry,
        WorldStateTemplate,
    )
    ...
    assert wsmgr.get_variable(e, "goblin_threat_level") == 20, (
        "Step 52/53 Failed: get/set_variable"
    )
    ...
    assert sel_theme is not None and sel_theme.theme_id == "goblin_cave", (
        "Step 59 Failed: select_theme_by_story"
    )
    ...
    assert relmgr.get_relationship_level(e, "gwen") == 2, (
        "Step 65/66 Failed: get_relationship_level"
    )
    ...
    from world_event_system import WorldEventData, WorldEventManager, WorldEventRegistry
    ...
    assert ok_we and "blood_moon" in e.active_world_events, (
        "Step 71 Failed: trigger_event"
    )
    ...
    eng = Engine()
    assert hasattr(eng, "storyteller_manager"), (
        "Step 72 Failed: storyteller_manager on Engine"
    )
    assert hasattr(eng, "choice_manager"), "Step 72 Failed: choice_manager on Engine"
    assert hasattr(eng, "world_state_manager"), (
        "Step 72 Failed: world_state_manager on Engine"
    )
    assert hasattr(eng, "procedural_dungeon_generator"), (
        "Step 72 Failed: procedural_dungeon_generator on Engine"
    )
    assert hasattr(eng, "relationship_manager"), (
        "Step 72 Failed: relationship_manager on Engine"
    )
    assert hasattr(eng, "world_event_manager"), (
        "Step 72 Failed: world_event_manager on Engine"
    )
    ...
    assert loaded_eng.player.story_flags.get("test_flag") is True, (
        "Step 72 State persistence failed"
    )
    assert "blood_moon" in loaded_eng.player.active_world_events, (
        "Step 72 State persistence failed"
    )
    assert loaded_eng.player.character_relationships["gwen"]["trust"] == 50, (
        "Step 72 State persistence failed"
    )

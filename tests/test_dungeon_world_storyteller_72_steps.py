...
def test_all_72_steps_storyteller_system():
    ...
    assert (
        scen_raw and "scenario_templates" in scen_raw
    ...
    assert (
        cho_raw and "choice_consequences" in cho_raw
    ...
    assert (
        ws_raw and "world_state_template" in ws_raw
    ...
    assert "goblin_threat_level" in wst.get(
    ...
    assert (
        cr_raw and "relationship_templates" in cr_raw
    ...
    assert (
        len(sv.get("interaction_effects", [])) >= 2
    ...
    assert "spared_cubs_resolved" in gpb.get(
        "unlock_conditions", []
    ...
    assert (
        "# TODO: Story/world state fields" in ent_code
    ...
    assert ok_act and e.story_flags.get(
        "goblin_invasion_active"
    ...
    assert (
        ok_cho and e.story_variables.get("rescued_villagers_count") == 5
    ...
    from world_state_system import WorldStateManager, WorldStateRegistry, WorldStateTemplate
    ...
    assert (
        sel_theme is not None and sel_theme.theme_id == "goblin_cave"
    ...
    assert (
        relmgr.get_relationship_level(e, "gwen") == 2
    ...
    assert hasattr(
        eng, "procedural_dungeon_generator"
    ...
    assert (
        loaded_eng.player.story_flags.get("test_flag") is True
    ...
    assert (
        loaded_eng.player.character_relationships["gwen"]["trust"] == 50
    ), "Step 72 State persistence failed"

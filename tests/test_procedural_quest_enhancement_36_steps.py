...
def test_all_36_steps_procedural_quest_enhancement():
    ...
    assert "enemy" in dn and "item" in dn and "stage" in dn and "difficulty" in dn, "Step 5-8 Failed"
    ...
    from procedural_quest_generator import (
        REGISTRY, ProceduralQuestGenerator,
        ProceduralQuestManager, GeneratedQuest, QuestObjectiveSpec,
    )
    # Step 9-10: Registry 読み込み + get_display_name
    REGISTRY.load()
    assert REGISTRY.get_display_name("enemy", "magma_elemental") == "マグマエレメンタル", "Step 10 Failed"
    assert REGISTRY.get_display_name("enemy", "unknown_x") == "unknown_x", "Step 10 Failed: fallback"
    ...
    stage_names = set(REGISTRY.get_display_name("stage", s) for s in REGISTRY.all_settings().keys())
    ...
    # ---------------------------------------------------------
    # Step 13-14: データクラス拡張
    q0 = GeneratedQuest(quest_id="c1", title="t", archetype_id="slay", difficulty_id="tutorial",
                        reward_id="copper", setting_id="forest")
    q0.chain_id = "c1"; q0.parent_id = ""; q0.depth = 0
    ...
    # Step 21: 深度上限で打ち切り
    deep = GeneratedQuest(archetype_id="slay", difficulty_id="abyss", reward_id="legendary",
                          setting_id="abyss", depth=5)
    ...
    assert "c" in p2.procedural_quest.active_chains or any(
        k for k in p2.procedural_quest.active_chains), "Step 23 Failed: active_chains"
    print("[OK] Step 23 (active_chains 連鎖状態の記録)")

    # Step 24: 累積報酬（cascade_bonus 合算）
    obj_b = QuestObjectiveSpec(objective_id="o", description="x", target_type="kill",
                               target_id="goblin", required_count=1, cascade_bonus={"fame": 5})
    qb = GeneratedQuest(title="b", archetype_id="slay", difficulty_id="tutorial",
                        reward_id="copper", setting_id="forest",
                        objectives=[obj_b], reward={"gold": 10, "exp": 5, "items": [], "bonus": {"fame": 2}})
    p3 = Entity()
    ...
    q = gen.generate_board_quest(p, seed=11); q.depth = 0
    golds = []
    for _ in range(5):
        ...
        if f is None:
            break
        q = f
    assert all(golds[i + 1] > golds[i] for i in range(len(golds) - 1)), "Step 25 Failed: cascade growth"

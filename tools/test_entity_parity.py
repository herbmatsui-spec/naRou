...
from core.tile_atlas import TileAtlas, AUTOTILE_MAP
        ...
        for direction in range(td.directions):
            for state in td.states:
                for frame in range(td.frames):
                    uv_32 = atlas.get_uv(tile_id, direction=direction, state=state, frame=frame, scale="32")
                    
                    # Verify frame progression (horizontal)
                    if frame > 0:
                        prev_uv = atlas.get_uv(tile_id, direction=direction, state=state, frame=frame-1, scale="32")
                        if uv_32.x != prev_uv.x + td.frame_width:
                            print(f"  FAIL: Frame progression broken at dir={direction}, state={state}, frame={frame}")
                            all_passed = False
...
def test_entity_animstate():
    ...
    print(f"  After attack trigger: state={anim.state}, frame={anim.frame}, attack_timer={anim.attack_timer:.2f}")

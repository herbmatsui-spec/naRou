...
class PerformanceOptimizer:
    ...
    def run_stress_test(
        self, func: Callable, duration: int = 10, *args, **kwargs
    ) -> dict[str, Any]:
    ...
    def run_load_test(
        self, func: Callable, target_rps: float, duration: int = 10, *args, **kwargs
    ) -> dict[str, Any]:
    ...
    def run_sustainability_test(
        self, func: Callable, duration: int = 30, *args, **kwargs
    ) -> dict[str, Any]:
    ...
    def run_endurance_test(
        self, func: Callable, duration: int = 60, *args, **kwargs
    ) -> dict[str, Any]:
    ...
    def run_scalability_test(
        self, func: Callable, max_workers: int = 8, *args, **kwargs
    ) -> dict[str, Any]:
        ...
        for workers in [1, 2, 4, max_workers]:
            ...
            for _ in range(workers * 5):  # 5 tasks per worker
                t = threading.Thread(
                    target=lambda: (
                        latencies.append(worker_task()[0])
                        if worker_task()[1] is None
                        else (errors.__iadd__(1))
                    )
                )
                threads.append(t)
                t.start()
    ...
    def run_compatibility_test(self) -> dict[str, Any]:
        ...
        try:
            ...
            compatibility["tests"]["optimizer"] = (
                "PASS" if hasattr(result, "improvement") else "FAIL"
            )
        except Exception as e:

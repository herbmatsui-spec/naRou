...
def validate_directory(
    directory: str, config: dict, asset_type: str
    ...
    all_issues = []
    stats = {"total_files": 0, "valid_files": 0, "invalid_files": 0, "files": []}
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            file_info = {"path": file_path, "valid": is_valid, "issues": issues}
    ...
    for def_path in def_files:
        ...
        for name, tdef in defined.items():
            if name not in atlas_tiles:
                issues.append(
                    f"Tile '{name}' defined in {os.path.basename(def_path)} "
            ...
            if expected_frames != actual_frames:
                issues.append(
                    f"Tile '{name}' frame count mismatch: "

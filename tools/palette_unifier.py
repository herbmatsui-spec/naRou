...
class PaletteUnifier:
    ...
    def load_master_palette(self) -> list[tuple[int, int, int]]:
        ...
        try:
            with open(self.master_palette_path) as f:
                data = json.load(f)
            ...
            logger.warning(
                f"Could not extract palette from {self.master_palette_path}, using default"
            )
            return [
                (0, 0, 0),
                (255, 255, 255),
    ...
    def unify_image_palette(
        self, image_path: Path, output_path: Path | None = None, max_colors: int = 16
    ) -> bool:
        """

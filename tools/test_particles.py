...
def test_particle_update():
    ...
    moved = sum(1 for i, (ix, iy) in enumerate(initial_positions)
...
def test_particle_drawing():
    ...
    non_zero_fg = sum(1 for y in range(24) for x in range(40)
                      if fg[x, y][0] > 0 or fg[x, y][1] > 0 or fg[x, y][2] > 0)
    non_zero_ch = sum(1 for y in range(24) for x in range(40)
                      if ch[x, y] != 0)

...
class PerformanceValidator:
    ...
    def run_comparison_test(
        self, func_a: Callable, func_b: Callable, test_name: str, *args, **kwargs
    ) -> dict[str, Any]:
        """比較テストを実行"""
        # Func Aのテスト
        ...
        time_improvement = (
            (
                (result_a["execution_time"] - result_b["execution_time"])
                / result_a["execution_time"]
                * 100
            )
            if result_a["execution_time"] > 0
            else 0
        )
...
<body>
    ...
    def generate_excel_report(self) -> str:
        ...
        for validation in self.validation_history:
            ...
            lines.append(
                f"{validation['timestamp']},{validation['function_name']},{validation['success']},{validation['execution_time']:.4f},{validation['cpu_usage']:.1f},{mem_mb:.2f}"
            )

...
def generate_normal_atlas():
    with open("demos/assets/font_atlas.json", "r", encoding="utf-8") as f:
        meta = json.load(f)
        ...
        if char == "#":
            ...
            for py in range(h):
                for px in range(w):
                    ...
                    # Bevel borders
                    if px < 3: nx += (3 - px) * 0.25
                    elif px >= w - 3: nx -= (px - (w - 3) + 1) * 0.25
                    if py < 3: ny += (3 - py) * 0.25
                    elif py >= h - 3: ny -= (py - (h - 3) + 1) * 0.25

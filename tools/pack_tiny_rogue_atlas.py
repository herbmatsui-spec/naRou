...
def pack_atlas(tiles: List[Tuple[int, str, str, str]]) -> Tuple[Image.Image, Dict[str, Dict[str, Any]]]:
    ...
    while i < len(tiles):
        ...
        if category in DIRECTIONAL_CATEGORIES and i + 3 < len(tiles):
            ...
            if all(t[3] == category for t in group) and all(t[0] == group[0][0] + j for j, t in enumerate(group)):
                ...
                print(f"Directional group: category={category}, base_id={base_id}, tiles={[t[2] for t in group]}")

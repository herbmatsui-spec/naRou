...
def process_fonts(config: Dict) -> bool:
    ...
    if os.path.exists(font_dir):
        ...
        for font_file in font_files:
            ...
            cmd = [
                sys.executable, 'tools/generate_font_atlas.py',
                '--font', font_path,
                '--output', os.path.join(config['directories']['output'], 'fonts'),
                '--size', str(config['font']['default_size'])
            ]
...
def process_sounds(config: Dict) -> bool:
    ...
    if os.path.exists(sound_dir):
        # Create sound index
        cmd = [
            sys.executable, 'tools/convert_sounds.py',
            'index', sound_dir,
            '--output', os.path.join(config['directories']['output'], 'sounds', 'sound_index.json')
        ]
...
def process_models(config: Dict) -> bool:
    ...
    if os.path.exists(model_dir):
        ...
        cmd = [
            sys.executable, 'tools/optimize_models.py',
            'stats', model_dir,
            '--output', os.path.join(config['directories']['output'], 'models', 'model_stats.json')
        ]
...
def package_assets(config: Dict) -> bool:
    ...
    for asset_type in ['tilesets', 'fonts', 'sounds', 'models']:
        ...
        if os.path.exists(asset_dir):
            count = len([f for f in os.listdir(asset_dir) 
...
def main():
    ...
    parser.add_argument('--steps', nargs='+', 
                       choices=['validate_source', 'process_tilesets', 'process_fonts', 
                               'process_sounds', 'process_models', 'optimize_assets', 
                               'package_assets', 'validate_output', 'all'],
                       default=['all'], help='Steps to run')
    ...
    if args.dry_run:
        ...
        steps = args.steps if args.steps != ['all'] else [
            'validate_source', 'process_tilesets', 'process_fonts', 
            'process_sounds', 'process_models', 'optimize_assets', 
            'package_assets', 'validate_output'
        ]
        for step in steps:
            print(f"  - {step}")
        return

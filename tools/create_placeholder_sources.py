...
def create_placeholder_image(color, size=(16, 16), filename=None):
    ...
    if size[0] >= 8 and size[1] >= 8:
        ...
        inner_color = tuple(min(255, c + 30) for c in color[:3]) + (color[3] if len(color) == 4 else 255,)
        draw.rectangle([inner_pos, (inner_pos[0] + inner_size[0], inner_pos[1] + inner_size[1])], fill=inner_color)
...
def main():
    ...
    terrain_names = ['floor_dungeon', 'wall_dungeon', 'water', 'stairs_down', 'stairs_up', 'trap']

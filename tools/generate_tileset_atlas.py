...
def _wall(size: int, rng: random.Random) -> Image.Image:
    ...
    for row, y in enumerate(range(0, size, brick_h)):
        ...
        for x in range(-offset, size, brick_h * 2):
            d.rectangle(
                [x + 1, y + 1, x + brick_h * 2 - 1, y + brick_h - 1],
                fill=_lighten(base, 12),
                outline=_darken(base, 20),
            )
    return img
...
def _item_gold(size: int, rng: random.Random, frame: int) -> Image.Image:
    ...
    r = size // 4
    d.ellipse(
        [c - r, c - r - bob, c + r, c + r - bob],
        fill=(240, 190, 40),
        outline=(120, 90, 10),
        width=max(1, size // 24),
    )
    d.ellipse(
        [c - r // 2, c - r // 2 - bob, c + r // 2, c + r // 2 - bob],
        fill=(255, 235, 120),
    )
    return img
...
def _item_potion(size: int, rng: random.Random) -> Image.Image:
    ...
    d.ellipse(
        [c - size // 8, size // 2, c + size // 8, size // 2 + size // 8],
        fill=(160, 240, 190),
    )
    return img
...
def _item_weapon(size: int, rng: random.Random) -> Image.Image:
    ...
    c = size // 2
    d.line(
        [(c, size // 5), (c, size - size // 4)],
        fill=(200, 200, 210),
        width=max(1, size // 12),
    )
    d.line(
        [(c - size // 4, size - size // 4), (c + size // 4, size - size // 4)],
        fill=(150, 90, 40),
        width=max(1, size // 10),
    )
    d.polygon(
        [(c, size // 6), (c - size // 16, size // 4), (c + size // 16, size // 4)],
        fill=(230, 230, 240),
    )
    return img
...
def _entity(
    ...
    if kind == "goblin":
        d.polygon(
            [
                (cx - hr, head_y - hr - bob),
                (cx - hr - hr // 2, head_y - hr - bob - hr),
                (cx - hr // 2, head_y - hr - bob),
            ],
            fill=(60, 60, 60),
        )
        d.polygon(
...
def _torch(size: int, rng: random.Random, frame: int) -> Image.Image:
    ...
    cx = size // 2
    d.rectangle(
        [cx - max(1, size // 16), size // 2, cx + max(1, size // 16), size - size // 8],
        fill=(120, 80, 40),
    )
...
def _make_tile(
    name: str, tile_def: dict, size: int, frame: int, direction: int, rng: random.Random
) -> Image.Image:
    upper = name.upper()
...
def generate_tileset(
    def_path: str, output_dir: str, tile_size: int = 32, source_dir: str | None = None
) -> dict:
    ...
    print(
        f"Generated {png_path} ({atlas_w}x{atlas_h}) and {json_path} "
...
def generate_autotile_set(
    def_path: str, output_dir: str, tile_size: int = 32, count: int = 4
) -> dict:
...
def generate_scaled_atlases(
    def_path: str, output_dir: str, sizes: list[int], base_size: int = 32
) -> list[str]:
    ...
    for s in sizes:
        ...
        factor = s / base_size
        scaled = base.resize(
            (max(1, int(base_w * factor)), max(1, int(base_h * factor))), Image.NEAREST
        )
...
def compress_png(png_path: str, out_path: str | None = None) -> int:
...
def document_tileset(
    def_path: str, output_dir: str, tile_size: int = 32, output_path: str | None = None
) -> str:

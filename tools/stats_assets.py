            ...
            if file.endswith(".json") and not file.endswith("_manifest.json"):
                ...
                if os.path.exists(png_path):
                    try:
                        ...
                        tile_size = metadata.get("tile_size", 0)
                        stats["tile_size_distribution"][str(tile_size)] = (
                            stats["tile_size_distribution"].get(str(tile_size), 0) + 1
                        )
                        ...
                        stats["size_distribution"][size_cat] = (
                            stats["size_distribution"].get(size_cat, 0) + 1
                        )
            ...
            if file.endswith(".json") and not file.endswith("_manifest.json"):
                ...
                if os.path.exists(png_path):
                    try:
                        ...
                        font_size = metadata.get("font_size", 0)
                        stats["font_size_distribution"][str(font_size)] = (
                            stats["font_size_distribution"].get(str(font_size), 0) + 1
                        )
        ...
        for file in files:
            if any(file.lower().endswith(ext) for ext in extensions):
                ...
                try:
                    ...
                    stats["duration_distribution"][dur_cat] = (
                        stats["duration_distribution"].get(dur_cat, 0) + 1
                    )
        ...
        for file in files:
            if any(file.lower().endswith(ext) for ext in extensions):
                ...
                try:
                    ...
                    stats["vertex_distribution"][vert_cat] = (
                        stats["vertex_distribution"].get(vert_cat, 0) + 1
                    )
                    ...
                    stats["complexity_distribution"][complex_cat] = (
                        stats["complexity_distribution"].get(complex_cat, 0) + 1
                    )
...
def print_stats_summary(stats: dict):
    ...
    print(f"\n{'=' * 70}")
    print("ASSET COLLECTION STATISTICS REPORT")
    print(f"{'=' * 70}")
    ...
    for coll_name, coll_stats in stats.get("collections", {}).items():
        ...
        if coll_name == "tilesets":
            ...
            if coll_stats.get("largest_tileset"):
                ...
                print(
                    f"    Largest: {lt['name']} ({lt['size_bytes'] / (1024 * 1024):.2f} MB, {lt['tile_count']} tiles)"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        default=["all"],
        help="Asset collections to analyze",
    )
    parser.add_argument(
        "--summary-only",
        action="store_true",
    ...
    if args.collections != ["all"]:
        filtered_collections = {
            k: v for k, v in stats["collections"].items() if k in collections_to_analyze
        }

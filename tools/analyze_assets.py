...
def analyze_tileset_quality(tileset_dir: str, config: dict) -> dict:
    ...
    if analysis["optimization_opportunities"]:
        analysis["recommendations"].append(
            "Consider regenerating atlases with better packing efficiency"
        )
...
def analyze_font_quality(font_dir: str, config: dict) -> dict:
    ...
    if analysis["optimization_opportunities"]:
        analysis["recommendations"].append(
            "Consider regenerating font atlases with better character packing"
        )
...
def analyze_sound_quality(sound_dir: str, config: dict) -> dict:
    ...
    if analysis["optimization_opportunities"]:
        analysis["recommendations"].append(
            "Consider converting uncompressed audio to compressed formats (OGG/MP3)"
        )
...
def analyze_model_quality(model_dir: str, config: dict) -> dict:
                            ...
                            analysis["optimization_opportunities"].append(
                                {
                                    ...
                                    "description": "Large OBJ file; consider converting to GLTF with compression",
                                }
                            )
    ...
    if analysis["optimization_opportunities"]:
        analysis["recommendations"].append(
            "Consider optimizing high-polygon models for real-time performance"
        )

    if any(issue["type"] == "large_file_size" for issue in analysis["quality_issues"]):
        analysis["recommendations"].append(
            "Apply mesh compression or consider LOD models for large assets"
        )
...
def print_analysis_summary(report: dict):
    ...
    print(f"\n{'=' * 70}")
    print("ASSET QUALITY ANALYSIS REPORT")
    print(f"{'=' * 70}")
    ...
    for analysis_name, analysis in report.get("analyses", {}).items():
        ...
        print(
            f"    Optimization Opportunities: {len(analysis.get('optimization_opportunities', []))}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze assets for quality and optimization opportunities"
    )
    parser.add_argument(
        "--config",

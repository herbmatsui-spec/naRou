...
def restore_from_backup(backup_source: str, restore_target: str, config: Dict) -> Dict:
    ...
    try:
        ...
        print(f"Restore completed: {stats['files_restored']} files, {stats['total_size_bytes']} bytes")
...
def restore_specific_files(backup_source: str, restore_target: str, 
                          file_list: List[str], config: Dict) -> Dict:
    ...
    try:
        ...
        print(f"Selective restore completed: {stats['files_restored']} files restored, {stats['files_not_found']} not found")
        ...
        for item in os.listdir(backup_dir):
            ...
            if os.path.isdir(item_path):
                ...
                if backup_info['timestamp'] is None:
                    ...
                    backup_info['datetime'] = time.strftime('%Y-%m-%d %H:%M:%S', 
...
def main():
    ...
    parser.add_argument('--restore-target', required=True,
                       help='Target directory to restore assets to')
    parser.add_argument('--list-backups', action='store_true',
                       help='List available backups and exit')
    parser.add_argument('--files', nargs='+',
                       help='Specific files to restore (relative to backup root)')
    ...
    if args.list_backups:
        ...
        if os.path.isdir(args.backup_source):
            ...
            if not os.path.exists(backup_dir):
                backup_dir = os.path.join(config['directories'].get('output', 'assets/build'), 
                                        'backups', args.backup_source)
    ...
    if result:
        print(f"\n{'='*60}")
        print(f"RESTORE OPERATION RESULTS")
        print(f"{'='*60}")

...
def document_sounds(sound_files: list[str], output_path: str) -> str:
    ...
    for sf in sound_files:
        ...
        lines.append(
            f"| {os.path.basename(sf)} | {meta['file_size']} | "
...
def export_sounds(sound_files: list[str], export_dir: str, target_format: str = "ogg") -> dict:
    ...
    for f in sound_files:
        out = os.path.join(
            export_dir, os.path.splitext(os.path.basename(f))[0] + "." + target_format
        )
...
def main():
    ...
    convert_parser.add_argument(
        "--format", default="ogg", choices=["ogg", "mp3", "wav"], help="Target format"
    )
    ...
    index_parser.add_argument(
        "--output", default="assets/sounds/sound_index.json", help="Output index file"
    )

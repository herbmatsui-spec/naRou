...
class AssetQualityFilter:
    ...
    def validate_asset(self, image_path: Path, 
                      expected_size: Optional[Tuple[int, int]] = None,
                      check_transparency: bool = False,
                      min_transparent_ratio: float = 0.0,
                      max_colors: Optional[int] = None,
                      check_seamless: bool = False) -> Tuple[bool, List[str]]:
        """
        ...
        if check_transparency:
            if not self.check_transparency(image_path, min_transparent_ratio):
                issues.append(f"Insufficient transparency: minimum {min_transparent_ratio*100}% required")
...
def main():
    ...
    parser.add_argument("input_path", type=str, help="Input file or directory path")
    parser.add_argument("--output", type=str, help="Output directory for filtered assets (for filter action)")

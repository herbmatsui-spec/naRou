...
def optimize_model(
    ...
    try:
        ...
        print(f"Optimized {input_path} -> {output_path} (format: {target_format}, level: {optimization_level})")
        return True
    except Exception as e:
...
def main():
    ...
    scan_parser.add_argument('--extensions', nargs='+', 
                           default=['.obj', '.fbx', '.gltf', '.glb', '.dae', '.3ds', '.blend', '.ply', '.stl'],
    ...
    opt_parser.add_argument('--format', default='gltf', choices=['gltf', 'glb', 'obj', 'fbx'],
                          help='Target format')
    opt_parser.add_argument('--level', choices=['low', 'medium', 'high'], default='medium',
    ...
    compress_parser.add_argument('--method', choices=['draco', 'gltf', 'meshopt'], default='gltf',
    ...
    stats_parser.add_argument('--output', default='assets/models/model_stats.json',

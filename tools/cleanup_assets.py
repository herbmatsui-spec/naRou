...
def cleanup_directory(directory: str, max_age_hours: float = 24, 
                     patterns: List[str] = None, dry_run: bool = False) -> Dict:
                    ...
                    if not os.path.exists(os.path.realpath(file_path)):
                        ...
                        if dry_run:
                            ...
                            try:
                                ...
                                stats['errors'].append(f"Error removing broken symlink {file_path}: {e}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument('--target', choices=['temp', 'cache', 'builds', 'logs', 'all', 'symlinks', 'empty-dirs'],
                       default='all', help='What to clean up')
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"CLEANUP OPERATION SUMMARY")
    print(f"{'='*60}")

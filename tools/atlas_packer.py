...
def load_manifest(manifest_path: Path) -> List[Tuple[int, str, str, str]]:
    ...
    with open(manifest_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            rows.append((int(row["index"]), row["filename"], row["suggested_id"], row["category"]))
    return rows
...
def scan_directory(tiles_dir: Path, pattern: str = "*.png") -> List[Tuple[int, str, str, str]]:
    ...
    for i, f in enumerate(files):
        ...
        category = "custom"
        if suggested_id.startswith("FLOOR"): category = "floor"
        elif suggested_id.startswith("WALL"): category = "wall"
        elif suggested_id.startswith("ITEM"): category = "item"
        elif suggested_id.startswith("MONSTER"): category = "monster"
        elif suggested_id.startswith("EFFECT"): category = "effect"
        elif suggested_id.startswith("UI"): category = "ui"
        elif suggested_id.startswith("DECOR"): category = "decoration"
...
def pack_atlas(
    ...
    while i < len(tiles):
        ...
        if category in directional_categories and i + 3 < len(tiles):
            group = tiles[i:i+4]
            if all(t[3] == category for t in group) and all(t[0] == group[0][0] + j for j, t in enumerate(group)):
                ...
                if y + stack_height + padding > max_atlas_size:
                    raise RuntimeError(f"Atlas too small! Need larger than {max_atlas_size}x{max_atlas_size}")
...
def generate_tileset_def(
    ...
    for tile_id, m in meta.items():
        ...
        if cat in ("floor", "wall"):
            ...
            defn.update({"variants": 1, "animated": m.get("animated", False), "frames": m.get("frames", 1), "fps": m.get("fps", 1), "directions": 1, "states": ["idle"]})
        elif cat == "item":
            defn.update({"variants": 1, "animated": False, "directions": 1, "states": ["idle"], "anchor_x": 0.5, "anchor_y": 0.5})
        elif cat in ("monster", "monster_variant", "player_npc"):
            defn.update({"variants": 1, "animated": True, "frames": m.get("frames", 4), "fps": m.get("fps", 8), "directions": m.get("directions", 4), "states": ["idle", "walk", "attack"], "anchor_x": 0.5, "anchor_y": 1.0})
        elif cat == "effect":
            defn.update({"variants": 1, "animated": True, "frames": m.get("frames", 3), "fps": m.get("fps", 12), "directions": 1, "states": ["cast"], "anchor_x": 0.5, "anchor_y": 0.5})
        elif cat == "ui":
            defn.update({"variants": 1, "animated": False, "directions": 1, "states": ["idle"], "anchor_x": 0.5, "anchor_y": 0.5})
        else:
...
def main():
    ...
    parser.add_argument("output_prefix", type=Path, help="Output prefix (e.g., assets/tiles/tileset_mypack_16x16)")
    parser.add_argument("--manifest", type=Path, help="CSV manifest file (index,filename,suggested_id,category)")
    parser.add_argument("--tile-size", type=int, default=16, help="Tile size in pixels (default: 16)")
    parser.add_argument("--max-size", type=int, default=2048, help="Maximum atlas dimension (default: 2048)")
    parser.add_argument("--padding", type=int, default=1, help="Padding between tiles (default: 1)")
    parser.add_argument("--scale-name", type=str, help="Scale name for tileset_def.json (default: derived from tile-size)")
    parser.add_argument("--directional-cats", nargs="+", default=["monster", "monster_variant", "player_npc"],

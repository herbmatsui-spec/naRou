...
class AnimationInterpolator:
    ...
    def interpolate_frames(self, frames: List[Path], 
        ...
        if target_frame_count < len(frames):
            logger.warning(f"Target frame count ({target_frame_count}) is less than keyframe count ({len(frames)}). Returning keyframes.")
            return frames
        ...
        logger.info(f"{intervals} intervals, {frames_per_interval} frames per interval, {remainder} remainder")
            ...
            if i < len(frames) - 1:
                ...
                if frames_in_this_interval > 0:
                    ...
                    interpolated = self._interpolate_between_frames(
                        frames[i], frames[i+1], frames_in_this_interval + 1  # +1 to include the end frame
                    )
    ...
    def _interpolate_between_frames(self, start_frame: Path, end_frame: Path, 
                                   steps: int) -> List[Path]:
        """
        ...
        try:
            with Image.open(start_frame) as img1, Image.open(end_frame) as img2:
                ...
                if img1.size != img2.size:
                    logger.warning(f"Frame sizes differ: {img1.size} vs {img2.size}. Resizing to match.")
...
def main():
    ...
    parser.add_argument('input_frames', nargs='+', type=str,
                       help='Input frame files (in order)')
    ...
    if args.loop and len(frame_paths) >= 2:
        ...
        result_frames = interpolator.interpolate_frames(extended_frames, args.target_frames + 1, output_dir)
    ...
    if result_frames:
        logger.info(f"Successfully interpolated {len(frame_paths)} keyframes to {len(result_frames)} frames")
        return 0
    else:

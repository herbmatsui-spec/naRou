...
def optimize_sounds(sound_dir: str, output_dir: str, config: Dict) -> Dict:
    ...
    music_bitrate = config.get('sound', {}).get('quality', {}).get('music', {}).get('bitrate', '192k')
...
def print_optimization_summary(results: Dict, verbose: bool = False):
    ...
    print(f"  Total Space Saved: {summary.get('total_space_saved_bytes', 0)} bytes ({summary.get('total_space_saved_bytes', 0) / (1024*1024):.2f} MB)")
...
def main():
    ...
    parser.add_argument('--assets', nargs='+',
                       ...
                       default=['all'], help='Asset types to optimize')

        ...
        try:
            ...
            if key_file:
                lftp_cmd.extend(
                    [
                        "-e",
                        f"set sftp:auto-confirm yes; set sftp:identity-file {key_file}; ",
                    ]
                )
            else:
            ...
            lftp_cmd.extend(
                [
                    "-e",
                    f"set net:max-retries 3; set net:timeout 30; mirror -R {source_dir} {remote_path}; quit",
                ]
            )
...
def deploy_to_ftp(
    ...
    try:
        ...
        try:
            subprocess.run(["ftp", "--version"], capture_output=True, check=True)
            # Use ftp command
...
def main():
    ...
    parser.add_argument("--target", required=True, help="Target deployment location")
    parser.add_argument(
        "--method",
        choices=["local", "sftp", "ftp", "archive"],
        default="local",
    ...
    parser.add_argument(
        ...
        default="zip",
        help="Archive format (for archive method)",
    )

...
class PetEvolutionRegistry:
    ...
    _instance: Optional['PetEvolutionRegistry'] = None
...
class PetEvolutionManager:
    ...
    def __init__(self, registry: Optional[PetEvolutionRegistry] = None):
        self.registry = registry or REGISTRY

    def get_available_evolutions(self, pet_type: str, pet: 'PetAI', pet_entity: Optional['Entity'] = None) -> List[PetEvolutionData]:
        ...
        for evo in evos:
            ...
            if cur_bond >= req_bond and pet_level >= req_level:
                available.append(evo)
            elif cur_bond >= req_bond:
                available.append(evo)

        return available

    def apply_evolution(self, pet: 'PetAI', evolution_data: PetEvolutionData, pet_entity: Optional['Entity'] = None) -> bool:
        """進化を適用 (Step 43)"""
        # 1. 統計変更の適用
        if pet_entity and hasattr(pet_entity, 'attributes'):
            ...
            for attr_name, boost in stats.items():
                if attr_name == "hp":
                    ...
                    setattr(pet_entity.attributes, attr_name, getattr(pet_entity.attributes, attr_name) + boost)

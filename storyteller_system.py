...
class StorytellerRegistry:
    ...
    _instance: Optional[StorytellerRegistry] = None

    def __new__(cls) -> StorytellerRegistry:
    ...
    def load(self, file_path: str = "data/procedural_scenarios.yaml") -> None:
        ...
        if not os.path.exists(file_path):
            self._scenarios["goblin_invasion"] = StoryScenarioData(
                id="goblin_invasion", name="ゴブリンの侵略", description="ゴブリン部隊の襲撃"
            )
            return
        ...
        for sid, sdata in s_dict.items():
            ...
            for ch in sdata.get("chapters", []):
                choices = [StoryChoiceData(c.get("id", ""), c.get("description", ""), c.get("consequence", "")) for c in ch.get("choices", [])]
                chapters.append(StoryChapterData(
                    id=ch.get("id", ""),
                    name=ch.get("name", ""),
    ...
    def get(self, scenario_id: str) -> Optional[StoryScenarioData]:
...
class StorytellerManager:
    ...
    def __init__(self, registry: Optional[StorytellerRegistry] = None):
        self.registry = registry or REGISTRY

    def check_scenario_triggers(self, player: "Entity", engine: Optional[Any] = None) -> List[StoryScenarioData]:
    ...
    def activate_scenario(self, player: "Entity", scenario_id: str, engine: Optional[Any] = None) -> bool:
        """シナリオを開始・進行 (Step 40)"""
        ...
        if engine:
            ...
            engine.log(f"📖【物語の幕開け】シナリオ「{scen.name}」が開始された！", (255, 215, 0))

        return True

    def process_choice(self, player: "Entity", choice_id: str, engine: Optional[Any] = None) -> bool:
        """選択肢の決定を処理 (Step 40)"""
        if choice_id not in player.story_choices_made:
            player.story_choices_made.append(choice_id)

        from choice_system import REGISTRY as CHOICE_REG, ChoiceManager

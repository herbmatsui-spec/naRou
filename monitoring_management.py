...
class MonitoringManager:
    ...
    def save_config(self):
        ...
        print(f"Saved monitoring config")
    ...
    def run_check(self, name):
        ...
        try:
            if check_type == "http":
                ...
                import subprocess
                result = subprocess.run(target, shell=True, capture_output=True, timeout=30)
                success = result.returncode == 0
                return success, result.stdout.decode() if success else result.stderr.decode()
    ...
    def trigger_alert(self, alert_name, alert, result):
        ...
        if action == "webhook":
            import requests
            try:
                requests.post(params.get("url"), json={
                    "alert": alert_name,
            ...
            import subprocess
            try:
                subprocess.run(params.get("cmd", ""), shell=True)
            except Exception as e:
            ...
            while self.running:
                ...
                with open(log_file, "a") as f:
                    for name, result in results.items():
                        f.write(f"{result['timestamp']} - {name}: {'OK' if result['success'] else 'FAIL'} - {result['message']}\n")
...
def main():
    ...
    parser.add_argument("--add-check", nargs=4, metavar=("NAME", "TYPE", "TARGET", "INTERVAL"), help="Add check")
    parser.add_argument("--remove-check", help="Remove check")
    parser.add_argument("--add-alert", nargs=3, metavar=("NAME", "CONDITION", "ACTION"), help="Add alert")
    ...
    if args.add_check:
        mgr.add_check(args.add_check[0], args.add_check[1], args.add_check[2], int(args.add_check[3]))

...
class GlobalRuleEngine:
    ...
    def __init__(
        ...
        self.devour_success_rate_override: Optional[float] = None
    ...
    def override_rule(
        ...
        player: Optional[CharacterState] = None,
        ...
        if cost_type == "MAX_HP":
            ...
            if player.max_hp <= base_hp_cost:
                self.override_count -= 1
                return False, f"生命力不足！ 改変の代償（最大HP -{base_hp_cost}）に耐えられません（現在最大HP: {player.max_hp}）。", 0
            ...
            return True, f"[SYSTEM ROOT] 世界法則 '{rule_name}' を '{value}' に改変！（代償：最大HP -{base_hp_cost} ➔ 残最大HP: {player.max_hp}）", 0

        elif cost_type == "KARMA":
            if available_karma < base_karma_cost:
                self.override_count -= 1
                return False, f"カルマ/資金不足！ 改変には {base_karma_cost} アルド/カルマ が必要です。（所持: {available_karma}）", 0
            ...
            return True, f"[SYSTEM ROOT] 世界法則 '{rule_name}' を '{value}' に改変！（消費カルマ: {base_karma_cost}）", base_karma_cost
...
class SkillEaterQuestSystem:
    ...
    def _init_quests(self):
        ...
        self.quests["q1_1"] = QuestDefinition("q1_1", "最底辺の生存戦略", 1, "ハンターから《初級剣術》を喰らう")
        self.quests["q2_8"] = QuestDefinition("q2_8", "敵対的買収（テイクオーバー）", 2, "ドン・ミダスを打倒し《黄金錬成》を喰らう")
        self.quests["q3_5"] = QuestDefinition("q3_5", "バベルの金庫室", 3, "世界銀行頭取を打倒し《マスタースキル》を喰らう")
    ...
    def check_boss_meta_counter(self, player: CharacterState, boss_id: str) -> Tuple[bool, str]:
        if boss_id == "midas_ceo":
            if player.has_skill("rar_gold_body") or player.has_skill("rar_infrared_vision"):
                ...
                return True, "【メタ攻略成功：スキル共鳴】合成スキルの周波数が《黄金錬成》の即死術式を完全中和しました！"
            ...
            if player.defense >= 80:
                ...
                return True, "【メタ攻略成功：圧倒的肉体】鍛え抜かれた防御力が《黄金錬成》の金化光線を力技で弾き返しました！"
            ...
            for s_id in player.get_skill_ids():
                ...
                if s_def and s_def.tier == SkillTier.COMMON:
                    ...
                    return True, f"【メタ攻略成功：身代わり破壊】《{s_def.name}》が身代わりとなって粉砕され、即死を回避しました！"
...
class ReincarnationMetaState:
    ...
    dominant_element_last_life: Optional[str] = None
...
class SkillEaterReincarnationSystem:
    ...
    def record_devoured_element(self, element: Optional[str]):
        if element:
    ...
    def apply_world_modifier(self) -> str:
        ...
        for s_def in self.registry.get_all_skills():
            ...
            elif dominant == "Fire" and "Water" in s_def.tags:
                s_def.market_value = int(s_def.market_value * 2.0)
            elif dominant == "Water" and "Fire" in s_def.tags:
                s_def.market_value = int(s_def.market_value * 2.0)
    ...
    def process_reincarnation(
        ...
        selected_carryover_skill_ids: List[str]
        ...
        if self.meta_state.devoured_element_counts:
            dominant = max(self.meta_state.devoured_element_counts, key=self.meta_state.devoured_element_counts.get)
        ...
        if self.meta_state.loop_count >= 2 and "first_eater_vault" not in self.meta_state.unlocked_secrets:

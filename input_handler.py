...
class InputHandler:
    ...
    def register_default_actions(cls) -> None:
        ...
        reg.register(
            "play",
            KeyBinding(KS.S, SkillTreeAction(), SHIFT, description="スキルツリー"),
        )
        reg.register(
            "play",
        ...
        reg.register(
            "play",
            KeyBinding(KS.PERIOD, WaitAction(), description="待機 (1ターン経過)"),
        )
        reg.register(
            "play",
            KeyBinding(
    ...
    def _handle_keydown_legacy(cls, event: tcod.event.KeyDown, engine: Engine) -> None:
        ...
        if engine.game_state == "skill_tree":
            if event.sym in (tcod.event.KeySym.S, tcod.event.KeySym.ESCAPE):
                ...
                if idx < len(avail):
                    target = avail[idx]
                    ok = engine.skill_tree_manager.learn_skill(
            ...
            if event.sym in (tcod.event.KeySym.G, tcod.event.KeySym.ESCAPE):
                ...
                if idx < len(g_keys):
                    ...
                    if ok:
                        ...
                        engine.log(
                            f"★【{g_info.name if g_info else target_gid}】に加入した！",
                            (100, 255, 200),
                        )
                        SoundManager.play_se("level_up")
                    else:
                        engine.log(
                            "ギルドに加入できませんでした（既に所属済みなど）。",
                            (255, 100, 100),
                        )
            return
        ...
        if engine.game_state == "context":
            ...
            ) or (tcod.event.KeySym.N1 <= event.sym <= tcod.event.KeySym.N9):
        ...
        if event.sym in (tcod.event.KeySym.UP, tcod.event.KeySym.K):
            ...
            dx = -1
        elif event.sym in (tcod.event.KeySym.RIGHT,):
            dx = 1

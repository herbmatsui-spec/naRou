    ...
    def initialize(cls) -> None:
        ...
        try:
            ...
            for i in range(4):
                ...
                tilemap.blit(layer_img, 0, 0, 0, i * band_height, tw, band_height)
...
class MapRenderer:
    ...
    _atlas_16: tcod.image.Image | None = None
    _atlas_32: tcod.image.Image | None = None
    ...
    def render(
        ...
        atlas_16: tcod.image.Image | None = None
        atlas_32: tcod.image.Image | None = None
        if use_pixel_art:
            cls._load_atlases()
                    ...
                    if context.game_map.visible[map_x][map_y]:
                        if (
                            ...
                            if context.game_map.tile_animations.get((map_x, map_y)):
                                # Animated tile
                                ux, uy, uw, uh = TILE_REGISTRY.get_animation_frame(
                                    tile_id, frame, scale
                                )
                            else:
                                # Static tile
                            ...
                            atlas_source.blit(
                                console,
                                vx * tile_size,
                                vy * tile_size,  # destination x, y
                                ux,
                                uy,
                                uw,
                                uh,  # source x, y, width, height
                            )
                            ...
                            if (map_x, map_y) == context.altar_pos:
                                lit_col, _ = DynamicLighting.calculate_tile_lighting(
                                    map_x, map_y, COLOR_ALTAR, light_sources
                                )
                                ...
                                lit_col, _ = DynamicLighting.calculate_tile_lighting(
                                    map_x, map_y, base_col, light_sources
                                )

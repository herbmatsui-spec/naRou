...
scenes = [
    {"floor": "B1F 洞窟の拠点", "action": "我が家でシエルと会話＆冒険準備中...", "player_pos": (2, 5), "mobs": [], "logs": ["『naRou: Masterpiece Edition』の世界へようこそ！", "妹分シエル「お兄ちゃん、今日も一緒に頑張ろうね！」"]},
    {"floor": "B1F ヴェルニスの街", "action": "鉱山の街ヴェルニスに到着！酒場で依頼を受注", "player_pos": (6, 4), "mobs": [("商人", (5, 3), (120, 200, 255)), ("街の案内人", (8, 6), (200, 220, 100))], "logs": ["街の案内人「ここは鉱山の街ヴェルニスだよ」", "酒場マスターから『子犬の救出依頼』を受けた！"]},
    {"floor": "B3F 子犬の洞窟", "action": "ダンジョン探索！ぷちの大群と交戦中", "player_pos": (10, 5), "mobs": [("ぷち", (11, 5), (100, 255, 150)), ("ぷち", (12, 6), (100, 255, 150)), ("オーク", (14, 4), (255, 120, 120))], "logs": ["ぷちに通常攻撃！ 18のダメージ！", "ぷちを撃破した！ (経験値+35, 120G)"]},
    {"floor": "B7F ネフィア深層", "action": "★火炎の古龍との決戦！範囲魔法を詠唱！", "player_pos": (8, 6), "mobs": [("★古龍", (14, 5), (255, 60, 60)), ("竜の眷属", (12, 3), (255, 140, 60))], "logs": ["★古龍のブレスを回避！", "必殺スキル『神威・次元斬』発動！ 248のクリティカル！"]},
    {"floor": "クリア 輪廻の祠", "action": "主クエスト制覇！第1回転生NG+へ突入", "player_pos": (11, 5), "mobs": [("女神ジュア", (11, 2), (255, 220, 100))], "logs": ["主クエスト『レシマスの秘宝』を完全踏破！", "転生ボーナス獲得: 全能力値+10%, カルマ+50, 継承スキル開放！"]}
]
...
for sc_idx, sc in enumerate(scenes):
    for sub in range(4):  # 4 frames per scene for smooth animation
        ...
        map_w = COLS * CELL_SZ
        map_h = ROWS * CELL_SZ
        draw.rectangle([(GRID_X - 4, GRID_Y - 4), (GRID_X + map_w + 4, GRID_Y + map_h + 4)], fill=(12, 16, 28), outline=PANEL_BORDER, width=2)
        ...
        for r in range(ROWS):
            for c in range(COLS):
                ...
                cy = GRID_Y + r * CELL_SZ
                # Draw floor dot
                if (r == 0 or r == ROWS - 1 or c == 0 or c == COLS - 1) and not (r == 5 and c == COLS - 1):
                    # Wall
                    draw.rectangle([(cx, cy), (cx + CELL_SZ - 2, cy + CELL_SZ - 2)], fill=(30, 38, 60))
                else:
                    # Floor
                    draw.text((cx + 8, cy + 4), ".", fill=(45, 60, 90))

        # Draw stairs
        draw.text((GRID_X + (COLS - 2) * CELL_SZ + 6, GRID_Y + 5 * CELL_SZ + 2), ">", fill=(100, 200, 255))
        ...
        draw.rectangle([(stat_x + 12, GRID_Y + 76), (stat_x + stat_w - 12, GRID_Y + 84)], fill=(30, 40, 50))
        draw.rectangle([(stat_x + 12, GRID_Y + 76), (stat_x + stat_w - 12, GRID_Y + 84)], fill=HP_GREEN)

        # MP Bar
        draw.text((stat_x + 12, GRID_Y + 96), "MP 190/190", fill=MP_BLUE)
        draw.rectangle([(stat_x + 12, GRID_Y + 114), (stat_x + stat_w - 12, GRID_Y + 122)], fill=(30, 40, 50))
        draw.rectangle([(stat_x + 12, GRID_Y + 114), (stat_x + int((stat_w - 24) * 0.85), GRID_Y + 122)], fill=MP_BLUE)
        ...
        log_h = H - log_y - 12
        draw.rectangle([(16, log_y), (W - 16, log_y + log_h)], fill=(12, 16, 26), outline=PANEL_BORDER, width=1)
        ...
        for l_idx, log_line in enumerate(sc["logs"][:2]):
            draw.text((26, log_y + 26 + l_idx * 16), f"• {log_line}", fill=TEXT_WHITE if l_idx == 0 else TEXT_MUTED)

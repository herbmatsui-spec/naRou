...
class PetProfile:
    ...
    fusion_history: List[str] = field(default_factory=list)  # 融合履歴（過去に融合した種族のリスト）
...
class PetQuestAnalyzer:
    ...
    def analyze_pet(self, player: Entity) -> Optional[PetProfile]:
...
def analyze_active_pet(player: Entity) -> Optional[PetProfile]:

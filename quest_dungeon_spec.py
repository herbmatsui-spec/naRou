...
class DungeonSpec:
    ...
    # クエスト連携
    quest_id: str = ""
    objective_mapping: Dict[str, str] = field(default_factory=dict)  # objective_id -> room_id/trap_id/enemy_id
...
class FloorSpec:
    ...
    boss_room_spec: Optional[RoomSpec] = None
    # 接続制約
...
def _parse_room_spec(data: Dict[str, Any]) -> RoomSpec:
    ...
    room_type = RoomType[room_type_str] if room_type_str in RoomType.__members__ else RoomType.CHAMBER
    ...
    for trap_data in data.get("traps", []):
        ...
        trap_type = TrapType[trap_type_str] if trap_type_str in TrapType.__members__ else TrapType.SPIKE
        trap = TrapSpec(

...
class JobRepository(CachedRepository[Job, str]):
    ...
    def __init__(self, jobs: dict[str, JobDefinition]):
        super().__init__(jobs)
        self._by_tier: dict[int, list[JobDefinition]] = {}
        self._build_indexes()
    ...
    def get_by_tier(self, tier: int) -> list[JobDefinition]:
        return self._by_tier.get(tier, [])
    
    def get_unlocked_for(self, player_level: int, player_skills: dict[str, int], 
                         player_stats: dict[str, int], current_job: str) -> list[JobDefinition]:
        """プレイヤーが転職可能な職業一覧"""
        available = []
        for job in self._data.values():
...
class GodRepository(CachedRepository[God, str]):
    ...
    def __init__(self, gods: dict[str, GodDefinition]):
        super().__init__(gods)
        self._by_domain: dict[str, list[GodDefinition]] = {}
        self._build_indexes()
    ...
    def get_by_domain(self, domain: str) -> list[GodDefinition]:
        return self._by_domain.get(domain, [])
    
    def get_by_favored_offer(self, offer_type: str) -> list[GodDefinition]:
...
class GuildRepository(CachedRepository[Guild, str]):
    ...
    def __init__(self, guilds: dict[str, GuildDefinition]):
        super().__init__(guilds)
        self._by_location: dict[str, list[GuildDefinition]] = {}
        self._build_indexes()
    ...
    def get_by_location(self, location: str) -> list[GuildDefinition]:
        return self._by_location.get(location, [])
    
    def get_available_for_rank(self, current_rank: str) -> list[GuildDefinition]:
        """現在のランクで昇格可能なギルド"""
...
__all__ = ["DungeonThemeRepository", "GodRepository", "GuildRepository", "JobRepository"]
...
class AchievementRepository(CachedRepository[Achievement, str]):
    ...
    def __init__(self, achievements: dict[str, Achievement]):
        super().__init__(achievements)
        self._by_type: dict[str, list[Achievement]] = {}
        self._hidden: list[Achievement] = []
        self._build_indexes()
    ...
    def get_by_reward_type(self, reward_type: str) -> list[Achievement]:
        ...
        for ach in self._data.values():
            if (
                reward_type == "title"
                and ach.reward_title
                or reward_type == "gold"
                and ach.reward_gold
                or reward_type == "item"
                and ach.reward_item
                or reward_type == "skill_point"
                and ach.reward_skill_points
            ):
                result.append(ach)
        return result
...
class TitleRepository(CachedRepository[Title, str]):
    ...
    def get_by_category(self, category: str) -> list[Title]:
        return self._by_category.get(category, [])

    def get_visible(self) -> list[Title]:
        return [t for t in self._data.values() if not t.is_hidden]

    def get_hidden(self) -> list[Title]:
        return self._hidden[:]

    def get_by_condition_type(self, cond_type: str) -> list[Title]:
        result = []
        for title in self._data.values():

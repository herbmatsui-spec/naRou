...
from src.relationships import create_engine, RelationshipType
...
""")
    ...
    betrayal_result = engine.betrayal.commit_betrayal(
        "mordred", "player", __import__('src.relationships.betrayal', fromlist=['BetrayalType']).BetrayalType.BACKSTAB,
        context={'evidence_available': True, 'witnesses': ['elena']}
    )
    print(f"[7] 裏切り発生: 敵対レベル変化 = {betrayal_result['impact'].get('favorability', 'N/A')}")
    ...
    if load_result['success']:
        level = new_engine.manager.get_relationship_level("player", "elena", RelationshipType.ROMANCE)

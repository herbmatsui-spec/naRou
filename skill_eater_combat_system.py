...
INDIGESTION_COMBOS: Dict[Tuple[str, str], Tuple[str, int]] = {
    ("Fire", "Water"): ("【消化不良：水蒸気爆破】胃袋の中で反発反応！主人公が自傷ダメージを受けた！", 25),
...
class AnalysisResult:
    ...
    system_log: str
    synergy_hint: Optional[str] = None
...
class BattleActionResult:
    ...
    damage_dealt: int = 0
    heal_amount: int = 0
    stolen_skill_id: Optional[str] = None
    stolen_skill_name: Optional[str] = None
...
class SkillEaterCombatSystem:
    ...
    def analyze_target(self, analyzer: CharacterState, target: CharacterState) -> AnalysisResult:
        ...
        for skill_id in target.get_skill_ids():
            ...
            if lv >= 7:
                can_see_full = True
            elif lv >= 3 and skill_def.tier in [SkillTier.COMMON, SkillTier.RARE]:
                can_see_full = True
            elif lv < 3 and skill_def.tier == SkillTier.COMMON:
                can_see_full = True
    ...
    def execute_hack(self, analyzer: CharacterState, target: CharacterState) -> BattleActionResult:
        sounds = []
        events = []
        ...
        if is_success:
            ...
            return BattleActionResult(
                ...
                target_name=target.name,
                success=True,
                log_messages=[f"【ハック成功！】{analyzer.name}は{target.name}の防壁を突破！ 真のデータ構造を開示しました！"],
                played_sounds=sounds,
                presentation_events=events
            )
        else:
            ...
            return BattleActionResult(
                ...
                target_name=target.name,
                success=False,
                log_messages=[f"【ハック失敗…】逆探知カウンターを受け、{analyzer.name}の知力が一時的に低下した！"],
                played_sounds=sounds,
                presentation_events=events
            )

    # Step 45: スキル任意破棄コマンド（Emote: emote_drop.png）
    def discard_skill(self, character: CharacterState, skill_id: str) -> Tuple[bool, str, List[str], List[PresentationEvent]]:
        ...
        return True, f"《{s_name}》を脳内メモリから消去しました。（空きメモリ増加、精神侵食度 -20 ➔ {character.addiction_buildup}）", ["dropLeather.ogg"], [evt]
    ...
    def execute_basic_attack(self, attacker: CharacterState, defender: CharacterState) -> BattleActionResult:
        sounds = []
        events = []
    ...
    def execute_devour(
        ...
        if not available_skills or prey.is_husk:
            ...
            return BattleActionResult(
                ...
                success=False,
                log_messages=[f"{prey.name}は既にスキルを持たない空っぽ（Husk）だ！ 喰らうものがない！"],
                played_sounds=[],
                presentation_events=events
            )
        ...
        if predator.current_memory_usage + cost > predator.max_memory_capacity:
            ...
            return BattleActionResult(
                ...
                target_name=prey.name,
                success=False,
                log_messages=[f"【脳内メモリ容量不足！】《{skill_name}》（必要: {cost}）を記憶する空きがありません！（現在: {predator.current_memory_usage}/{predator.max_memory_capacity}） スキルを破棄してください！"],
        ...
        logs.append(f"{predator.name}は禁忌の力《喰らい》を発動！ 対象スキル:《{skill_name}》（成功率: {int(rate*100)}%）")
        ...
        if is_success:
            ...
            logs.append(f"{predator.name}は新たなスキル《{skill_name}》を獲得！（メモリ使用量: {predator.current_memory_usage}/{predator.max_memory_capacity}）")
            ...
            bonus_dmg, indig_dmg, combo_logs, combo_sounds, combo_events = self.evaluate_devour_combo(predator, prey, primary_elem)
            ...
            logs.append(f"【捕食失敗……！】スキルの拒絶反応により暴走が発生！ {predator.name}は {backlash_damage} の反動ダメージを受けた！")
    ...
    def process_turn_end(self, character: CharacterState) -> Tuple[List[str], List[str], List[PresentationEvent]]:
        logs = []
        sounds = []
        events = []
        ...
        if character.addiction_buildup >= 100:
            ...
            logs.append(f"【精神崩壊警報！】{character.name}の脳が強力なスキルに侵食され『スキル中毒（Addicted）』に陥った！")
            ...
            logs.append(f"※警告：{character.name}の精神侵食度が限界寸前（{character.addiction_buildup}/100）です。")

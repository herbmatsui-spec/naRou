...
def main():
    ...
    parser.add_argument("--selective", nargs=3, metavar=("BACKUP", "PATTERNS", "TARGET"), help="Selective restore")
    parser.add_argument("--list-rp", action="store_true", help="List restore points")
    parser.add_argument("--rollback", nargs=2, metavar=("RESTORE_POINT", "TARGET"), help="Rollback to restore point")
    ...
    if args.find is not None:
        ...
        for backup in backups:
            print(f"{backup['name']} - {backup['size']} bytes - {backup['created_at']} ({backup['source_dir']})")
    elif args.restore:
        ...
        for restoration in mgr.get_history():
            print(f"{restoration['restored_at']} - {restoration['backup_name']} -> {restoration['target_dir']} ({restoration['status']})")
    else:
        parser.print_help()

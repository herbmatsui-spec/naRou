...
class PetDispatchManager:
    ...
    def start_dispatch(self, pet_id: str, dungeon_id: str, duration_turns: int = 3) -> Dict[str, Any]:
        """Step 21: 派遣開始と成功率計算"""
        ...
        success_rate = min(0.95, dungeon["base_success_rate"] + (suitability["match_rate"] * 0.20) + (pet["level"] * 0.02))
    ...
    def resolve_dispatch(self, pet_id: str, force_success: Optional[bool] = None) -> Dict[str, Any]:
        ...
        is_success = force_success if force_success is not None else (random.random() <= dispatch_info["success_rate"])
    ...
    def treat_pet(self, pet_id: str, medical_junk_cost: int = 50) -> Dict[str, Any]:
        ...
        return {"success": True, "message": f"{pet['name']} has been successfully treated and ready for dispatch."}

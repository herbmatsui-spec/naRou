...
def demo_basic_functionality():
    ...
    print(f"   取得/作成されたレイヤー: {test_layer.zone}:{test_layer.biome}:{test_layer.depth}:{test_layer.dimension}")
    ...
    if game_map:
        ...
        if game_map.world_layer:
            print(f"   マップに関連付けられたレイヤー: {game_map.world_layer.zone}:{game_map.world_layer.biome}:{game_map.world_layer.depth}:{game_map.world_layer.dimension}")
    print()
    ...
    for i, adj_layer in enumerate(adjacent_layers[:3]):  # 最初の3つだけ表示
        print(f"   隣接レイヤー{i+1}: {adj_layer.zone}:{adj_layer.biome}:{adj_layer.depth}:{adj_layer.dimension}")

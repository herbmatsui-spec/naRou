...
def _create_starter_items(kernel: Kernel) -> list:
    from item_system import Inventory, Item, create_sample_item, CAT_FOOD, QUALITY_NORMAL
...
def _spawn_dungeon(kernel: Kernel, engine) -> None:
    from constants import (
        SPAWN_SNAIL_CHANCE, SPAWN_MONSTER_CHANCE, SPAWN_ITEM_CHANCE,
        SPAWN_RESOURCE_NODE_CHANCE, SNAIL_SPEED, SNAIL_COLOR
    )
...
class GameplayLoop:
    ...
    def player_act(self, dx: int, dy: int) -> bool:
        ...
        if target and target not in (self.engine.player, self.engine.pet):
            ...
            self.engine.floating_texts.append(FloatingText(f"-{dmg}", target.x, target.y - 0.2, (255, 100, 100) if not is_crit else (255, 230, 80)))
            ...
            if (tx, ty) == self.engine.altar_pos:
                ...
                self.engine.log(f"神【{GodInfo.GODS[self.engine.player.god_id]['name']}】の祭壇。([p]祈る [o]捧げる)", (255, 215, 0))
            ...
            if tile == TILE_TRAP:
                ...
                self.engine.log("トラップ発動！ 毒矢が急所を貫く！ (-6 HP)", (255, 80, 80), level="WARNING")
            self.engine.player.energy -= ENERGY_THRESHOLD
            return True
        return False
    ...
    def advance_world(self) -> None:
        ...
        while self.engine.player.energy < ENERGY_THRESHOLD and cycle < max_cycles:
            cycle += 1
            actor, _ = self.engine.turn_queue.step_next_actor(self.engine.entity_manager.get_living_entities())
        ...
        for entity in list(self.engine.entity_manager.get_living_entities()):
            if entity.hp > 0:
                ...
                if logs and (entity == self.engine.player or self.engine.has_los(Point(self.engine.player.x, self.engine.player.y), Point(entity.x, entity.y))):
                    for l in logs:
        ...
        if hasattr(self.engine, 'quest_scheduler'):
            ...
            available = self.engine.quest_scheduler.get_available_quests(context, self.engine.player)

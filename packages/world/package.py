...
class WorldPackage(IPackage):
    ...
    def setup(self, kernel: Kernel) -> None:
        ...
        kernel.register_system(
            "world_event_manager", WorldEventManager(world_event_reg)
        )

        archaeology_reg = ArchaeologyRegistry()
        archaeology_reg.load()
        kernel.register_system(
            "archaeology_manager", ArchaeologyManager(archaeology_reg)
        )

        world_state_reg = WorldStateRegistry()
        world_state_reg.load()
        kernel.register_system(
            "world_state_manager", WorldStateManager(world_state_reg)
        )

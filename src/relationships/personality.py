...
class PersonalityProfile:
    ...
    def get_dominant_trait(self) -> Optional[PersonalityTrait]:
        """最も強い特性を取得"""
        if not self.traits:
            return None
    ...
    def calculate_relationship_modifier(self, relationship_type: RelationshipType) -> float:
        ...
        if relationship_type == RelationshipType.FRIENDSHIP:
            ...
            modifier = 0.3 + (1 - self.get_trait(PersonalityTrait.CONSCIENTIOUSNESS)) + self.get_trait(PersonalityTrait.NEUROTICISM) * 0.3
        elif relationship_type == RelationshipType.MENTORSHIP:
            modifier = 0.5 + self.get_trait(PersonalityTrait.OPENNESS) * 0.5 + self.get_trait(PersonalityTrait.CONSCIENTIOUSNESS) * 0.5
        elif relationship_type == RelationshipType.ROMANCE:
            modifier = 0.4 + self.get_trait(PersonalityTrait.OPENNESS) * 0.3 + self.get_trait(PersonalityTrait.AGREEABLENESS) * 0.3
        elif relationship_type == RelationshipType.RIVALRY:
            modifier = 0.5 + self.get_trait(PersonalityTrait.EXTRAVERSION) * 0.3 + (1 - self.get_trait(PersonalityTrait.AGREEABLENESS)) * 0.2
...
class PersonalitySystem:
    ...
    def assign_personality(self, character_id: str, 
                         traits: Optional[Dict[PersonalityTrait, float]] = None,
                         archetype: Optional[CharacterArchetype] = None) -> PersonalityProfile:
        """キャラクターにパーソナリティを割り当て"""
        # 既存のプロファイルをチェック
    ...
    def _calculate_relationship_tendencies(self, profile: PersonalityProfile) -> Dict[RelationshipType, float]:
        """関係傾向を計算"""
    ...
    def _calculate_trait_modifier(self, profile: PersonalityProfile, 
        ...
        if rel_type == RelationshipType.FRIENDSHIP:
            ...
            betrayal_factor = (1 - t.get(PersonalityTrait.CONSCIENTIOUSNESS, 0.5)) * 0.7 + \
                           t.get(PersonalityTrait.NEUROTICISM, 0.5) * 0.3
            modifier *= (0.6 + betrayal_factor)
        elif rel_type == RelationshipType.MENTORSHIP:
            modifier *= (0.6 + t.get(PersonalityTrait.OPENNESS, 0.5) * 0.4 + \
                        t.get(PersonalityTrait.CONSCIENTIOUSNESS, 0.5) * 0.4)
        elif rel_type == RelationshipType.ROMANCE:
            modifier *= (0.6 + t.get(PersonalityTrait.OPENNESS, 0.5) * 0.3 + \
    ...
    def get_interaction_modifier(self, source_id: str, target_id: str,
        ...
        if target_profile:
            ...
            if interaction_type in [InteractionType.TALK, InteractionType.GIFT, InteractionType.EMOTIONAL_SUPPORT]:
    ...
    def generate_random_personality(self, character_id: str, 
                                  archetype: Optional[CharacterArchetype] = None) -> PersonalityProfile:
        """ランダムなパーソナリティを生成"""
    ...
    def get_compatibility_between(self, char_a: str, char_b: str) -> float:
        ...
        complementarity = 0.0
        if PersonalityTrait.EXTRAVERSION in profile_a.traits and PersonalityTrait.EXTRAVERSION in profile_b.traits:
    ...
    def predict_relationship_development(self, char_a: str, char_b: str,
    ...
    def get_profile(self, character_id: str) -> Optional[PersonalityProfile]:
        """パーソナリティプロファイルを取得"""
        ...
        return {
            'profiles': {
                char_id: {
                    ...
                    'dominant_trait': profile.dominant_trait.value if profile.dominant_trait else None,
    ...
    def deserialize(self, data: Dict[str, Any]) -> None:
        ...
        for char_id, profile_data in data.get('profiles', {}).items():
            profile = PersonalityProfile(
                ...
                archetype=CharacterArchetype(profile_data['archetype']) if profile_data.get('archetype') else None,
                dominant_trait=PersonalityTrait(profile_data['dominant_trait']) if profile_data.get('dominant_trait') else None,
                relationship_tendencies={
                    RelationshipType(k): v for k, v in profile_data.get('relationship_tendencies', {}).items()
                }
            )

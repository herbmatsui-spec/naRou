...
class RomanceState:
    ...
    relationship_start: Optional[float] = None
...
class RomanceMechanics:
    ...
    def initiate_romance(self, char_a: str, char_b: str) -> Optional[RomanceState]:
        """二人の間でロマンスを開始"""
        # 既存の状態をチェック
        ...
        state = RomanceState(
            ...
            relationship_start=time.time() if initial_stage in [RomanceStage.DATING, RomanceStage.COMMITTED] else None
        )
    ...
    def update_romance_stage(self, char_a: str, char_b: str) -> Optional[RomanceStage]:
        ...
        if state.romance_level >= self._romance_config['marriage_threshold'] and \
           state.commitment_level >= 80:
            new_stage = RomanceStage.MARRIED
        elif state.romance_level >= self._romance_config['commitment_threshold'] and \
             state.commitment_level >= 50:
    ...
    def can_confess(self, char_a: str, char_b: str) -> bool:
        ...
        return romance_level >= self._romance_config['confession_threshold'] and \
               favorability >= 30
    
    def confess(self, char_a: str, char_b: str, 
                confession_type: str = "sincere") -> Dict[str, Any]:
        """告白を実行"""
        ...
        if success:
            ...
            self.rm.modify_relationship(
                char_a, char_b, InteractionType.CONFESSION, 20
            )
            state.romance_level = self.rm.get_relationship_level(char_a, char_b, RelationshipType.ROMANCE)
            ...
            # 関係を弱める（拒絶）
            self.rm.modify_relationship(
                char_a, char_b, InteractionType.CONFESSION, -10
            )
            state.romance_level = self.rm.get_relationship_level(char_a, char_b, RelationshipType.ROMANCE)
    ...
    def _calculate_confession_success(self, char_a: str, char_b: str, 
                                    confession_type: str) -> float:
        """告白の成功確率を計算"""
        # 基本確率
        base_prob = 0.5
    ...
    def check_jealousy(self, char_a: str, char_b: str, 
                      third_party: str) -> Dict[str, Any]:
        """嫉妬をチェック（三角関係）"""
        ...
        third_romance = self.rm.get_relationship_level(char_a, third_party, RelationshipType.ROMANCE)
        third_favorability = self.rm.get_relationship_level(char_a, third_party, RelationshipType.FAVORABILITY)
    ...
    def break_up(self, char_a: str, char_b: str, 
                reason: str = "drifted_apart") -> Dict[str, Any]:
        """別れを実行"""
        couple_key = tuple(sorted([char_a, char_b]))
        state = self.romance_states.get(couple_key)
        if not state:
    ...
    def calculate_anniversary_bonus(self, char_a: str, char_b: str) -> Dict[str, Any]:
        ...
        if years >= 1.0:
            ...
            self.rm.modify_relationship(char_a, char_b, InteractionType.EMOTIONAL_SUPPORT, bonus_amount)
    ...
    def register_romance_event_handler(self, event_type: RomanceEventType,
    ...
    def get_romance_state(self, char_a: str, char_b: str) -> Optional[RomanceState]:
        """ロマンス状態を取得"""
        couple_key = tuple(sorted([char_a, char_b]))
        return self.romance_states.get(couple_key)
    
    def get_active_romances(self, character_id: Optional[str] = None) -> List[RomanceState]:
        """アクティブなロマンスを取得"""
        if character_id is None:
            return [state for state in self.romance_states.values() 
                   if state.stage not in [RomanceStage.STRANGERS]]
        
        return [state for state in self.romance_states.values()
               if character_id in state.couple_id and 
               state.stage not in [RomanceStage.STRANGERS]]

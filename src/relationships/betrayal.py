...
class BetrayalConflictSystem:
    ...
    def commit_betrayal(self, betrayer_id: str, victim_id: str, 
                      betrayal_type: BetrayalType, 
        ...
        # 敵対関係を強化
        self.rm.modify_relationship(betrayer_id, victim_id, InteractionType.COMBAT_ENEMY, impact // 3)
    ...
    def _check_revenge_trigger(self, victim_id: str, betrayer_id: str, severity: int) -> bool:
        ...
        if victim_id in self.revenge_cooldowns:
            if current_time - self.revenge_cooldowns[victim_id] < self._config['revenge_cooldown']:
                return False
    ...
    def execute_revenge(self, avenger_id: str, target_id: str, 
        ...
        self.rm.modify_relationship(target_id, avenger_id, InteractionType.COMBAT_ENEMY, impact // 2)
        ...
        # イベント発行
        self._emit_event("revenge_executed", {
    ...
    def attempt_reconciliation(self, party_a: str, party_b: str, 
                             mediator_id: Optional[str] = None,
                             sincerity: float = 0.5) -> Dict[str, Any]:
        """和解を試みる"""
        # 対立状態を確認
        ...
        success_chance = base_chance + mediator_bonus + sincerity_bonus - intensity_penalty - attempt_penalty
        ...
        if success:
            ...
            self.rm.modify_relationship(party_a, party_b, InteractionType.EMOTIONAL_SUPPORT, improvement)
            self.rm.modify_relationship(party_a, party_b, InteractionType.EMOTIONAL_SUPPORT, improvement // 2)
            
            # 対立状態を和解済みに
            self._update_conflict_state(party_a, party_b, ConflictState.RECONCILED, "reconciliation")
            
            self._stats['successful_reconciliations'] += 1
            
            # イベント発行
            self._emit_event("reconciliation_success", {
                'party_a': party_a,
                'party_b': party_b,
            ...
            if random.random() < 0.3:
                ...
                self._update_conflict_state(party_a, party_b, ConflictState.HOSTILE, "failed_reconciliation")
            
            # イベント発行
            self._emit_event("reconciliation_failed", {
                'party_a': party_a,
                'party_b': party_b,
                'mediator_id': mediator_id,
                'timestamp': time.time()
    ...
    def _update_conflict_state(self, party_a: str, party_b: str, 
                             new_state: ConflictState, trigger: str) -> ConflictRecord:
        """対立状態を更新"""
    ...
    def get_conflict_state(self, party_a: str, party_b: str) -> Optional[str]:
        """対立状態を取得"""
        record = self.get_conflict_record(party_a, party_b)
        return record.state.value if record else None
    
    def get_conflict_record(self, party_a: str, party_b: str) -> Optional[ConflictRecord]:
        """対立記録を取得"""
        key = tuple(sorted([party_a, party_b]))
    ...
    def calculate_trust_recovery(self, party_a: str, party_b: str, 
                              days_passed: float = 1.0) -> int:
        """信頼の回復を計算・適用"""
        record = self.get_conflict_record(party_a, party_b)
        if not record:
            return 0
        ...
        if recovery > 0:
            self.rm.modify_relationship(party_a, party_b, InteractionType.EMOTIONAL_SUPPORT, recovery)
            self.rm.modify_relationship(party_a, party_b, InteractionType.EMOTIONAL_SUPPORT, recovery)
    ...
    def spread_rumor(self, source_id: str, target_id: str, 
                   rumor_type: str = "betrayal", credibility: float = 0.5) -> Dict[str, Any]:
        """噂を広める（評判への影響）"""
        # 噂の影響を計算
        ...
        for other_id in self.graph.nodes:
            ...
            if edge:
                edge.add_modifier(self.rm._create_modifier(
    ...
    def get_betrayal_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            'active_conflicts': len([r for r in self.conflict_records.values() 

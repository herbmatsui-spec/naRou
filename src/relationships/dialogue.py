...
class DialogueTemplate:
    ...
    max_level: int
    templates: List[str]
    required_archetype: Optional[CharacterArchetype] = None
...
class DialogueGenerationSystem:
    ...
    def _load_topics(self) -> Dict[DialogueContext, List[str]]:
        ...
        return {
            DialogueContext.SMALL_TALK: [
                "最近の天気について", "この街の噂話", "美味しい店の話", 
                "趣味の話", "最近読んだ本", "旅の思い出"
            ],
            DialogueContext.QUEST: [
                "近くで起きている事件", "困っている村人", "盗まれた品物",
                "魔物の出没", "古い遺跡の探索", "ギルドの依頼"
            ],
    ...
    def generate_dialogue(self, speaker_id: str, listener_id: str,
                        context: DialogueContext,
                        relationship_type: Optional[RelationshipType] = None) -> Optional[GeneratedDialogue]:
        """対話を生成"""
        # 話し手と聞き手の情報を取得
        ...
        if self.personality_system:
            ...
            if speaker_profile:
                personality_modifier = speaker_profile.get_trait(PersonalityTrait.EXTRAVERSION)
    ...
    def _select_template(self, context: DialogueContext,
                       relationship_type: RelationshipType,
                       relationship_level: int,
                       speaker_id: str = "") -> Optional[DialogueTemplate]:
        """条件に合うテンプレートを選択"""
        candidates = []
        ...
        for template in self.templates:
            ...
            if template.required_archetype:
                speaker_profile = self.personality_system.get_profile(speaker_id) if self.personality_system else None
        ...
        if not candidates:
            ...
            for template in self.templates:
                if template.context == context and template.min_level <= relationship_level <= template.max_level:
                    candidates.append(template)
    ...
    def _generate_follow_up_options(self, context: DialogueContext, mood: DialogueMood) -> List[str]:
        """フォローアップオプションを生成"""
    ...
    def generate_procedural_dialogue(self, speaker_id: str, listener_id: str,
                                   context: DialogueContext,
                                   relationship_type: Optional[RelationshipType] = None) -> Optional[GeneratedDialogue]:
        ...
        base_text = self._generate_base_text(context, relationship_type, relationship_level, speaker_node, listener_node)
    ...
    def _generate_base_text(self, context: DialogueContext,
                          relationship_type: RelationshipType,
                          level: int, speaker_node: Any, listener_node: Any) -> str:
        """ベーステキストを生成"""
    ...
    def _determine_mood_from_level(self, relationship_type: RelationshipType, level: int) -> DialogueMood:
        """レベルから雰囲気を決定"""
    ...
    def _record_dialogue(self, dialogue: GeneratedDialogue) -> None:
        ...
        if len(self.dialogue_history[key]) > self._config['max_history_per_pair']:
            self.dialogue_history[key] = self.dialogue_history[key][-self._config['max_history_per_pair']:]
    
    def get_dialogue_history(self, speaker_id: str, listener_id: str, 
                           limit: int = 10) -> List[GeneratedDialogue]:
        """対話履歴を取得"""
        key = (speaker_id, listener_id)
        history = self.dialogue_history.get(key, [])
        return history[-limit:] if limit else history
    
    def generate_relationship_reflection(self, character_id: str, 
                                       other_id: str) -> str:
        """関係についての内省的な対話を生成"""
    ...
    def _create_reflection_text(self, rel_type: RelationshipType, level: int, 
                             other_name: str) -> str:
        """内省テキストを作成"""
        rel_names = {
    ...
    def set_character_speech_pattern(self, character_id: str, 

...
class MentorshipState:
    ...
    last_lesson: Optional[float] = None
    mentorship_start: Optional[float] = None
...
class MentorshipMechanics:
    ...
    def _load_skill_knowledge(self) -> Dict[str, SkillKnowledge]:
        """スキル知識ベースをロード"""
        return {
            "basic_sword": SkillKnowledge("basic_sword", "基本剣術", SkillTransferType.BASIC, 10, 10),
            "advanced_sword": SkillKnowledge("advanced_sword", "上級剣術", SkillTransferType.ADVANCED, 30, 30, ["basic_sword"]),
            "secret_sword": SkillKnowledge("secret_sword", "秘剣", SkillTransferType.SECRET, 60, 60, ["advanced_sword"]),
            "ultimate_sword": SkillKnowledge("ultimate_sword", "奥義・天剣", SkillTransferType.ULTIMATE, 90, 90, ["secret_sword"]),
            "basic_magic": SkillKnowledge("basic_magic", "基本魔法", SkillTransferType.BASIC, 10, 15),
            "advanced_magic": SkillKnowledge("advanced_magic", "上級魔法", SkillTransferType.ADVANCED, 35, 40, ["basic_magic"]),
            "forbidden_magic": SkillKnowledge("forbidden_magic", "禁呪", SkillTransferType.SECRET, 65, 80, ["advanced_magic"]),
            "healing_art": SkillKnowledge("healing_art", "治癒術", SkillTransferType.BASIC, 15, 20),
            "resurrection": SkillKnowledge("resurrection", "復活の術", SkillTransferType.ULTIMATE, 95, 95, ["healing_art"]),
            ...
            "leadership": SkillKnowledge("leadership", "統率学", SkillTransferType.ADVANCED, 45, 45),
    ...
    def establish_mentorship(self, master_id: str, disciple_id: str, 
                           initial_stage: MentorshipStage = MentorshipStage.ACQUAINTANCE) -> Optional[MentorshipState]:
        """師弟関係を確立"""
        # 既存の関係をチェック
        ...
        # 関係レベルを取得
        mentorship_level = self.rm.get_relationship_level(master_id, disciple_id, RelationshipType.MENTORSHIP)
    ...
    def update_mentorship_stage(self, master_id: str, disciple_id: str) -> Optional[MentorshipStage]:
        """師弟ステージを更新"""
        key = (master_id, disciple_id)
        ...
        if new_stage != old_stage:
            ...
            if new_stage == MentorshipStage.APPRENTICE and old_stage == MentorshipStage.ACQUAINTANCE:
    ...
    def teach_skill(self, master_id: str, disciple_id: str, 
        ...
        for prereq in skill.prerequisites:
            if prereq not in state.skills_learned:
                return {"success": False, "reason": "prerequisites_not_met", "missing": prereq}
        ...
        state.mentorship_level = self.rm.get_relationship_level(master_id, disciple_id, RelationshipType.MENTORSHIP)
    ...
    def learn_skill(self, disciple_id: str, master_id: str, 
                  skill_id: str, effort: int = 50) -> Dict[str, Any]:
        """スキルを学ぶ（弟子側のアクション）"""
    ...
    def check_disciple_betrayal(self, master_id: str, disciple_id: str, 
                              betrayal_severity: int = 10) -> Dict[str, Any]:
        """弟子の裏切りをチェック・実行"""
        state = self.mentorship_states.get((master_id, disciple_id))
        if not state:
            return {"success": False, "reason": "no_mentorship"}
        ...
        if state.loyalty <= 20:
            ...
            # 関係を大幅に悪化
            self.rm.modify_relationship(master_id, disciple_id, InteractionType.BETRAYAL, -betrayal_severity * 2)
            state.mentorship_level = self.rm.get_relationship_level(master_id, disciple_id, RelationshipType.MENTORSHIP)
        ...
        return {"success": False, "reason": "loyalty_too_high", "loyalty": state.loyalty}
    
    def protect_master(self, disciple_id: str, master_id: str, 
                     sacrifice_level: int = 50) -> Dict[str, Any]:
        """師を守る（弟子の行動）"""
        state = self.mentorship_states.get((master_id, disciple_id))
        if not state:
            return {"success": False, "reason": "no_mentorship"}
    ...
    def register_mentorship_event_handler(self, event_type: str, 
                                      handler: Callable[..., Any]) -> None:
        """師弟イベントハンドラーを登録"""
        self._event_handlers[event_type].append(handler)
    
    def _emit_mentorship_event(self, event_type: str, state: MentorshipState, 
                             **kwargs: Any) -> None:
        """師弟イベントを発行"""
    ...
    def get_mentorship_state(self, master_id: str, disciple_id: str) -> Optional[MentorshipState]:
        """師弟状態を取得"""
        return self.mentorship_states.get((master_id, disciple_id))
    
    def get_active_mentorships(self, character_id: Optional[str] = None, 
                             role: str = "either") -> List[MentorshipState]:
        """アクティブな師弟関係を取得"""
        ...
        for state in self.mentorship_states.values():
            if role in ["master", "either"] and state.master_id == character_id:
                result.append(state)
            elif role in ["disciple", "either"] and state.disciple_id == character_id:
                result.append(state)
        return result

...
from .models import (
    ...
    RelationshipNode, RelationshipEdge, RelationshipModifier,
...
class RelationshipManager:
    ...
    def __init__(self, data_path: str = "data/character_relations.yaml"):
        ...
        self._threshold_listeners: Dict[Tuple[str, str, RelationshipType, RelationshipLevel], List[Callable]] = defaultdict(list)
    ...
    def _parse_template(self, template_id: str, data: Dict[str, Any]) -> RelationshipTemplate:
        ...
        rel_type_input = data.get('relationship_type', data.get('relationship_types', [RelationshipType.FAVORABILITY.value]))
        if isinstance(rel_type_input, list):
            # 配列の場合、最初の要素をメインタイプとする（後方互換性のため）
            main_rel_type = RelationshipType(rel_type_input[0]) if rel_type_input else RelationshipType.FAVORABILITY
        else:
    ...
    def establish_relationship(self, source_id: str, target_id: str, 
        ...
        if not source_node:
            source_node = self.initialize_character(
                source_id, 
                source_name or f"Character_{source_id}"
            )
        ...
        if not target_node:
            target_node = self.initialize_character(
                target_id,
                target_name or f"Character_{target_id}"
            )
        
        # テンプレートから関係を作成（多層対応）
        relationship_types = getattr(template, 'relationship_types', [template.relationship_type])
        initial_levels = getattr(template, 'initial_levels', {template.relationship_type.value: template.initial_level})
        decay_rates = getattr(template, 'decay_rates', {template.relationship_type.value: template.decay_rate})
    ...
    def modify_relationship(self, source_id: str, target_id: str,
                          ...
                          multiplier: float = 1.0) -> Dict[RelationshipType, int]:
        ...
        for edge in edges:
            ...
            # リスナーに通知
            self._notify_relationship_changed(source_id, target_id, edge.relationship_type, final_amount)
            
            # しきい値リスナーをチェック
            self._check_threshold_listeners(source_id, target_id, edge.relationship_type, edge.level)
        
        return changes
    
    def get_relationship_level(self, source_id: str, target_id: str, 
    ...
    def get_relationship_level_category(self, source_id: str, target_id: str, 
                                      relationship_type: RelationshipType) -> RelationshipLevel:
        """関係レベルのカテゴリを取得"""
    ...
    def _calculate_personality_modifier(self, source_id: str, target_id: str, 
    ...
    def add_change_listener(self, callback: Callable[[str, str, RelationshipType, int], None]) -> None:
        """関係変更リスナーを追加"""
        self._change_listeners.append(callback)
    
    def add_threshold_listener(self, source_id: str, target_id: str, 
                             relationship_type: RelationshipType,
                             threshold_level: RelationshipLevel,
                             callback: Callable[[str, str, RelationshipType, int], None]) -> None:
    ...
    def _notify_relationship_changed(self, source_id: str, target_id: str, 
                                   relationship_type: RelationshipType, change_amount: int) -> None:
        """関係変更をリスナーに通知"""
    ...
    def _notify_relationship_created(self, source_id: str, target_id: str, template_id: str) -> None:
        """新しい関係作成をリスナーに通知"""
        # ここでは単純に変更リスナーを呼び出す
        for callback in self._change_listeners:
            try:
    ...
    def _check_threshold_listeners(self, source_id: str, target_id: str,
                                 relationship_type: RelationshipType, new_level: int) -> None:

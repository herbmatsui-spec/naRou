...
class MemoryFragment:
    ...
    other_id: str | None  # 関連する他キャラクター（オプション）
    memory_type: MemoryType
    importance: MemoryImportance
    description: str
    timestamp: float
    emotional_intensity: float = 0.5  # 0.0〜1.0
    associated_relationship: RelationshipType | None = None
    decay_rate: float = 0.0001  # 記憶の薄れ率
    last_recalled: float = field(default_factory=time.time)
    recall_count: int = 0
    linked_memories: list[str] = field(default_factory=list)

    def get_effective_intensity(self, current_time: float | None = None) -> float:
        """現在の感情強度を取得（時間経過で減衰）"""
        ...
        if (
            self.memory_type == MemoryType.NOSTALGIA
    ...
    def should_fade(self, current_time: float | None = None) -> bool:
        """記憶が薄れるべきかチェック"""
        if current_time is None:
            current_time = time.time()
...
class MemorySystem:
    ...
    def __init__(self, relationship_manager: RelationshipManager):
        ...
        # 統計
        self._stats = {"total_memories": 0, "memories_recalled": 0, "memories_faded": 0}
    ...
    def create_memory(
        ...
        # 最大記憶数を超えた場合は古いものを削除
        if (
            len(self.memories_by_character[character_id])
            > self._config["max_memories_per_character"]
        ):
    ...
    def _calculate_importance(
        self, change_amount: int, context: dict[str, Any]
    ) -> MemoryImportance:
    ...
    def recall_memory(self, memory_id: str) -> MemoryFragment | None:
        """記憶を呼び起こす"""
    ...
    def get_memories_for_character(
        self, character_id: str, memory_type: MemoryType | None = None, limit: int = 10
    ) -> list[MemoryFragment]:
        """キャラクターの記憶を取得"""
    ...
    def apply_memory_influence(
        ...
        for memory in character_memories:
            ...
            effect = int(
                intensity
                * self._config["memory_influence_on_relationship"]
                * (
                    1
                    if memory.memory_type
                    in [
                        MemoryType.POSITIVE_EVENT,
                        MemoryType.NOSTALGIA,
    ...
    def decay_memories(self, current_time: float | None = None) -> int:
        """薄れるべき記憶を処理"""
        if current_time is None:
            current_time = time.time()
...
class RelationshipDecaySystem:
    ...
    def apply_decay(self, current_time: float | None = None) -> dict[str, int]:
        """時間経過による減衰を適用"""
        if current_time is None:
            current_time = time.time()
        ...
        for edge in self.graph.edges.values():
            ...
            if change != 0:
                ...
                edge.last_interaction = (
                    edge.last_interaction
                )  # 更新しない（減衰はインタラクションではない）

                total_changes[
                    f"{edge.source_id}->{edge.target_id}:{edge.relationship_type.value}"
                ] += change
    ...
    def _calculate_decay_change(
        ...
        # 最大減衰量でクリップ
        decay_amount = max(
            -self._config["max_decay_per_cycle"],
    ...
    def get_decay_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_changes_recorded": sum(
                abs(change)
                for record in self._decay_history

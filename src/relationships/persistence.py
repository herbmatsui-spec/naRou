...
class RelationshipPersistence:
    ...
    def load_from_dict(self, data: Dict[str, Any]) -> bool:
        ...
        if version != self.CURRENT_VERSION:
            ...
            print(f"Warning: Loading relationship data version {version}, current is {self.CURRENT_VERSION}")
    ...
    def create_backup(self, base_filename: str) -> Optional[str]:
        """バックアップを作成"""
...
class ComprehensiveRelationshipSaveSystem:
    ...
    def __init__(self, relationship_manager: RelationshipManager):
        ...
        # サブシステム参照（後で設定）
        self.romance_system: Optional[Any] = None
        self.mentorship_system: Optional[Any] = None
        self.betrayal_system: Optional[Any] = None
        self.memory_system: Optional[Any] = None
        self.personality_system: Optional[Any] = None
        self.dialogue_system: Optional[Any] = None
        self.faction_system: Optional[Any] = None
        self.quest_integration: Optional[Any] = None
        self.world_integration: Optional[Any] = None
    
    def register_subsystem(self, name: str, system: Any) -> None:
        """サブシステムを登録"""
        setattr(self, name, system)
    
    def save_comprehensive(self, filename: str, format: SaveFormat = SaveFormat.JSON) -> Dict[str, Any]:
        """包括的なセーブ"""
        save_data = {
    ...
    def load_comprehensive(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'romance' in subsystems and self.romance_system and hasattr(self.romance_system, 'deserialize'):
                self.romance_system.deserialize(subsystems['romance'])
                loaded.append('romance')
            
            if 'mentorship' in subsystems and self.mentorship_system and hasattr(self.mentorship_system, 'deserialize'):
                self.mentorship_system.deserialize(subsystems['mentorship'])
                loaded.append('mentorship')
            
            if 'betrayal' in subsystems and self.betrayal_system and hasattr(self.betrayal_system, 'deserialize'):
                self.betrayal_system.deserialize(subsystems['betrayal'])
                loaded.append('betrayal')
            
            if 'memory' in subsystems and self.memory_system and hasattr(self.memory_system, 'deserialize'):
                self.memory_system.deserialize(subsystems['memory'])
                loaded.append('memory')
            
            if 'personality' in subsystems and self.personality_system and hasattr(self.personality_system, 'deserialize'):
                self.personality_system.deserialize(subsystems['personality'])
                loaded.append('personality')
            
            if 'dialogue' in subsystems and self.dialogue_system and hasattr(self.dialogue_system, 'deserialize'):
            ...
            if 'quest' in subsystems and self.quest_integration and hasattr(self.quest_integration, 'deserialize'):
                self.quest_integration.deserialize(subsystems['quest'])
                loaded.append('quest')
            
            if 'world' in subsystems and self.world_integration and hasattr(self.world_integration, 'deserialize'):
...
# 便利な関数
def save_relationships(rm: RelationshipManager, filename: str, 
                     format: str = "json", comprehensive: bool = True,
                     save_system: Optional[ComprehensiveRelationshipSaveSystem] = None) -> Dict[str, Any]:
    """関係データを保存する便利関数"""
...
def load_relationships(rm: RelationshipManager, filename: str,

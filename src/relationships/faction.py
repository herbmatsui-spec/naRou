...
from .models import RelationshipType, FactionAffiliation, InteractionType
...
class FactionRelationshipSystem:
    ...
    def register_faction(self, faction_id: str, name: str, 
                       ideology: str = "", power_level: int = 0) -> FactionNode:
        """新しい派閥を登録"""
        if faction_id not in self.factions:
            self.factions[faction_id] = FactionNode(
                faction_id=faction_id,
    ...
    def assign_to_faction(self, character_id: str, faction_id: str, 
                        affiliation: FactionAffiliation = FactionAffiliation.MEMBER) -> bool:
        """キャラクターを派閥に所属させる"""
        # キャラクターが存在するかチェック
    ...
    def get_faction_relation(self, faction_a: str, faction_b: str) -> Optional[FactionRelation]:
        """二つの派閥間の関係を取得"""
        # 順序を正規化
    ...
    def aggregate_faction_relationships(self, faction_id: str) -> Dict[str, Dict[str, int]]:
        ...
        if faction_id in self._member_relationship_cache and \
           current_time - self._last_cache_update < 3600:  # 1時間キャッシュ
        ...
        member_counts: Dict[str, Dict[RelationshipType, int]] = defaultdict(lambda: defaultdict(int))
        ...
        for member_id in faction.members:
            ...
            for other_id, edge in self.graph.get_related_nodes(member_id):
                ...
                for other_faction_id, _ in other_node.faction_affiliations.items():
                    ...
                    aggregated[other_faction_id][edge.relationship_type.value] += edge.level * member_weight
    ...
    def update_faction_relation(self, faction_a: str, faction_b: str, 
                              delta: int, conflict_data: Optional[Dict[str, Any]] = None) -> Optional[FactionRelation]:
        """派閥間関係を更新"""
        ...
        if key not in self.faction_relations:
            ...
            # 平均関係強度を計算
            avg_strength = self._calculate_initial_faction_strength(aggregated_a, aggregated_b, faction_a, faction_b)
    ...
    def _calculate_initial_faction_strength(self, agg_a: Dict[str, Dict[str, int]],
                                          agg_b: Dict[str, Dict[str, int]],
                                          faction_a: str, faction_b: str) -> int:
        """初期派閥関係強度を計算"""
    ...
    def check_faction_conflicts(self) -> List[Dict[str, Any]]:
        ...
        for (faction_a, faction_b), relation in self.faction_relations.items():
            if relation.relation_type in [FactionRelationType.TENSE, FactionRelationType.HOSTILE, FactionRelationType.WAR]:
                conflicts.append({
                    'faction_a': faction_a,
                    'faction_b': faction_b,
    ...
    def simulate_faction_event(self, event_type: str, faction_id: str, 
                             target_faction_id: Optional[str] = None,
                             magnitude: int = 10) -> List[Dict[str, Any]]:
        ...
        if event_type == "war_declaration" and target_faction_id:
            ...
            relation = self.update_faction_relation(
                faction_id, target_faction_id, -magnitude,
            ...
            # 領土紛争
            relation = self.update_faction_relation(
                faction_id, target_faction_id, -magnitude // 2,
            ...
            for other_faction_id, rel_types in aggregated.items():
                ...
                for other_char_id, other_node in self.graph.nodes.items():
                    ...
                    if other_faction_id in other_node.faction_affiliations:
                        ...
                        if influence != 0:
                            influences[other_char_id] = influences.get(other_char_id, 0) + influence // 10

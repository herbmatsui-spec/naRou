...
class EventToInteractionMapper:
    ...
    @classmethod
    def get_interaction_type(cls, event_type: GameEventType) -> Optional[InteractionType]:
...
class RelationshipEventHandler:
    ...
    def _process_standard_event(self, event_type: GameEventType, event_data: Dict[str, Any]) -> None:
        """標準的なゲームイベントを処理"""
        # マッピングを取得
        ...
        try:
            ...
            if changes:
                self.event_bus.publish("RELATIONSHIP_UPDATED", {
                    'source_id': source_id,
                    'target_id': target_id,
                    'changes': changes,
                    'event_type': event_type.value,
    ...
    def _enrich_context(self, event_type: GameEventType, 
                       context: Dict[str, Any], 
                       event_data: Dict[str, Any]) -> None:
        """イベントタイプに応じてコンテキストを enrichment"""
        # クエスト関連イベント
        if event_type in [GameEventType.QUEST_COMPLETED, GameEventType.QUEST_FAILED,
        ...
        # 会話関連イベント
        elif event_type in [GameEventType.PLAYER_TALK_NPC, GameEventType.NPC_TALK_PLAYER]:
            context.update({
        ...
        # アイテム関連イベント
        elif event_type in [GameEventType.PLAYER_GIVE_ITEM_NPC, GameEventType.NPC_GIVE_ITEM_PLAYER]:
            context.update({
        ...
        # 社会的関連イベント
        elif event_type in [GameEventType.PLAYER_CONFESSED_TO_NPC, GameEventType.NPC_CONFESSED_TO_PLAYER]:
            context.update({
        ...
        elif event_type in [GameEventType.PLAYER_ARGUED_WITH_NPC, GameEventType.NPC_ARGUED_WITH_PLAYER]:
            context.update({
        ...
        # 取引関連イベント
        elif event_type in [GameEventType.TRADE_COMPLETED, GameEventType.TRADE_FAILED,
                           GameEventType.CREDIT_EXTENDED, GameEventType.DEFAULT_ON_DEBT]:
            context.update({
    ...
    def register_custom_event_handler(self, event_type: GameEventType, 
                                    handler: Callable[[Dict[str, Any]], None]) -> None:
        """カスタムイベントハンドラーを登録"""
        self._custom_event_handlers[event_type].append(handler)
    
    def unregister_custom_event_handler(self, event_type: GameEventType, 
                                      handler: Callable[[Dict[str, Any]], None]) -> None:
        """カスタムイベントハンドラーの登録を解除"""
        if event_type in self._custom_event_handlers:
            try:
...
# グローバルインスタンス（オプション）
_global_event_handler: Optional[RelationshipEventHandler] = None


def initialize_event_handler(relationship_manager: RelationshipManager) -> RelationshipEventHandler:
    """グローバルイベントハンドラーを初期化"""
...
def get_event_handler() -> Optional[RelationshipEventHandler]:
    """グローバルイベントハンドラーを取得"""
    return _global_event_handler

...
from .models import InteractionType, RelationshipModifier, RelationshipType
...
class CumulativeEffect:
    ...
    def get_effect(self) -> RelationshipModifier | None:
        """累積効果を取得し、カウンターをリセット"""
...
class DynamicRelationshipSystem:
    ...
    def __init__(self, relationship_manager: RelationshipManager):
        ...
        self._cumulative_effects: dict[
            tuple[str, str, RelationshipType, InteractionType], CumulativeEffect
        ] = {}
    ...
    def _calculate_immediate_effect(
        self,
        source_id: str,
        target_id: str,
        interaction_type: InteractionType,
        ...
        if interaction_type == InteractionType.GIFT:
            ...
            changes[RelationshipType.TRUST] = (
                base_amount * 2
            )  # TRUSTは仮のタイプ、実際はFAVORABILITYに含む
    ...
    def _merge_changes(
        self, *change_dicts: dict[RelationshipType, int]
    ) -> dict[RelationshipType, int]:
        """複数の変更辞書をマージ"""

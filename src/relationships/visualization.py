...
class RelationshipVisualizer:
    ...
    def visualize_as_text(self, character_id: Optional[str] = None, 
        ...
        if character_id:
            ...
            if node.faction_affiliations:
                factions_str = ", ".join(f"{k}={v.value}" for k, v in node.faction_affiliations.items())
            ...
            if not related:
                ...
                for other_id, edge in related:
                    ...
                    if detailed:
                        ...
                        lines.append(f"       最終インタラクション: {time.strftime('%Y-%m-%d %H:%M', time.localtime(edge.last_interaction))}")
                        if edge.modifiers:
    ...
    def visualize_as_json(self, character_id: Optional[str] = None) -> str:
        """JSON形式で可視化"""
        if character_id:
            node = self.graph.get_node(character_id)
            if not node:
    ...
    def visualize_as_dot(self, character_id: Optional[str] = None) -> str:
        """Graphviz DOT形式で可視化"""
        ...
        if character_id:
            ...
            for other_id, edge in related:
                ...
                lines.append(f'  "{character_id}" -> "{other_id}" [label="{edge.relationship_type.value}: {edge.level}", color={color}];')
        else:
            # 全体
            for edge in self.graph.edges.values():
                color = self._get_edge_color(edge.level)
                lines.append(f'  "{edge.source_id}" -> "{edge.target_id}" [label="{edge.relationship_type.value}: {edge.level}", color={color}];')
        
        lines.append("}")
        return "\n".join(lines)
    
    def visualize_as_mermaid(self, character_id: Optional[str] = None) -> str:
        """Mermaid形式で可視化"""
        lines = ["graph LR"]
        ...
        if character_id:
            ...
            for other_id, edge in related:
                ...
                lines.append(f'  {character_id}[{character_id}] {line_style} {other_id}[{other_id}]')
    ...
    def export_visualization(self, format: VisualizationFormat = VisualizationFormat.TEXT,
    ...
    def generate_insights(self, character_id: str) -> Optional[RelationshipInsight]:
        """キャラクターの関係洞察を生成"""
        node = self.graph.get_node(character_id)
        if not node:
            return None
        ...
        # 正の関係（上位5）
        positive = sorted(
            [r for r in relationships if r[2] > 0],
        ...
        # 最強の絆
        strongest = sorted(
            relationships, key=lambda x: abs(x[2]), reverse=True
        )[:5]
        
        # カウント
        conflict_count = sum(1 for r in relationships if r[1] == RelationshipType.ENMITY and r[2] < 0)
        romance_count = sum(1 for r in relationships if r[1] == RelationshipType.ROMANCE and r[2] > 20)
        mentorship_count = sum(1 for r in relationships if r[1] == RelationshipType.MENTORSHIP and r[2] > 20)
    ...
    def analyze_graph_health(self) -> Dict[str, Any]:
        ...
        for edge in self.graph.edges.values():
            if edge.level >= 90:
                extreme_positive.append((edge.source_id, edge.target_id, edge.relationship_type.value, edge.level))
            elif edge.level <= -90:
                extreme_negative.append((edge.source_id, edge.target_id, edge.relationship_type.value, edge.level))
    ...
    def trace_relationship_path(self, source_id: str, target_id: str,
                              max_hops: int = 6) -> Optional[List[Tuple[str, str, str]]]:
        """二つのキャラクター間の関係経路を追跡"""
    ...
    def debug_relationship(self, character_a: str, character_b: str,
        ...
        if relationship_type:
            ...
            for i, mod in enumerate(edge.modifiers[-10:], 1):  # 最新10件
                lines.append(f"  {i}. {mod.interaction_type.value}: {mod.amount:+d} "
    ...
    def create_debug_report(self, character_id: Optional[str] = None) -> str:
        """デバッグレポートを作成"""
        lines = []
        lines.append("=" * 70)
...
# デバッグコマンド用のヘルパー関数
def debug_command_visualize(rm: RelationshipManager, character_id: Optional[str] = None, 
                          format: str = "text") -> str:
    """デバッグコマンド：可視化"""
...
def debug_command_debug_relationship(rm: RelationshipManager, 
                                  char_a: str, char_b: str, 
                                  rel_type: Optional[str] = None) -> str:
    """デバッグコマンド：関係デバッグ"""
    visualizer = RelationshipVisualizer(rm)
...
def debug_command_report(rm: RelationshipManager, character_id: Optional[str] = None) -> str:
    """デバッグコマンド：レポート作成"""
    visualizer = RelationshipVisualizer(rm)

...
from .models import RelationshipType, InteractionType, FactionAffiliation
...
class BranchingScenarioGenerator:
                ...
                "branches": [
                    {
                        ...
                        "choices": [
                            {"choice_id": "romantic_confession", "text": "本当の気持ちを伝える", "requirements": {}},
                            {"choice_id": "casual_confession", "text": "軽い気持ちで伝える", "requirements": {}}
                        ]
                    },
                    {
                        "branch_id": "wait",
                        ...
                        "choices": [
                            {"choice_id": "wait_longer", "text": "もう少し関係を深めてから", "requirements": {}}
                        ]
                    }
                ]
            },
                ...
                "branches": [
                    {
                        ...
                        "choices": [
                            {"choice_id": "violent_confront", "text": "暴力で解決する", "requirements": {"combat_skill": 30}},
                            {"choice_id": "verbal_confront", "text": "言葉で問い詰める", "requirements": {}}
                        ]
                    },
                    {
                        "branch_id": "forgive",
                        "description": "許しを乞う",
                        "consequences": {"betrayal_delta": -20, "favorability_delta": 10},
                        "choices": [
                            {"choice_id": "sincere_apology", "text": "心から謝罪する", "requirements": {}}
                        ]
                    }
                ]
            },
                ...
                "branches": [
                    {
                        "branch_id": "accept",
                        "description": "後継者となる",
                        "consequences": {"mentorship_delta": 10, "skill_unlock": "master_technique"},
                        "choices": [
                            {"choice_id": "accept_gracefully", "text": "光栄に受ける", "requirements": {}}
                        ]
                    },
                    {
                        "branch_id": "decline",
                        ...
                        "choices": [
                            {"choice_id": "polite_decline", "text": "感謝しつつ断る", "requirements": {}}
                        ]
                    }
                ]
            },
                ...
                "branches": [
                    {
                        ...
                        "choices": [
                            {"choice_id": "active_support", "text": "積極的に支持する", "requirements": {}},
                            {"choice_id": "passive_support", "text": "静かに支持する", "requirements": {}}
                        ]
                    },
                    {
                        "branch_id": "mediate",
                        "description": "和平を模索する",
                        "consequences": {"faction_reputation_delta": 5, "peace_bonus": 30},
                        "choices": [
                            {"choice_id": "diplomatic_mediation", "text": "外交的に仲介する", "requirements": {"diplomacy": 40}}
                        ]
                    }
                ]
            }
        ...
        for target_id, rel_dict in relationships.items():
            for rel_type, level in rel_dict.items():
                ...
                for template_id, template in self._scenario_templates.items():
                    ...
                    if comparison == "greater_than" and level > threshold:
                        should_trigger = True
                    elif comparison == "less_than" and level < threshold:
                        should_trigger = True
                    elif comparison == "equals" and level == threshold:
                        should_trigger = True
    ...
    def _check_conflict_scenarios(self, player_id: str) -> List[GeneratedScenario]:
        ...
        for (target_a, rel_dict_a), (target_b, rel_dict_b) in itertools.combinations(relationships.items(), 2):
            # 同じタイプの関係で対立があるかチェック
    ...
    def _create_scenario_from_template(self, template_id: str, 
                                     template: Dict[str, Any],
                                     involved_chars: List[str]) -> Optional[GeneratedScenario]:
        """テンプレートからシナリオを作成"""
        # 重複チェック
        dedupe_key = (ScenarioTriggerType.RELATIONSHIP_THRESHOLD, tuple(involved_chars), template_id)
    ...
    def _create_conflict_scenario(self, player_id: str, target_a: str, target_b: str, 
                                rel_type: RelationshipType) -> Optional[GeneratedScenario]:
        """関係対立シナリオを作成"""
        dedupe_key = (ScenarioTriggerType.RELATIONSHIP_CONFLICT, (player_id, target_a, target_b), rel_type.value)
        if self._is_recently_generated(dedupe_key):
            return None
        ...
        branches = [
            ScenarioBranch(
                ...
                available_choices=[{"choice_id": "side_with_a", "text": f"{name_a}の味方をする", "requirements": {}}]
            ),
            ScenarioBranch(
                branch_id="support_b",
                description=f"{name_b}を支持する",
                consequences={"favorability_delta": {target_b: 10, target_a: -10}},
                available_choices=[{"choice_id": "side_with_b", "text": f"{name_b}の味方をする", "requirements": {}}]
            ),
            ScenarioBranch(
                branch_id="mediate",
                description="両者を仲介する",
                consequences={"favorability_delta": {target_a: 5, target_b: 5, "diplomacy_bonus": 15}},
                available_choices=[{"choice_id": "mediate_conflict", "text": "平和的に解決する", "requirements": {"diplomacy": 30}}]
            )
        ]
    ...
    def _create_triangular_scenario(self, player_id: str, target_ids: List[str]) -> Optional[GeneratedScenario]:
        """三角関係シナリオを作成"""
        dedupe_key = (ScenarioTriggerType.TRIANGULAR_RELATIONSHIP, tuple([player_id] + target_ids), "love_triangle")
        if self._is_recently_generated(dedupe_key):
            return None
        ...
        branches = [
            ScenarioBranch(
                ...
                available_choices=[
                    {"choice_id": f"choose_{tid}", "text": f"{self._get_character_name(tid)}を選ぶ", "requirements": {}}
            ...
            ScenarioBranch(
                ...
                available_choices=[{"choice_id": "keep_options", "text": "曖昧なままにする", "requirements": {}}]
            )
        ]
    ...
    def _create_faction_scenario(self, player_id: str, player_faction: str, 
                               enemy_faction: str, members: List[str]) -> Optional[GeneratedScenario]:
        """派閥緊張シナリオを作成"""
        dedupe_key = (ScenarioTriggerType.FACTION_TENSION, (player_id, player_faction, enemy_faction), "tension")
        if self._is_recently_generated(dedupe_key):
            return None
        ...
        branches = [
            ScenarioBranch(
                ...
                available_choices=[{"choice_id": "escalate_conflict", "text": "派閥の誇りを守る", "requirements": {}}]
            ),
            ScenarioBranch(
                branch_id="seek_peace",
                description="和平を模索する",
                consequences={"faction_reputation_delta": -10, "peace_bonus": 35},
                available_choices=[{"choice_id": "seek_peace", "text": "和平交渉を提案する", "requirements": {"diplomacy": 50}}]
            )
        ]
    ...
    def apply_scenario_choice(self, scenario: GeneratedScenario, branch_id: str, 
                            choice_id: str, player_id: str = "player") -> Dict[str, Any]:
        """プレイヤーの選択を適用し、結果を返す"""
    ...
    def _mark_as_generated(self, dedupe_key: Tuple) -> None:
        ...
        self._recently_generated = [
            (k, t) for k, t in self._recently_generated

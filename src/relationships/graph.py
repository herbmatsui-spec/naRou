...
from .models import (
    RelationshipType, InteractionType,
...
class RelationshipGraph:
    ...
    def get_node(self, character_id: str) -> Optional[RelationshipNode]:
        """ノードを取得"""
    ...
    def remove_edge(self, source_id: str, target_id: str, relationship_type: RelationshipType) -> bool:
        """エッジを削除"""
        return self._remove_edge_internal(source_id, target_id, relationship_type)
    
    def _remove_edge_internal(self, source_id: str, target_id: str, relationship_type: RelationshipType) -> bool:
        """内部エッジ削除メソッド"""
    ...
    def get_edge(self, source_id: str, target_id: str, relationship_type: RelationshipType) -> Optional[RelationshipEdge]:
        """特定の関係タイプのエッジを取得"""
    ...
    def get_related_nodes(self, character_id: str, relationship_type: Optional[RelationshipType] = None) -> List[Tuple[str, RelationshipEdge]]:
    ...
    def get_strongest_relationships(self, character_id: str, limit: int = 10, 
                                  relationship_type: Optional[RelationshipType] = None) -> List[Tuple[str, RelationshipEdge, int]]:
        """最も強い関係（絶対値で）を取得"""
        relationships = []
    ...
    def get_positive_relationships(self, character_id: str, 
                                 relationship_type: Optional[RelationshipType] = None,
                                 min_level: int = 20) -> List[Tuple[str, RelationshipEdge]]:
        """肯定的な関係を取得"""
        positive = []
        for other_id, edge in self.get_related_nodes(character_id, relationship_type):
    ...
    def get_negative_relationships(self, character_id: str,
                                 relationship_type: Optional[RelationshipType] = None,
                                 max_level: int = -20) -> List[Tuple[str, RelationshipEdge]]:
        """否定的な関係を取得"""
        negative = []
        for other_id, edge in self.get_related_nodes(character_id, relationship_type):
    ...
    def apply_decay_to_all(self, current_time: Optional[float] = None) -> Dict[str, int]:
    ...
    def find_path(self, source_id: str, target_id: str, 
                  relationship_type: Optional[RelationshipType] = None,
                  max_hops: int = 6) -> Optional[List[str]]:
        """二つのノード間のパスを見つける（BFS）"""
    ...
    def calculate_clustering_coefficient(self, character_id: str) -> float:
        ...
        for i, neighbor_a in enumerate(neighbors):
            for neighbor_b in neighbors[i+1:]:
                # 両方向のエッジをチェック
                if self.has_edge(neighbor_a, neighbor_b, RelationshipType.FRIENDSHIP) or \
                   self.has_edge(neighbor_b, neighbor_a, RelationshipType.FRIENDSHIP):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> RelationshipGraph:
        ...
        # 統計情報を復元
        graph.stats.update(data.get("stats", {"node_count": 0, "edge_count": 0, "last_updated": time.time()}))

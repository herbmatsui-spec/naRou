...
class WorldStateRelationshipIntegration:
    ...
    def __init__(self, relationship_manager: RelationshipManager, 
        ...
        # フェーズごとの関係修飾子
        self._phase_modifiers: Dict[WorldPhase, Dict[RelationshipType, float]] = self._load_phase_modifiers()
        ...
        # 最後のフェーズ
        self._last_phase: Optional[WorldPhase] = None
                ...
                if rel_type in modifiers:
                    ...
                    if delta != 0:
                        self.rm.modify_relationship(
                            player_id, target_id,
                            InteractionType.EMOTIONAL_SUPPORT if delta > 0 else InteractionType.ARGUMENT,
                            delta
                        )
    ...
    def trigger_world_event(self, event_id: str, event_type: WorldEventImpact,
                          affected_characters: Optional[List[str]] = None,
                          magnitude: int = 10,
                          duration_days: int = 7,
                          description: str = "") -> WorldEventRelationshipEffect:
        """世界イベントを発動し、関係に影響"""
        ...
        if event_type == WorldEventImpact.UNIFYING:
            ...
            rel_types = [RelationshipType.FAVORABILITY, RelationshipType.FRIENDSHIP, RelationshipType.MENTORSHIP]
            ...
            delta_range = (-magnitude * 2, -magnitude)
            rel_types = [RelationshipType.ENMITY, RelationshipType.BETRAYAL, RelationshipType.FAVORABILITY]
        elif event_type == WorldEventImpact.CELEBRATORY:
            delta_range = (magnitude, magnitude * 2)
            rel_types = [RelationshipType.FAVORABILITY, RelationshipType.ROMANCE, RelationshipType.FRIENDSHIP]
        elif event_type == WorldEventImpact.CHAOTIC:
            delta_range = (-magnitude, magnitude)  # ランダムな方向
            rel_types = [RelationshipType.ENMITY, RelationshipType.BETRAYAL, RelationshipType.FAVORABILITY, RelationshipType.ROMANCE]
        else:  # CALMING
            delta_range = (magnitude // 2, magnitude)
        ...
        for i in range(len(affected_characters)):
            for j in range(i + 1, len(affected_characters)):
                ...
                for edge in edges:
                    ...
                    self.rm.modify_relationship(char_a, char_b, InteractionType.EMOTIONAL_SUPPORT, delta)
    ...
    def calculate_global_trend(self, player_id: str = "player") -> WorldRelationshipTrend:
        ...
        global_favorability_avg = favorability_sum / favorability_count if favorability_count > 0 else 0.0
    ...
    def predict_world_relationship_future(self, days_ahead: int = 30, 
                                       player_id: str = "player") -> Dict[str, Any]:
        """未来の世界関係状態を予測"""
    ...
    def _predict_future_phase(self, days_ahead: int) -> Optional[WorldPhase]:
        """未来のフェーズを予測（簡易実装）"""
        if self.world_state_manager:
            try:

...
class QuestRelationshipIntegration:
    ...
    def __init__(
        ...
        branching_generator: BranchingScenarioGenerator | None = None,
    ):
    ...
    def register_quest_relationship_gate(
        self,
        quest_id: str,
        requirements: list[dict[str, Any]],
        success_bonus: dict[str, Any] | None = None,
        failure_consequences: dict[str, Any] | None = None,
    ) -> QuestRelationshipGate:
        """クエストの関係ゲートを登録"""
    ...
    def register_quest_reward_relationship(
        self,
        quest_id: str,
        target_character: str,
        relationship_type: RelationshipType,
    ...
    def check_quest_availability(
        ...
        for req in gate.requirements:
            ...
            if requirement_type == QuestRelationRequirement.MIN_LEVEL.value:
                ...
                # 敵対関係がないことを確認
                enmity_level = self.rm.get_relationship_level(
                    player_id, target_id, RelationshipType.ENMITY
                )
                if enmity_level > 0:
                    unmet_requirements.append(
                        {
                ...
                romance_level = self.rm.get_relationship_level(
                    player_id, target_id, RelationshipType.ROMANCE
                )
                if romance_level < 20:
                    unmet_requirements.append(
                        {
                ...
                mentorship_level = self.rm.get_relationship_level(
                    player_id, target_id, RelationshipType.MENTORSHIP
                )
    ...
    def apply_quest_completion_effects(
        self, quest_id: str, player_id: str = "player", success: bool = True
    ) -> dict[str, Any]:
        """クエスト完了時の関係効果を適用"""
        results = {}
        ...
        if success:
            ...
            for npc in npcs:
                if npc != player_id:
                    self.rm.modify_relationship(
                        player_id, npc, InteractionType.QUEST_COOPERATION, 10
                    )
    ...
    def generate_relationship_quest(
        self, player_id: str = "player", character_id: str | None = None
    ...
    def get_quest_relationship_status(
        self, quest_id: str, player_id: str = "player"
    ) -> dict[str, Any]:
        """クエストの関係ステータスを取得"""
        gate = self.quest_gates.get(quest_id)
    ...
    def _on_quest_completed(
        ...
        for target, changes in results.items():
            if "delta" in changes:
                logs.append(
                    f"【関係変化】{target}との{changes['relationship_type']}関係が{changes['delta']:+d}されました"
                )
            if "bonus_delta" in changes:
                logs.append(
                    f"【ボーナス関係】{target}との{changes['bonus_type']}関係が{changes['bonus_delta']:+d}されました"
                )

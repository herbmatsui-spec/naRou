...
class KarmaRegistry:
    ...
    def get(self, karma_id: str = "default") -> Optional[KarmaData]:
        """特定のカーマデータを取得"""
...
class KarmaManager:
    ...
    def __init__(self, registry: Optional[KarmaRegistry] = None):
    ...
    def update_karma(self, player: Any, action: str, amount: int = 1) -> Tuple[int, int]:
        ...
        player.karma_law_chaos = max(law_chaos_range[0], min(law_chaos_range[1], player.karma_law_chaos))
        player.karma_good_evil = max(good_evil_range[0], min(good_evil_range[1], player.karma_good_evil))

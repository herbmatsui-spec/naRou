...
scenes = [
    # Scene 1: Start in Cave Home with Ciel
    {
        ...
        "logs": [
            ...
            (
                "【初心者ガイド】[?]キーでいつでもヘルプ・操作一覧を確認できます！",
                (120, 255, 200),
            ),
            (
                "【操作】矢印:移動 [Space]:行動 [l]:調査 [i]:荷物 [Shift+S]:ツリー",
                (180, 220, 255),
            ),
        ],
        "fx": [],
    ...
    # Scene 2: Combat in Puppy Cave
    {
        ...
        "fx": [
            ("text", "-18", (13, 5.5), (255, 100, 100)),
            ("particle", "💥", (13, 6), (255, 180, 50)),
        ],
    },
    # Scene 3: Advanced Systems (Skill Tree & High Tier Dungeon)
    {
        ...
        "fx": [
            ("text", "-248 CRIT!", (16, 6), (255, 230, 80)),
            ("particle", "✧", (10, 7), (180, 140, 255)),
        ],
    },
    # Scene 4: Reincarnation & NG+ Loop
    {
        ...
        "logs": [
            ...
            (
                "獲得ボーナス: 基礎能力値継承 / 固有称号『伝説の救世主』獲得",
                (255, 240, 120),
            ),
            ("次なる旅路へ… naRou: Masterpiece Edition", (200, 220, 255)),
        ],
        "fx": [
            ("particle", "✨", (13, 7), (255, 240, 100)),
            ("particle", "✨", (14, 5), (255, 215, 0)),
        ],
    },
]
...
for sc_idx, sc in enumerate(scenes):
    for sub in range(4):
        ...
        draw.text(
            (540, 16),
            f"💰 {sc['gold']}G  💎 {sc['plat']}P  ⚖️ {sc['karma']}  Turn:{sc['turn']}",
            fill=(200, 220, 255),
        )
        ...
        draw.rectangle(
            [(MAP_X, MAP_Y + 30), (MAP_X + map_w, MAP_Y + 30 + map_h)],
            fill=(12, 14, 22),
        ...
        for r in range(MAP_ROWS):
            for c in range(MAP_COLS):
                ...
                if dist > 10:
                    ...
                    if is_wall:
                        draw.rectangle(
                            [(tx, ty), (tx + CELL_W, ty + CELL_H)],
                            fill=COLOR_WALL_LIT if dist < 5 else (90, 75, 60),
                        )
                        draw.rectangle(
                            [(tx + 1, ty + 1), (tx + CELL_W - 1, ty + CELL_H - 1)],
                            fill=(45, 38, 30),
                        )
                    else:
        ...
        if sc["altar_pos"]:
            ax, ay = sc["altar_pos"]
            draw.text(
                (MAP_X + ax * CELL_W + 3, MAP_Y + 30 + ay * CELL_H),
                "⛩️",
                fill=COLOR_ALTAR,
            )
        ...
        sx, sy = sc["stairs_pos"]
        draw.text(
            (MAP_X + sx * CELL_W + 5, MAP_Y + 30 + sy * CELL_H),
            ">",
            fill=(100, 200, 255),
        )
        ...
        # Pet
        draw.text(
            (MAP_X + pet_x * CELL_W + 4, MAP_Y + 30 + pet_y * CELL_H + bob),
            "p",
            fill=COLOR_PET_PINK,
        )
        # Player
        draw.text(
            (MAP_X + px * CELL_W + 4, MAP_Y + 30 + py * CELL_H + bob),
            "@",
            fill=(255, 255, 255),
        )
        ...
        for fx_type, fx_val, (fx_x, fx_y), fx_col in sc["fx"]:
            draw.text(
                (
                    MAP_X + int(fx_x * CELL_W) - 4,
                    MAP_Y + 30 + int(fx_y * CELL_H) - sub * 2,
                ),
                fx_val,
                fill=fx_col,
            )
        ...
        # Minimap Frame
        draw.rectangle(
            [(side_x, MAP_Y + 30), (side_x + side_w, MAP_Y + 120)],
            fill=PANEL_BG,
            outline=PANEL_BORDER,
            width=1,
        )
        ...
        draw.rectangle(
            [(side_x, MAP_Y + 128), (side_x + side_w, MAP_Y + 30 + map_h)],
            fill=PANEL_BG,
            outline=PANEL_BORDER,
            width=1,
        )
        draw.text(
            (side_x + 8, MAP_Y + 134),
            f"名無しの冒険者 Lv.{sc['level']}",
            fill=COLOR_GOLD_YELLOW,
        )
        draw.text(
            (side_x + 8, MAP_Y + 152),
            f"職業: ウォーリア ({sc['hunger']})",
            fill=TEXT_WHITE,
        )
        ...
        draw.rectangle(
            [(side_x + 8, MAP_Y + 208), (side_x + 8 + hp_len, MAP_Y + 214)],
            fill=COLOR_HP_GREEN,
        )
        ...
        draw.rectangle(
            [(side_x + 8, MAP_Y + 240), (side_x + 8 + mp_len, MAP_Y + 246)],
            fill=COLOR_MP_BLUE,
        )

...
class BaseFacility:
    ...
    required_secret_skill: Optional[str] = None
...
class SkillEaterEconomySystem:
    def __init__(
        ...
        self.factions: Dict[str, FactionState] = {
            "midas": FactionState(id="midas", name="ミダス・ホールディングス", reputation=-50, is_hostile=True),
            "resistance": FactionState(id="resistance", name="スキル開放戦線", reputation=30, is_hostile=False),
            "bank": FactionState(id="bank", name="世界スキル銀行", reputation=0, is_hostile=False),
            "broker": FactionState(id="broker", name="闇市場ブローカー", reputation=10, is_hostile=False)
        }
    ...
    def sell_skill_to_black_market(self, player: CharacterState, skill_id: str) -> Tuple[bool, int, str]:
        ...
        if skill_def.is_illegal:
            ...
            return True, value, f"《{skill_def.name}》（違法合成品）を闇市場に密売し、{value} アルドを獲得！（警戒度: {self.heat_level}）"

        return True, value, f"《{skill_def.name}》を闇市場に売却し、{value} アルドを獲得しました！"

    def sell_skill_to_normal_market(self, player: CharacterState, skill_id: str) -> Tuple[bool, int, str]:
        ...
        if skill_def.is_illegal:
            ...
            return False, 0, f"査定拒絶：『{skill_def.name}』は認可外の違法合成スキルです。公認市場では取り扱えません。"
        ...
        return True, value, f"《{skill_def.name}》を正規市場に売却し、{value} アルドを獲得しました。"

    def check_inspector_raid(self) -> Tuple[bool, Optional[CharacterState], str]:
    ...
    def takeover_branch(self, branch_name: str, seized_aldo: int) -> str:
        ...
        self.factions["midas"].influence_points = max(0, self.factions["midas"].influence_points - 300)
        self.factions["resistance"].influence_points += 300
        self.factions["resistance"].reputation = min(100, self.factions["resistance"].reputation + 20)
    ...
    def upgrade_facility(self, player: CharacterState, facility_id: str) -> Tuple[bool, str]:
        ...
        if self.aldo_currency < facility.upgrade_cost_aldo:
            ...
            return False, f"アルドが不足しています。（必要: {facility.upgrade_cost_aldo} アルド, 所持: {self.aldo_currency} アルド）"

        if facility.required_secret_skill and not player.has_skill(facility.required_secret_skill):
        ...
        return True, f"【施設強化完了】{facility.name} が Lv.{facility.level} に昇格しました！"
